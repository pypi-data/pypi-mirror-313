...
import numpy as np
from pathlib import Path
from fasr.utils import read_yaml, timer
from fasr.data import Audio, AudioList, AudioChannel, AudioSpan, AudioSpanList
import torch
import os

DEBUG = True if os.environ.get("FASR_DEBUG_MODE", "0") == "1" else False
...
class SegmentPredictor:
    ...
    @timer("reset all", round_num=3, log_time=DEBUG)
    ...
    @timer("compute db", round_num=3, log_time=DEBUG)
    ...
    @timer("compute db", round_num=4, log_time=DEBUG)
    ...
    @timer("scorer", round_num=3, log_time=DEBUG)
    def __call__(
        self,
        score: np.ndarray,
        waveform: np.ndarray,
    ...
    @timer("detect common frames", round_num=5, log_time=DEBUG)
    ...
    @timer("detect last frames", round_num=5, log_time=DEBUG)
