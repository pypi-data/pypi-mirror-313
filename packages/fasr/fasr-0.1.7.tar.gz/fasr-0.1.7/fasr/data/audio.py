...
class AudioToken(BaseDoc):
    ...
    waveform: NdArray | Tensor | None = Field(
        None, description="Waveform of the token."
    )
...
class AudioSpan(BaseDoc):
    ...
    waveform: NdArray | Tensor | None = Field(
        None, description="Waveform of the segment."
    )
    ...
    def display(self):
        ...
        return IPAudio(data=self.waveform, rate=self.sample_rate)
...
class AudioChannel(BaseDoc):
    ...
    waveform: Tensor | NdArray | None = Field(
        None, description="Waveform of the audio file."
    )
    sample_rate: int | None = Field(None, description="Sample rate of the audio file.")
    feats: NdArray | Tensor | None = Field(
        None,
        description="Features of the audio file provided by the frontend.",
        exclude=True,
    )
    steps: Optional[AudioSpanList[AudioSpan]] = Field(
        None,
        description="Steps of the audio file provided by the detector.",
        exclude=True,
    )
    ...
    def display(self, start_ms: float | None = None, end_ms: float | None = None):
        ...
        data = self.select_waveform(start_ms=start_ms, end_ms=end_ms)

        return IPAudio(data=data, rate=self.sample_rate)
...
class Audio(BaseDoc):
    ...
    url: Optional[Union[str, Path]] = Field(None, description="URL of the audio file.")
    sample_rate: Optional[int] = Field(
        None, description="Sample rate of the audio file."
    )
    waveform: NdArray | Tensor | None = Field(
        None, description="Waveform of the audio file."
    )
    ...
    mono: Optional[bool] = Field(None, description="Whether the audio file is mono.")
    duration: Optional[float] = Field(None, description="Duration of the audio file.")
    pipeline: Optional[List[str]] = Field(None, description="List of processing steps.")
    is_bad: Optional[bool] = Field(None, description="Whether the audio file is bad.")
    is_last: Optional[bool] = Field(
        None, description="Whether the audio file is the last."
    )
    bad_reason: Optional[Union[str, Exception]] = Field(
    ...
    def resample_channel(self, sample_rate: int) -> "Audio":
        ...
        for channel in self.channels:
            data = librosa.resample(
                channel.waveform, orig_sr=channel.sample_rate, target_sr=sample_rate
            )
    ...
    def chunk(self, chunk_size_ms: int) -> "AudioList['Audio']":
        ...
        if self.duration_ms > chunk_size_ms:
            for i in range(num_chunks):
                ...
                if self.channels is not None:
                    for channel in self.channels:
                        chunk_waveform = channel.waveform[start:end]
                        chunk_channel = AudioChannel(
                            id=channel.id,
                            waveform=chunk_waveform,
                            sample_rate=channel.sample_rate,
                        )
                        chunk_audio.append_channel(chunk_channel)
...
class AudioList(DocList):
    ...
    def save_binary(
        ...
        Args:
            ...
            protocol (Literal[ &quot;protobuf&quot;, &quot;pickle&quot;, &quot;json&quot;, &quot;json, optional): _description_. Defaults to &quot;protobuf&quot;.
            compress (Literal[&quot;lz4&quot;, &quot;bz2&quot;, &quot;lzma&quot;, &quot;zlib&quot;, &quot;gzip&quot;] | None, optional): _description_. Defaults to None.
            show_progress (bool, optional): _description_. Defaults to False.
            filter_bad (bool, optional): _description_. Defaults to False.
        """
    ...
    def clear(self, clear_text: bool = False) -> "AudioList":
        for audio in self:
            audio: Audio
...
class AudioExample(BaseDoc):
    ...
    def validate_audio_example(self):
        if len(self.x.channels) != len(self.y.channels):
            raise ValueError(
                f"Input and target audio must have the same number of channels."
            )
        return self
