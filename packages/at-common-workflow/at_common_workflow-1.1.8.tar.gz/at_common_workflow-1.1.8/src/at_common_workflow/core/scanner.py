...
class TaskScanner:
    ...
    @staticmethod
    def load(name: str, directory: str) -> Optional[Task]:
        """
        Load a task by its name from a directory.
        
        Args:
            name: The name of the task to find
            directory: Directory to search for the task
            
        Returns:
            Task object if found, None otherwise
        """
        # Reuse scan method to get all tasks
        tasks = TaskScanner.scan(directory)
                ...
                for root, _, files in os.walk(directory):
                    for file in files:
                        if file.endswith('.py'):
                            try:
                                file_path = Path(root) / file
                                spec = importlib.util.spec_from_file_location(task_info["name"], str(file_path))
                                if spec is None or spec.loader is None:
                                    continue
                                module = importlib.util.module_from_spec(spec)
                                spec.loader.exec_module(module)
                                ...
                                for _, obj in inspect.getmembers(module):
                                    if (inspect.isfunction(obj) and 
                                        hasattr(obj, '_is_workflow_task') and 
                                        getattr(obj, '_name', obj.__name__) == name):
                                        
                                        return Task(
                                            name=getattr(obj, '_name', obj.__name__),
                                            description=getattr(obj, '_description', None) or obj.__doc__ or "",
                                            func=obj,
                                            parameters=getattr(obj, '_parameters', {}),
                                            requires=getattr(obj, '_requires', {}),
                                            provides=getattr(obj, '_provides', {})
                                        )
                            except Exception:
                                continue
        return None
