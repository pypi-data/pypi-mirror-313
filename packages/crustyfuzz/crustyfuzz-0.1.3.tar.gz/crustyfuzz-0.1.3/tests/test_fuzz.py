...
def testQuickRatioEqual():
    assert (
        fuzz.QRatio("new york mets", "new york mets", processor=utils.default_process)
        == 100
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (fuzz.partial_token_sort_ratio, 100),
        (fuzz.token_ratio, 100),
        (fuzz.partial_token_ratio, 100),
        # no match
        (fuzz.token_set_ratio, 0),
        (fuzz.partial_token_set_ratio, 0),
        (fuzz.WRatio, 0),
        (fuzz.QRatio, 0),
    ],
)
def test_empty_string(scorer, expected: int) -> None:
    """
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda s: utils.default_process(s),
    ],
)
@pytest.mark.parametrize("scorer", scorers)
...
def test_help(scorer):
    ...
    since they are implemented in Rust as well
    """
    help(scorer)
