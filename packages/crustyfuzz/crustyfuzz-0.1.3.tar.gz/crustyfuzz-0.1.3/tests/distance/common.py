...
# from crustyfuzz._feature_detector import AVX2, supports
from crustyfuzz.distance import metrics
from tests.common import GenericScorer, Scorer, is_none

scorer_modules = [metrics]
# try:
#     if supports(AVX2):
...
def create_generic_scorer(func_name, get_scorer_flags):
    scorers = [
        Scorer(
            ...
            editops=getattr(metrics, func_name + "_editops", None),
            opcodes=getattr(metrics, func_name + "_opcodes", None),
        )
        for mod in scorer_modules
    ]

    return GenericScorer(scorers, get_scorer_flags)
...
DamerauLevenshtein = create_generic_scorer(
    "damerau_levenshtein",
    get_scorer_flags_damerau_levenshtein,
)
