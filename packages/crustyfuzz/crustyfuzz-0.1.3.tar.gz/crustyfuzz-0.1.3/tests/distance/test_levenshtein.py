"""Tests for the levenshtein distance metrics."""

from __future__ import annotations

from crustyfuzz import utils
from crustyfuzz.distance import Editop, Opcode
...
def test_Editops():
    ...
    """
    assert Levenshtein.editops("0", "").as_list() == [Editop("delete", 0, 0)]
    assert Levenshtein.editops("", "0").as_list() == [Editop("insert", 0, 0)]

    assert Levenshtein.editops("00", "0").as_list() == [Editop("delete", 1, 1)]
    assert Levenshtein.editops("0", "00").as_list() == [Editop("insert", 1, 1)]
...
def test_Opcodes():
    ...
    """
    assert Levenshtein.opcodes("", "abc").as_list() == [Opcode("insert", 0, 0, 0, 3)]
