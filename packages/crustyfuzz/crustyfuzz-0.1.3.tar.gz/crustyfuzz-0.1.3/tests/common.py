"""Common parts of the test suite for crustyfuzz."""
...
import pytest
from crustyfuzz import process

try:
    from pandas import NA as pandas_NA
except BaseException:
    pandas_NA = None  # type: ignore[assignment]


def _get_scorer_bounds(scorer: Any, scorer_kwargs: dict[str, Any]) -> tuple[int, int]:
    # TODO: might need to get the rust funcs to pass args (Levenshtein)
...
def scorer_tester(scorer, s1, s2, catch_exceptions=False, **kwargs):
    score = call_and_maybe_catch(scorer, s1, s2, **kwargs)
    exception = isinstance(score, Exception)
    ...
    extract_one_res1 = call_and_maybe_catch(
        process.extract_one,
        s1,
        [s2],
    ...
    extract_iter_res1 = call_and_maybe_catch(
        list,
        process.extract_iter(s1, [s2], scorer=scorer, **process_kwargs),
        catch_exceptions=catch_exceptions,
    )

    if exception:
        assert compare_exceptions(extract_one_res1, score)
        assert compare_exceptions(extract_res1, score)
        assert compare_exceptions(extract_iter_res1, score)
    elif is_none(s1) or is_none(s2):
        assert extract_one_res1 is None
        assert extract_res1 == []
        assert extract_iter_res1 == []
    elif kwargs.get("score_cutoff") is not None:
        worst_score, optimal_score = _get_scorer_bounds(
            scorer,
            process_kwargs.get("scorer_kwargs", {}),
        )
        lowest_score_worst = optimal_score > worst_score
        is_filtered = (
            score < kwargs["score_cutoff"]
            if lowest_score_worst
            else score > kwargs["score_cutoff"]
        )
        ...
        if is_filtered:
            ...
            assert pytest.approx(score) == extract_one_res1[1]
            assert pytest.approx(score) == extract_res1[0][1]
            assert pytest.approx(score) == extract_iter_res1[0][1]
    else:
        assert pytest.approx(score) == extract_one_res1[1]
        assert pytest.approx(score) == extract_res1[0][1]
        assert pytest.approx(score) == extract_iter_res1[0][1]
    ...
    #
    #     if exception:
    #         assert compare_exceptions(cdist_scores1, score)
    #         assert compare_exceptions(cdist_scores3, score)
    #     else:
    #         assert np.all(np.isclose(cdist_scores1, score))
    #         assert np.all(np.isclose(cdist_scores3, score))
...
class GenericScorer:
    def __init__(self, scorers, get_scorer_flags):
    ...
    def _editops(self, s1, s2, catch_exceptions=False, **kwargs):
        results = [
            call_and_maybe_catch(
                scorer.editops, s1, s2, catch_exceptions=catch_exceptions, **kwargs
            )
            for scorer in self.scorers
        ]
    ...
    def _opcodes(self, s1, s2, catch_exceptions=False, **kwargs):
        results = [
            call_and_maybe_catch(
                scorer.opcodes, s1, s2, catch_exceptions=catch_exceptions, **kwargs
            )
            for scorer in self.scorers
        ]
    ...
    def _validate(self, s1, s2, catch_exceptions=False, **kwargs):
        ...
        dist = call_and_maybe_catch(
            self._distance, s1, s2, catch_exceptions=catch_exceptions, **kwargs
        )
        sim = call_and_maybe_catch(
            self._similarity, s1, s2, catch_exceptions=catch_exceptions, **kwargs
        )
        norm_dist = call_and_maybe_catch(
    ...
    def distance(self, s1, s2, catch_exceptions=False, **kwargs):
        dist, _, _, _ = self._validate(
            s1, s2, catch_exceptions=catch_exceptions, **kwargs
        )
    ...
    def normalized_distance(self, s1, s2, catch_exceptions=False, **kwargs):
        if not is_none(s1) and not is_none(s2):
            _, _, norm_dist, _ = self._validate(
                s1, s2, catch_exceptions=catch_exceptions, **kwargs
            )
    ...
    def normalized_similarity(self, s1, s2, catch_exceptions=False, **kwargs):
        if not is_none(s1) and not is_none(s2):
            _, _, _, norm_sim = self._validate(
                s1, s2, catch_exceptions=catch_exceptions, **kwargs
            )
            if "score_cutoff" not in kwargs:
                return norm_sim
        return self._normalized_similarity(
            s1, s2, catch_exceptions=catch_exceptions, **kwargs
        )
