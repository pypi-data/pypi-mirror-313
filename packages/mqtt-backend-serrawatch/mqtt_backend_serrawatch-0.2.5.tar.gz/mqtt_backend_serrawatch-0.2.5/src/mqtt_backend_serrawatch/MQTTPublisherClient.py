...
class MQTTPublisherClient:
    def __init__(self, topics: TopicsConfig, broker_address: str, port: int = 1883):
        ...
        self.logger.debug(self.logger,"MQTTPublisherClient initialized with broker: {broker_address}, port: {port}, topics: {topics}")
    ...
    def on_connect(self, client, userdata, flags, rc):
        ...
        """
        if rc == 0:
            self.logger.info(self.logger,f"Connected to broker {self.broker}:{self.port} successfully")
        else:
            self.logger.error(self.logger,f"Connection error with code: {rc}")

    def connect(self):
        """
        Connects the client to the MQTT broker and starts the network loop.
        """
        self.logger.debug(self.logger,f"Attempting to connect to broker {self.broker}:{self.port}")
        self.client.connect(self.broker, self.port, keepalive=60)
        self.client.loop_start()
        self.logger.info(self.logger,"MQTT loop started")
    ...
    def publish(self, message_type: str, value: str):
        ...
        """
        self.logger.debug(self.logger,f"Attempting to publish message of type: {message_type} with value: {value}")
        if message_type in self.topics.publishers:
            topic = self.topics.publishers[message_type]
            self.logger.debug(self.logger,f"Publishing to topic: {topic}")
            self.client.publish(topic, value)
            self.logger.info(self.logger,f"Message published: {value} to {topic}")
        else:
            self.logger.warning(self.logger,f"Unrecognized message type: {message_type}")
    ...
    def publish_on_interrupt(self, interrupt_event, message_type: str, value: str):
        ...
        :param value: The value to publish.
        """
        self.logger.debug(self.logger,"Waiting for an interrupt event to publish a message...")
        while not interrupt_event.is_set():
            interrupt_event.wait()  # Wait for the interrupt to be triggered
            self.logger.info(self.logger,"Interrupt triggered, publishing the message...")
    ...
    def disconnect(self):
        ...
        """
        self.logger.debug(self.logger,"Attempting to disconnect from MQTT broker")
        self.client.loop_stop()
        self.client.disconnect()
        self.logger.info(self.logger,"Disconnected from broker")
