...
def usaggregate(data, level, 
    ...
    def align_column_names(df, zip_relations, tracts):
        """
        Aligns column names in the DataFrame based on matching values in zip_relations or tracts.
        """
        df = df.astype(str)
       ...
       # Skip normalization and matching for columns named 'Date' or 'Year'
           if col in ['Date', 'Year']:
               continue
           
           # Randomly sample up to 100 values from the column
           ...
           for ref_name, ref_df in reference_dataframes.items():
               for ref_col in ref_df.columns:
                   # Check if any sample value matches the reference column's unique values
                   if set(sample_values) & set(ref_df[ref_col].dropna().unique()):
                       df.rename(columns={col: ref_col}, inplace=True)
                       break  # Stop checking further reference columns once a match is found
               
               # If the column was renamed, stop checking other DataFrames
    ...
    aggregated_data = []
    for df in data:
        df = align_column_names(df, zip_relations, tracts)
