...
def init_graph():
    ...
    for ax in axes.flat:  # 对于多行，遍历所有子图并关闭轴
        ax.axis('off')
    return figure,axes
def subplots_show(axes, x, t, predict, t_alpha=1, label=None):
    # if label is None:
    #     label=''
    axes[0].cla()
    axes[0].axis('off')
    ...
    predict_feature=predict.reshape(1, predict.shape[0], predict.shape[1], predict.shape[2])
    predict_feature=datatofeature(predict_feature,mode='label')
    ...
    # axes[0].set_title('Feature' + label)
    # axes[1].set_title('Label' + label)
