...
class BERTRunner:
    ...
    def get_active_model(self, scope_code):
        ...
        try:
            response = self.api_client.post_data(endpoint=endpoint, data=data)
    ...
    def download_model(self, run_id):
        ...
        try:
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                MINIO_CLIENT.fget_object(MINIO_BUCKET, f"model/{run_id}.zip", download_path)
                os.system(f"unzip {download_path} -d {model_path}")      
