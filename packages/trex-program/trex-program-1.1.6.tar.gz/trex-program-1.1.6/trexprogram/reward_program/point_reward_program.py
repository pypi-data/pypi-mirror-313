...
class PointSchemeProgram(SchemeRewardProgram):
    ...
    def __init__(self, merchant_acct, program_configuration):
        super(PointSchemeProgram, self).__init__(merchant_acct, program_configuration)
    ...
    def give(self, customer_acct, transaction_details, reward_set=1): 
        if self.is_eligible_based_on_exclusivity(customer_acct) and self.is_eligible_for_limited_to_specific_day_condition(transaction_details):
            
            logger.debug('PointSchemeProgram: Going to give reward')
            ...
            balance_of_reward_amount_limit  = self.get_balacne_of_reward_amount_limit(customer_acct, transaction_details)
...
class PointGiveawayProgram(GiveawayRewardBaseProgram):
    
    def __init__(self, merchant_acct, program_configuration, reward_set=1):
        super(PointGiveawayProgram, self).__init__(merchant_acct, program_configuration, reward_set=reward_set)
