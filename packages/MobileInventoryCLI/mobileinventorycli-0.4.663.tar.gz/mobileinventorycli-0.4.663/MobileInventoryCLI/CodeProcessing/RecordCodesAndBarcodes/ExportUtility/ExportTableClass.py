...
class ExportTable:
	...
	def exportTaggedEntry(self):
		...
		with Session(ENGINE) as session:
			...
			shards=[]
			which=Prompt.__init2__(None,func=FormBuilderMkText,ptext=f"{tagText}\nExport Selected Tags from Entry Table | which index(es): ",helpText=tagText,data="list")
			if which in [None,]:
	...
	def exportSelected(self):
		...
		with Session(ENGINE) as session:
			...
			for num,i in enumerate(tables):
				...
				msg=f"{self.colorized(num,ct)} - {Fore.orange_red_1}{i}{Style.reset}"
				msgF.append(msg)
			which=Prompt.__init2__(None,func=FormBuilderMkText,ptext=f"{'\n'.join(msgF)}\nExport Selected Tables | which index(es): ",helpText='\n'.join(msgF),data="list")
			if which in [None,]:
				print("User Cancelled!")
				return
			try:
