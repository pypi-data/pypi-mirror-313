...
def test_net_import_with_differentiated_prices(parameter_net_import_single_horizon):
    """
    ...
    parameters.net_import_max_yearly_energy_import.loc[{"resource": "electricity"}] = 1000
    parameters.net_import_max_yearly_energy_export.loc[{"resource": "electricity"}] = 1000

    # Max wind capacity of 50 MW
    parameters["conversion_power_capacity_max"] = xr.DataArray(
        [50], dims="conversion_tech", coords={"conversion_tech": ["wind_onshore"]}
    )
    ...
    parameters.operation_year_duration[:] = parameters.time_step_duration.sum().values
    model = build_model(parameters)
    model.solve(solver_name="highs")
    solution = model.solution.dropna(dim="year_dec", how="all").squeeze()

    assert_equal(solution.operation_net_import_import.to_numpy(), np.array([10, 0, 0]))
    assert_equal(solution.operation_net_import_export.to_numpy(), np.array([0, 0, 10]))
    assert_equal(solution.planning_conversion_power_capacity.to_numpy(), np.array([50]))
    ...
    assert np.isclose(model.objective.value, total_costs), "Objective value mismatch"
