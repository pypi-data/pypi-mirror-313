...
def add_combined(
    ...
    """
    Add combined-related components to the Linopy model, including variables, costs, and constraints.
    ...
    Args:
        ...
        annualised_totex_def (Linopy.Expression):
            Expression representing the annualised total expenditure (totex), which will be updated with combined-specific costs.
        time_step (float):
        ...
        operation_adequacy_constraint (Linopy.Expression):
            Expression representing operational adequacy, which ensures supply meets demand. This will be updated with combined-related contributions.

    Returns:
        Linopy.Model:
            The model updated with combined-related variables, costs, and constraints.
    ...
    **Variables**
    ...
    - *Operation*
        - ``operation_combined_power_capacity``
            Represents the operational power capacity for each combined technology by area and operational year.
        - ``operation_combined_power``
            Represents the operational power output for each combined technology by area, mode, hour, and operational year.

        - *Intermediate variables*
            - ``operation_combined_net_generation``
                Represents the net generation from combined technologies, calculated for each area, technology, hour, resource, and operational year.

    - *Planning*
        - ``planning_combined_power_capacity``
            Represents the planned power capacity for each combined technology by area, decision year, and investment year.

    - *Costs (intermediate variables)*:
        - ``operation_combined_costs``
            Represents the operational costs associated with combined technologies for each area, technology, and operational year.
        - ``planning_combined_costs``
            Represents the planning costs associated with combined technologies for each area, technology, and operational year.
    ...
    **Constraints**
    ...
    - *Operation*
        ...
        - *Intermediate variables definition*
            - ``operation_combined_net_generation_def``
                Relates net generation to power output and conversion factor, ensuring consistent accounting.

    - *Planning*
        - ``planning_combined_power_capacity_min_constraint``
            Sets a lower limit on planned power capacity investments for combined technologies.
        - ``planning_combined_power_capacity_max_constraint``
            Sets an upper limit on planned power capacity investments for combined technologies.
    ...
    These additions ensure that the combined technologies operate within feasible and efficient limits, respecting capacity constraints, ramping capabilities, and power output limits. The model is thereby enhanced to accurately simulate combined energy behavior and costs.
    """
    m = model
    p = model_parameters
    ...
    m.add_constraints(
        -operation_combined_net_generation
        + p.time_step_duration * (p.combined_factor * operation_combined_power).sum(["mode"])
        == 0,
    ...
    m.add_constraints(
        ...
        + operation_year_normalization
        * (
            p.combined_variable_cost * (p.time_step_duration * operation_combined_power).sum("hour")
        ).sum("mode")
