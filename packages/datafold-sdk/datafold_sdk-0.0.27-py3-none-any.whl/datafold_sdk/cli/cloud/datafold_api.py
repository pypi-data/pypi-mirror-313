...
from datafold_sdk.cli.errors import DataDiffCloudDiffFailed, DataDiffCloudDiffTimedOut, DataDiffDatasourceIdNotFoundError
...
class TCloudApiDataSourceSchema(pydantic.BaseModel):
    ...
    @classmethod
    def from_orm(cls, obj: Any) -> Type[T]:
...
class TCloudApiDataDiffSummaryResult(pydantic.BaseModel):
    status: str
    pks: TSummaryResultPrimaryKeyStats
    values: TSummaryResultValueStats
    schema_: TSummaryResultSchemaStats
    deps: TSummaryResultDependencyDetails
    ...
    def from_orm(cls, obj: Any) -> Type[T]:
        ...
        deps = TSummaryResultDependencyDetails.construct(deps=obj["dependencies"]) if "dependencies" in obj else None
...
class DatafoldAPI():
    ...
    host: Optional[str] = "https://app.datafold.com"
    timeout: int = 30

    def __init__(self, api_key: str, host: str, headers: Dict[str, str], timeout: int):
    ...
    def get_org_meta(self) -> TCloudApiOrgMeta:
        response = self.make_get_request("api/v1/organization/meta")
        response_json = response.json()
