...
class TransportService:
    ...
    def __init__(self, transport: ITransport):
        ...
        self._legacy_mode = True  # Default to legacy mode for compatibility
    ...
    async def register_agent(self, agent_id: str) -> None:
        ...
        logger.info(f"Registering agent {agent_id} with transport service")
        async with self._state_lock:
            if self._state != ServiceState.RUNNING:
                logger.error(f"Cannot register agent {agent_id}: Service not running")
                raise RuntimeError("Transport service not running")
            self._legacy_mode = False  # Switch to new mode when first agent registers
            self._active_agents.add(agent_id)
            logger.info(f"Agent {agent_id} registered successfully")

    async def deregister_agent(self, agent_id: str) -> None:
        """Deregister an agent from the transport service."""
        logger.info(f"Deregistering agent {agent_id} from transport service")
        async with self._state_lock:
            self._active_agents.discard(agent_id)
            # Clean up agent's subscriptions
            if agent_id in self._subscriptions:
                channels = list(self._subscriptions[agent_id])
                logger.debug(
                    f"Cleaning up {len(channels)} subscriptions for agent {agent_id}"
                )
                for channel in channels:
                    await self.unsubscribe(agent_id, channel)
    ...
    async def stop(self) -> None:
        ...
        # Only wait for agents in new mode
        if not self._legacy_mode and self._active_agents:
            logger.info(f"Waiting for {len(self._active_agents)} agents to deregister")
            try:
                await asyncio.wait_for(self._shutdown_complete.wait(), timeout=10.0)
                logger.debug("All agents successfully deregistered")
            except asyncio.TimeoutError:
                logger.warning("Timeout waiting for agents to deregister")
                # Force cleanup of remaining agents
                remaining = list(self._active_agents)
