...
class RedisTransport(ITransport):
    ...
    async def subscribe(self, channel: str) -> AsyncGenerator[Message, None]:
        ...
        async with self._get_subscription(channel) as state:
            if state.message_queue is None:
                raise RuntimeError(f"Message queue not initialized for channel {channel}")
            ...
            while not self._shutdown_event.is_set():
                try:
                    ...
                    if message is None:  # End of stream
                        break
                    # Add validation that message.target_id matches channel
                    if message.target_id != channel:
                        logger.warning(f"Received message with mismatched target_id: {message.target_id} on channel {channel}")
                        continue
                    yield message
                except asyncio.CancelledError:
                    break
