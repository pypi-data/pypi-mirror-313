    ...
    async def _handle_messages(self) -> None:
        ...
        try:
            ...
            async with self._transport.message_stream(
                subscriber_id=self._sender_id,  # "core"
                channel=self._sender_id,        # "core"
            ) as stream:
                async for message in stream:
                    ...
                    try:
                        # Verify message is intended for core
                        if message.target_id != self._sender_id:
                            logger.warning(f"Received message with incorrect target: {message.target_id}")
                            continue
