...
class ControllerAgent:
    ...
    async def lintCheck(self, working_files, original_prompt_language, file_attachments, focused_files, snow_mode, auto_mode):
        ...
        linter = Linter(root=self.repo.get_repo_path())

        while retries < max_retries:
            try:
                # Run linter on all working files
                lint_results = linter.lint_files(list(working_files))
                ...
                if not files_with_errors:
                    logger.info(" #### `LintChecker` found no errors - code quality check passed")
                ...
                logger.info(" #### `ErrorHandler` has detected linting issues and will commence work on the fix")
                ...
                # Update files to fix
                fixing_related_files.update(working_files)
                fixing_related_files.update(totalfile)
                fixing_related_files.update(context.get('working_files', []))
                ...
                commits = await self.get_pro_fixing_requests(
                    final_request, all_context, list(final_working_files), context_files, "top expert bug fixers", [], "", file_attachments, []
                )
                ...
                logger.error(f"  `ErrorHandler` encountered an error during the linting process: {str(e)}")
                retries += 1
                continue

        logger.info(" #### `LintChecker` reports that fixes failed after maximum retries")
        return list(totalfile)
