        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "    - Frontend:\n"
                    "        - STRICT RULE: Never group HTML/CSS/JS files together in the same group, even if they are related (e.g., main.html and main.css must be in separate groups).\n"
                    "        - STRICT RULE: Never group React components with their styles in the same group (e.g., Component.jsx and Component.css must be in separate groups).\n"
                    "        - STRICT RULE: Never group Vue components with their styles in the same group (e.g., Component.vue and its styles must be in separate groups).\n"
                    "        - STRICT RULE: Never group Angular components with their styles in the same group (e.g., component.ts and component.scss must be in separate groups).\n"
                    "        - STRICT RULE: For desktop apps, never group UI definitions with their style sheets in the same group (e.g., XAML/QML files and their style files).\n"
                    "        - HTML structure files MUST be in groups before any CSS files.\n"
                    "        - One main HTML file (either index, home, or main) MUST be in a group before other HTML files.\n"
                    "        - One main CSS file (either global, home, or main) MUST be in a separate group before other CSS files.\n"
                    "        - Other CSS files MUST be in separate groups AFTER their corresponding HTML files.\n"
                    "        - JavaScript files MUST be in separate groups from both HTML and CSS files.\n"
                    "        - UI component definitions MUST be in separate groups from their style implementations.\n"
                    "        - State management logic MUST be in separate groups from UI components.\n"
                    "        - Client-side routing configuration MUST be in its own group.\n"
                    "        - Utility functions and helpers MUST be in early separate groups.\n"
                    "    - Backend:\n"
