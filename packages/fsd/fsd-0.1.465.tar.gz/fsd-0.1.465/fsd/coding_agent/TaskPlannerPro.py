        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "19. STRICT RULE: Never group files of different presentation layers in the same group. This applies to:\n"
                    "    - HTML and CSS files must ALWAYS be in separate groups, even if they are related (e.g., main.html and main.css)\n"
                    "    - React components (.jsx/.tsx) and their style files (.css/.scss) must be in separate groups\n"
                    "    - Vue components (.vue) and their style files must be in separate groups\n"
                    "    - Angular components (.ts) and their style files (.css/.scss) must be in separate groups\n"
                    "    - Desktop app UI files (.xaml/.ui) and their style files must be in separate groups\n"
                    "    - Mobile app layout files and their style files must be in separate groups\n"
                    "    This rule is absolute and takes precedence over other grouping optimizations.\n"
