...
def test_export__ultralytics(tmp_path: Path) -> None:
    ...
    out = tmp_path / "out.pt"
    export.export(out=out, checkpoint=ckpt_path, part="model", format="ultralytics")
    loaded_model = YOLO(out)
    assert isinstance(loaded_model, YOLO)


@pytest.mark.skipif(YOLO is None, reason="ultralytics is not installed")
def test_export__ultralytics__invalid_model(tmp_path: Path) -> None:
    ckpt_path, ckpt = _get_checkpoint(tmp_path=tmp_path)
    with pytest.raises(ValueError, match="Invalid model format: 'ultralytics'"):
        export.export(
            out=tmp_path / "out.pt",
            checkpoint=ckpt_path,
            part="model",
...
def test_export__invalid_format() -> None:
    ...
    with pytest.raises(
        ...
        match=re.escape(
            ...
            "['torch_model', 'torch_state_dict', 'ultralytics']"
        ),
    ):
        export.export(
            out="out.pt",
