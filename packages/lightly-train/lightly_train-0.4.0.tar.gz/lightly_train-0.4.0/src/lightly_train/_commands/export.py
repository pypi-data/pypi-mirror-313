...
def export(
    ...
    Args:
        ...
        format:
            Format to save the model in. Valid options are ['torch_model',
            'torch_state_dict', 'ultralytics']. 'torch_state_dict' is the recommended
            option and ensures compatibility with different LightlyTrain versions.
            It exports the model's state dict which can be loaded with
            ...
            'ultralytics' exports the model in the Ultralytics format and should be used
            for fine-tuning Ultralytics models. This format is only compatible with
            Ultralytics models.
        overwrite:
...
class ModelFormat(Enum):
    ...
    ULTRALYTICS = ULTRALYTICS_PACKAGE.name
...
def _export_model(model: Module, format: ModelFormat, out: Path) -> None:
    ...
    elif format == ModelFormat.ULTRALYTICS:
        package = package_helpers.get_package(package_name=format.value)
        if package.is_supported_model(model=model):
            package.export_model(model=model, out=out)
        else:
            raise ValueError(
                f"Invalid model format: '{format.value}' is not supported for model "
                f"with type '{type(model)}'"
            )
    else:
