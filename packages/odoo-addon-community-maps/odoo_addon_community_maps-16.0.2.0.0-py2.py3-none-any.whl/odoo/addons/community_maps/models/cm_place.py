...
class CmPlace(models.Model):
    ...
    def crowdfunding_notifications(self, request, submission_id=False):
        if request.notify_admins:
            template = request.template_admins
            for admin in request.map_id.admins_to_notify:
                template.with_context({'email_to': admin.email,'lang':admin.lang}).send_mail(res_id=self.id)
        for submission in self.form_submission_ids:
            ...
            if submission.id != submission_id or not submission_id:
                ...
                if request.notify_target_submissions:
                    ...
                    if filtered_metadata_ids:
                        ...
                        template.send_mail(res_id=submission.id)
