...
def test_nested_parameters(temp_dir):
    ...
    tracker = ExperimentTracker("nested_test")
...
def test_complex_metrics(temp_dir):
    ...
    config = Config.get_instance()
    config.set_data_dir(temp_dir)
    tracker = ExperimentTracker("metrics_test")
...
def test_artifacts_with_complex_data(temp_dir):
    ...
    config = Config.get_instance()
    config.set_data_dir(temp_dir)
    tracker = ExperimentTracker("artifacts_test")
    ...
    api = ExperimentAPI()
    loaded_df = api.get_artifact(
        experiment_name=tracker.experiment_name,
        run_id=tracker.run_id,
        artifact_path="complex_data.csv",
        artifact_type="data",
        load_content=True
    )
...
def test_experiment_query(temp_dir):
    ...
    for i in range(3):
        for j in range(2):
            config = Config.get_instance()
            config.set_data_dir(temp_dir)
            tracker = ExperimentTracker(f"exp_{i}")
