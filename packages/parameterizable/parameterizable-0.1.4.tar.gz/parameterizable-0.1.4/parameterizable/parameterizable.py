    ...
    def __get_metaparams__(self) -> dict[str, Any]:
        ...
        for key, value in metaparams.items():
            if is_parameterizable(value):
                metaparams[key] = value.__get_metaparams__()
...
def get_object_from_metaparams(metaparams: dict[str, Any]) -> Any:
    ...
    assert {CLASSNAME_METAPARAM_KEY,BUILTIN_TYPE_KEY} & set(metaparams)

    if BUILTIN_TYPE_KEY in metaparams:
        assert len(metaparams) == 1
        type_name = metaparams[BUILTIN_TYPE_KEY]
    ...
    for key, value in metaparams.items():
        if key == CLASSNAME_METAPARAM_KEY:
            continue
        elif (isinstance(value, dict) and
              set(value) & {CLASSNAME_METAPARAM_KEY, BUILTIN_TYPE_KEY}):
