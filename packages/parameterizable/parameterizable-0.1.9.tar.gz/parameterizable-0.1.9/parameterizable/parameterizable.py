...
and for converting the parameters to and from an exportable dictionary
(a dictionary that only contains basic types and exportable sub-dictionaries).
"""
...
class ParameterizableClass:
    ...
    and exportable sub-dictionaries).
    ...
    def __get_exportable_params__(self) -> dict[str, Any]:
        """ Get the parameters of an object as an exportable dictionary.
        ...
        They are returned as an 'exportable' dictionary: a dictionary that only
        contains basic types and exportable sub-dictionaries.
        """
        exportable_params = self.get_params()
        exportable_params[CLASSNAME_PARAM_KEY] = self.__class__.__name__
        for key, value in exportable_params.items():
            if is_parameterizable(type(value)):
                exportable_params[key] = value.__get_exportable_params__()
            elif isinstance(value, type) and value in _supported_builtin_types:
                exportable_params[key] = {
    ...
    @classmethod
    def __get_default_exportable_params__(cls) -> dict[str, Any]:
        """ Get default parameters of a class as an  exportable dictionary.
        ...
        contains basic types and exportable sub-dictionaries.
        """
        params = cls().__get_exportable_params__()
        return params
...
def get_object_from_exportable_params(exportable_params: dict[str, Any]) -> Any:
    """ Create an object from a dictionary of parameters.

    This function creates an object from a dictionary of exportable parameters.
    The dictionary should have been created by the .__get_exportable_params__()
    method of a ParameterizableClass object.
    """
    assert isinstance(exportable_params, dict)
    assert {CLASSNAME_PARAM_KEY, BUILTIN_TYPE_KEY} & set(exportable_params)

    if BUILTIN_TYPE_KEY in exportable_params:
        assert len(exportable_params) == 1
        type_name = exportable_params[BUILTIN_TYPE_KEY]
        return _supported_builtin_type_names[type_name]

    params = dict()
    class_name = exportable_params[CLASSNAME_PARAM_KEY]
    ...
    for key, value in exportable_params.items():
        if key == CLASSNAME_PARAM_KEY:
            ...
            params[key] = get_object_from_exportable_params(value)
        else:
            params[key] = value
...
def is_parameterizable(cls: Any) -> bool:
    ...
    if not isinstance(cls, type):
        return False
    if not hasattr(cls, "__get_exportable_params__"):
        return False
    if not callable(cls.__get_exportable_params__):
        return False
    if not hasattr(cls, "get_params"):
        return False
    if not callable(cls.get_params):
        return False
    if not hasattr(cls, "__get_default_exportable_params__"):
        return False
    if not callable(cls.__get_default_exportable_params__):
        return False
...
def register_parameterizable_class(obj: Any):
    ...
    used with the get_object_from_exportable_params()
    """
