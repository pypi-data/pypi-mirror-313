...
class AlipayAuthCreateSessionRequest(AlipayRequest):
    ...
    def __to_ams_dict(self):
        params = dict()
        if hasattr(self, "__product_code") and self.product_code:
            params['productCodeType'] = self.product_code

        if hasattr(self, "__agreement_info") and self.agreement_info:
            params['agreementInfo'] = self.agreement_info.to_ams_dict()

        if hasattr(self, "__scopes") and self.scopes:
            scopes_list = []
            for scope in self.scopes:
                scopes_list.append(scope.value)
            params['scopes'] = scopes_list

        if hasattr(self, "__payment_method") and self.payment_method:
            params['paymentMethod'] = self.payment_method.to_ams_dict()

        if hasattr(self, "__payment_redirect_url") and self.payment_redirect_url:
