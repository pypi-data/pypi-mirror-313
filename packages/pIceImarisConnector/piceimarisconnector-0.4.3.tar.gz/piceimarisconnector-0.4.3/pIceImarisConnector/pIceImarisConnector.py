...
class pIceImarisConnector(object):
    ...
    **REMARK**
    ...
    def createAndSetSpots(
        ...
        # Is each radius a scalar or a triplet?
        if len(radii[0]) == 1:
            is_radius_scalar = True
        elif len(radii[0]) == 3:
            is_radius_scalar = False
        else:
            raise ValueError("radii must either be scalar or a [x, y, z] triplet.")
        ...
        if is_radius_scalar:
            ...
            newSpots.Set(coords, timeIndices, np.ones(len(radii), dtype=np.float32).tolist())
    ...
    def _findNewestVersionDir(self, directory):
        ...
        for d in subDirs:
            ...
            match = re.search(r"(?P<major>\d+)\.(?P<minor>\d+)(\.)?(?P<patch>\d+)", d)
            if not match:
                continue
