...
from reflex_cli.v2.utils.dependency import extract_domain


def login(
    loglevel: Annotated[
        constants.LogLevel,
        typer.Option(constants.LogLevel.INFO, help="The log level to use."),
...
def deploy(
    ...
    **kwargs,
    ...
    if not project:
        console.error(
            "No project selected. Please select a project using `reflex apps project-select` or "
            "specify a project id using `reflex deployv2 --project <project_id>`."
        )
        raise typer.Exit(1)
