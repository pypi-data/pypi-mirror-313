...
from reflex_cli.v2.utils.dependency import is_valid_url
...
def authenticated_token() -> tuple[str, dict[str, Any]]:
    ...
    validated_info = {}
    access_token = get_existing_access_token()
    if access_token and not (validated_info := validate_token_with_retries(access_token)):
        access_token = ""
...
def select_project(project: str, token: str | None = None) -> str:
    ...
    try:
        ...
        return f"failed to fetch token from {HOSTING_CONFIG_FILE} due to: {ex}"
...
def authenticate_on_browser() -> tuple[str, dict[str, Any]]:
    ...
    if not is_valid_url(HOSTING_UI):
        console.error(f"Invalid hosting URL: {HOSTING_UI}. Ensure the URL is in the correct format and includes a valid scheme")
        raise typer.Exit(1)
