...
from pawnlib.utils.http import IconRpcHelper, NetworkInfo, AsyncIconRpcHelper, CallHttp, append_http
...
async def check_network_api_availability(network_api):
    ...
    """
    # Extract host and port from the network_api URL
    ...
    # Optionally, perform additional checks like making an HTTP request to verify the API is responsive
            ...
            for port, result in zip(open_ports, results):
                ...
                try:
                    pawn.console.log(f'{status_color}Port {port}: Status={status:<3}, Height={height:,}, nid={nid}, '
                                     ...
                                     f'TX Cnt={recent_tx_count},{state}')
                except Exception as e:
...
def main():
    ...
    if args.command == "info":
        ...
        asyncio.run(find_and_check_stat(2))
