...
class Femtodriver:
    def __init__(self, model_source_dir: str | None = MODEL_SOURCE_DIR):
    ...
    def compile_metadata(
        ...
        @param: force_femtocrux_compile: whether to use femtocrux (only useful internally)
        ...
        if self.model_dir is not None:
            ...
            elif DEV_MODE and not force_femtocrux_compile:
    ...
    def generate_inputs(
        ...
        n_inputs: int = 2,
        input_file_sample_indices: list | None = None,
    ) -> np.ndarray:
        """
        Read inputs from a file or generate some fake inputs for testing the model in simulation.

        If no input_file is specified, you can specify n_inputs to generate fake inputs for
        ...
        @param: n_inputs: allows you to specify length of fake input if not using a wav file input
        @param: input_file_sample_indices: the slice of the input to simulate. Useful if the input is long to make
                simulation faster.

        @returns np.ndarray of shape (1, frames, samples/features) where 1 is the batch dim for inference
        """

        N = n_inputs

        if input_file is None:
            inputs = spu_runner.make_fake_inputs(N, style="random")
        else:
            if input_file.endswith(".wav"):
                input_vals = self._create_shaped_input_from_wav(input_file, spu_runner)
            elif input_file.endswith(".npy"):
                input_vals = np.load(input_file)
            else:
                raise RuntimeError(
            ...
            if input_file_sample_indices is not None:
                lo, hi = input_file_sample_indices[0], input_file_sample_indices[1]
                for k, v in inputs.items():
    ...
    def execute_runner(
        ...
        function generate_inputs() to turn wav files into the correct shape ndarray.

        @param: spu_runner: The spu_runner to execute
        @param: inputs: An ndarray that matches the shape required by the model
        @param: input_period: the input period processing time for a frame in seconds
        @param: force_femtocrux_sim: whether to use femtocrux to run the sim (only useful internally)

        @returns: returns a dictionary which contains the internals activations and outputs of a runner.
        """
        match femto_runner:
            case SPURunner():
                runner_name = "hardware"
            case FQIRArithRunner():
                runner_name = "fqir"
            case FMIRRunner():
                runner_name = "fmir"
            case SimRunner():
            ...
            case FXRunner():
                runner_name = "fasmir"
            case _:
                logger.warning("unable to detect runner for simulation")
                raise (Exception("Passed unknown femtorunner type"))

        femto_runner.reset()
        output_vals, internal_vals, output_valid_mask = femto_runner.run(inputs)
    ...
    def _prepare_comparisons(
        ...
        for comp in comparisons:
            if comp == "hw":
                compare_runners.append(spu_runner)
                compare_names.append("hardware")
            elif comp == "fasmir":
                if DEV_MODE and not force_femtocrux_sim:
                    # FB runner
                ...
                if force_femtocrux_compile and force_femtocrux_sim:
        ...
        return compare_runners, compare_names
    ...
    def _run_comparisons(
        ...
        @oaram force_femtocrux_sim: whether to force the simulator to run through femtocrux (only useful internally)
        @param force_femtocrux_compile: whether to force the comilation to happen through femtocrux (only useful internally)
        ...
        compare_runners, compare_names = self._prepare_comparisons(
            ...
            sim_fmir=sim_fmir,
            force_femtocrux_sim=force_femtocrux_sim,
            force_femtocrux_compile=force_femtocrux_compile,
    ...
    def compile(
        ...
        @returns: The meta data directory on disk
        """
        ...
        self.spu_runner = self.create_SPURunner(
            self.compiled_data,
            self.meta_dir,
            noencrypt=noencrypt,
            hardware=hardware,
    ...
    def simulate(
        ...
        @returns: A string representation of the yaml sim metrics
        """
        if self.meta_dir:
        ...
        inputs = self.generate_inputs(
            ...
            n_inputs=n_inputs,
            input_file_sample_indices=input_file_sample_indices,
        )

        if DEV_MODE and not force_femtocrux_sim:
            # FB runner
            fasmir_runner = SimRunner(
                self.fasmir,  # model might be fqir, need to compile for SimRunner
                input_padding=self.spu_runner.io.input_padding,
                output_padding=self.spu_runner.io.output_padding,
            )
        ...
        return self._get_runner_metrics(fasmir_runner, input_period=input_period)
    ...
    def compare(
        ...
        @returns: True if the comparison matched, False otherwise
        """
        if self.meta_dir:
            logger.info("running Simulation")
        else:
    ...
    def run(
        ...
        n_inputs: int = 2,
        input_file: str = None,
        input_file_sample_indices: str | None = None,
        force_femtocrux_compile: bool = False,
        force_femtocrux_sim: bool = False,
        hardware: str = "fakezynq",
        runners: str = "",
        ...
        if len(self.comparisons) == 1 and "fasmir" in self.comparisons:
            if input_file_sample_indices is not None:
                input_file_sample_indices = input_file_sample_indices.split(",")
            ...
            self.write_metrics_to_disk(sim_result, output_dir=output_dir)
...
def main(argv, model_source_dir=MODEL_SOURCE_DIR):
    with Femtodriver(model_source_dir=model_source_dir) as fd:
        result = fd.main(argv)
