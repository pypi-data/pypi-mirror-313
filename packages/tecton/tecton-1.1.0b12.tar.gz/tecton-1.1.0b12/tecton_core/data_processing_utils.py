...
def split_spine(
    ...
    join_keys: List[str],
    # If num_partitions is None, it will use the value from the tecton config.
...
def pyarrow_split_spine(
    ...
    if strategy == "even":
        return _even_split_pyarrow(spine, join_keys, split_count)
    else:
        error = f"Unknown spine split strategy: {strategy}."
        raise ValueError(error)
