...
class PandasRealtimeFeaturePipeline(RealtimeFeaturePipeline):
    def __init__(
        ...
        # In python, the context is built for every single row, but we don't want to spam the secret server. We only resolve secrets once and rest of rows reuse those secrets.
    ...
    def _context_node_to_value(self, pipeline_node: PipelineNode) -> Optional[RealtimeContext]:
        ...
        resolved_secrets = {}
        if self._secret_references:
            if not self._secret_resolver:
                msg = "Can not find secret resolver."
                raise errors.TectonInternalError(msg)
            resolved_secrets = self._secret_resolver.resolve_map(self._secret_references)
        ...
        if self.is_python_mode or self.is_no_transformation_mode:
            ...
            return RealtimeContext(
                row_level_data=context_df, _mode=self.transformation_mode, secrets=self._resolved_secrets
            )
