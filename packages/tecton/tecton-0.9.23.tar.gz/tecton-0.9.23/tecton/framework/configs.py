...
class Aggregation(StrictModel):
    ...
    def _to_proto(
        ...
        proto = feature_view_pb2.FeatureAggregation(
            name=self.name
            if self.name
            else self._default_name(aggregation_interval, is_continuous, compaction_enabled),
            column=self.column,
        ...
        elif isinstance(self.time_window, TimeWindowSeries):
            proto.time_window_series.CopyFrom(self.time_window._to_proto())
        else:
    ...
    def _default_name(
        ...
        window_spec = self.time_window._to_spec()
        column_name = f"{self.column}_{self.function.resolved_name}_{window_spec.window_duration_string()}"
        ...
        if isinstance(self.time_window, TimeWindowSeries):
