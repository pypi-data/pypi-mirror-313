...
def deploy(ifname, spdk_cpu_mask):
    ...
    hyperthreading_enabled = utils.is_hyperthreading_enabled_via_siblings()
    ...
    if spdk_cores[-1] >= 64:
        ...
        return False
    core_distribution = utils.load_core_distribution_from_file(constants.JSON_CORES_FILE, len(spdk_cores))
    ...
    siblings = utils.parse_thread_siblings()
    isolated_full = set()
    ...
    if isolated_cores:
        ...
        utils.run_tuned()
    utils.store_cores_distribution(app_thread_core, jm_cpu_core, poller_cpu_cores,
