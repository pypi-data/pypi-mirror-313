...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.placeholder_path = PACKAGE_ROOT / "resources/no_video.jpg"
    ...
    def initialize_image(self):
        ...
        if os.path.exists(self.placeholder_path):
            self.placeholder_image = cv2.imread(str(self.placeholder_path))  # Ensure path is a string
            if self.placeholder_image is not None:
                self.placeholder_image = cv2.cvtColor(self.placeholder_image, cv2.COLOR_BGR2RGB)  # Correct BGR to RGB
            else:
                print(f"Error: Unable to load placeholder image {self.placeholder_path}")
                return
        else:
            print(f"Warning: Placeholder image not found at {self.placeholder_path}")
            return

        # Assign the placeholder image to all camera widgets
        if self.placeholder_image is not None:
            for widget in self.camera_widgets:
                # Convert placeholder_image to QImage and assign it
                qimage = QImage(
                ...
                # Define set_image and paintEvent for the widget
                ...
                # Initialize the widget with the placeholder image
                widget.image = qimage
