...
class ModelInput(BaseModel):
    ...
    class Tool(BaseModel):
        ...
        def json_schema(self) -> Dict[str, Any]:
            ...
            # We could either be using pydantic v1 or v2, so try to remove both
            parameters_without_definitions.pop("definitions", None)  # Pydantic v1
            parameters_without_definitions.pop("$defs", None)  # Pydantic v2
        ...
        @property
        def v1_definitions(self) -> Optional[Dict[str, Any]]:
            return self.function.parameters.get("definitions")

        @property
        def v2_definitions(self) -> Optional[Dict[str, Any]]:
            return self.function.parameters.get("$defs")
    ...
    def output_json_schema(self) -> Optional[Dict[str, Any]]:
        ...
        if tools is not None:
            ...
            v1_definitions_set = False
            v2_definitions_set = False
            for tool in tools:
                definitions = {}
                if tool.v1_definitions is not None:
                    if v2_definitions_set:
                        raise ValueError(
                            "Both pydantic v1 and v2 definitions found; please check schema."
                        )
                    v1_definitions_set = True
                    definitions = tool.v1_definitions
                if tool.v2_definitions is not None:
                    if v1_definitions_set:
                        raise ValueError(
                            "Both pydantic v1 and v2 definitions found; please check schema."
                        )
                    v2_definitions_set = True
                    definitions = tool.v2_definitions
            ...
            if v1_definitions_set:
                schema["definitions"] = combined_definitions
            elif v2_definitions_set:
                schema["$defs"] = combined_definitions
