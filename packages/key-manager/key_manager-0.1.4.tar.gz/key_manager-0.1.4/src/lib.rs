...
impl KeyManager {
    ...
    fn configure_jwt_validation(
        ...
        // if !validation.get_validate_signature() {
...
impl KeyManager {
    ...
    fn verify_token_by_kid(
        ...
        if validation.kid.is_none() {
            return Err(PyErr::new::<InvalidOperation, _>("Token kid must be specified"));
        }
        let header = Self::extract_header(token)?;
        let kid_binding = header.kid.clone();
        let kid = validation.kid.as_ref().unwrap();
        if let Some(kid) = kid_binding {
            if kid != kid {
                return Err(PyErr::new::<InvalidTokenError, _>("Token kid does not match the provided kid"));
            }
        }
        ...
        let jwt_validation = Self::configure_jwt_validation(validation)?;
        let decoding_key = self.key_store.get_decoded_public_key(Some(kid))?;
