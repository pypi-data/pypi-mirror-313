...
def benchmark_key_manager_key_story():
    try:
        ...
        # Token generation
        start_gen = time.time()
        token = key_manager.generate_token_by_kid(kid=kid, claims=claims)
        end_gen = time.time()

        # Token validation
        validations = TokenValidation()
        start_val = time.time()
        decoded = key_manager.verify_token_by_kid(token=token, kid=kid, validation=validations)
        end_val = time.time()
...
def benchmark_key_manager():
    try:
        ...
        # Token generation
        start_gen = time.time()
        token = key_manager.generate_token(private_key=PRIVATE_KEY, claims=claims)
        end_gen = time.time()
