# Python Client For Nutanix Files APIs

The Python client for Nutanix Files APIs is designed for Python client application developers offering them simple and flexible access to APIs that manage virtual file servers, create and configure shares for client access, protect them using DR and sync policies, provision storage space and administer security controls.
## Features
...
## Configuration
The python client for Nutanix Files APIs can be configured with the following parameters
...
```python
# Initialize the API
antivirus_servers_api_instance = AntivirusServersApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Get antivirus server by external identifier
try:
    api_response = antivirus_servers_api_instance.get_antivirus_server_by_id(extId)
...
try:
    ...
    api_response = antivirus_servers_api_instance.update_antivirus_server_by_id(body, extId, if_match=etag_value) # Use the extracted etag value
except ApiException as e:
```
...
```python
# Initialize the API
migrations_api_instance = MigrationsApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Fetches a list of migration jobs.
try:
    api_response = migrations_api_instance.list_migration_jobs(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=files&version=v4.0&language=python). This documentation is auto-generated, and the location may change.
