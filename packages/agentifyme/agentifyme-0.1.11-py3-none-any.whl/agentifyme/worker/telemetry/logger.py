...
class LoguruHandler:
    ...
    def _get_attributes(self, record) -> Attributes:
        ...
        if record["exception"] is not None:
            attributes[SpanAttributes.EXCEPTION_TYPE] = str(record["exception"].type)
            attributes[SpanAttributes.EXCEPTION_MESSAGE] = str(record["exception"].value)
            attributes[SpanAttributes.EXCEPTION_STACKTRACE] = str(record["exception"].traceback)
    ...
    def sink(self, record) -> None:
        print("Logging to OTLP", type(record), record)
