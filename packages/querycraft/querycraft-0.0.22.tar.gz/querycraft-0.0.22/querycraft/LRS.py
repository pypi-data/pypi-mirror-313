...
class LRS(object):
    ...
    def sendSBSExecute(self, sqlTXT: str, error=None):
        verb = Verb(id='http://univ-nantes.fr/asql/xapi/verbs/execute',
                    display=LanguageMap({'fr': 'Exécuter'}), )
        object = Activity(id='http://univ-nantes.fr/asql/xapi/activities/execute-sql',
                          definition=ActivityDefinition(
                              name=LanguageMap({'fr': 'Exécution SQL', 'en': 'Execute a SQL query'}),
                              description=LanguageMap(
                                  {'fr': 'Execute une requête SQL', 'en': 'Execute a SQL query'}),
                          ), )
        self.context.extensions = Extensions({'http://univ-nantes.fr/asql/xapi/terms/sql': sqlTXT})
        if error is None:
            resultat = Result(success=True, completion=True)
        else:
            resultat = Result(success=False,
                              completion=False,
                              extensions={'http://univ-nantes.fr/asql/xapi/terms/error': str(error)})
    ...
    def sendSBSpap(self, sqlTXT: str, error=None):
        verb = Verb(id='http://univ-nantes.fr/asql/xapi/verbs/sbsed',
                    display=LanguageMap({'fr': 'Effectuer Pas à pas ', 'en': 'Do step by step'}), )
        object = Activity(id='http://univ-nantes.fr/asql/xapi/activities/sbs-sql',
                          definition=ActivityDefinition(
