...
class Database:
    ...
    @classmethod
    def get(cls, dbcon: str, dbtype: str, debug):
        dbid = dbtype + '—' + str(dbcon)
        if dbid in cls.dblist:
            # print('$$$')
            return cls.dblist[dbid]
        elif dbtype == 'sqlite':
            db = DBSQLite(dbcon, debug)
            cls.dblist[dbid] = db
            return db
        elif dbtype == 'pgsql':
            db = DBPGSQL(dbcon, debug)
            cls.dblist[dbid] = db
            return db
        elif dbtype == 'mysql':
            db = DBMySQL(dbcon, debug)
            cls.dblist[dbid] = db
            return db
        else:
    ...
    def __init__(self, db=None, dbtype=None, debug=False):
        self.debug = debug
        if db is not None:
    ...
    def execute(self, sql):
        try:
            ...
            raise SQLException(f"Erreur Database (execute) :\n -> {sql} \n -> {e}")
...
class DBPGSQL(Database):
    ...
    def __init__(self, db, debug=False):
        self.dbPGID = dict()
        super().__init__(db=db, dbtype='pgsql', debug=debug)
...
class DBSQLite(Database):
    def __init__(self, db, debug):
        super().__init__(db=db, dbtype='sqlite', debug=debug)
        if not existFile(db):
...
class DBMySQL(Database):
    ...
    def __init__(self, db, debug):
        ...
        self.connection = None
        super().__init__(db=db, dbtype='mysql', debug=debug)
