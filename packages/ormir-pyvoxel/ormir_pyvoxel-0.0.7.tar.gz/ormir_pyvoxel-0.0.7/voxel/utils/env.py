...
def output_dir() -> str:
    return os.path.abspath(os.path.join(_FILE_DIRECTORY, "../../.voxel"))
...
def log_file_path() -> str:
    return os.path.join(output_dir(), "voxel.log")
...
def debug(value: bool = None) -> bool:
    ...
    Note:
        ...
        for the default voxel logger. If debug state is turned off, logging
    ...
    def _is_debug():
        return os.environ.get("VOXEL_DEBUG", "") in ["True", "true"]

    def _toggle_debug(_old_value, _new_value):
        # TODO: Toggle voxel logging to debug mode.
        if _old_value == _new_value:
            return

        _vx_logger = logging.getLogger("voxel")
        if _new_value:
            _vx_logger.setLevel(logging.DEBUG)
            for h in _vx_logger.handlers:
                h.setLevel(logging.DEBUG)
        else:
            _vx_logger.setLevel(logging.DEBUG)  # the root logger should always log at DEBUG level
            for h in _vx_logger.handlers:
    ...
    if value is not None:
        ...
        if isinstance(value, bool):
            os.environ["VOXEL_DEBUG"] = str(value)
        elif isinstance(value, str) and value.lower() in ("true", "false", ""):
            os.environ["VOXEL_DEBUG"] = value
        else:
