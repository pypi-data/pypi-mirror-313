...
def create_parallel_AEs(X, enable_summary):
...
def train_model(windows, window_size=40, enable_summary=False, verbose=1, nr_epochs=200, nr_patience=10):
    new_windows = prepare_inputs(windows, nr_ae=2, window_size=window_size)
    pae, encoder = create_parallel_AEs(new_windows, enable_summary)
