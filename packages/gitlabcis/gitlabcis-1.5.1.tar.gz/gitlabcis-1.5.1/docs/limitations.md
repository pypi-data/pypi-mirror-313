...
- The `gitlabcis` tool does not support `groups` currently
- The `gitlabcis` tool will only look at the `project` level, not take into account group-level settings or permissions.
  - i.e. If you selected "Require approval from CODEOWNERS" at a group level and _not_ at the project level, you would `FAIL` the [1.1.7 - code_changes_require_code_owners](https://gitlab.com/gitlab-org/govern/compliance/engineering/cis/gitlabcis/-/blob/main/gitlabcis/recommendations/source_code/code_changes/code_changes_require_code_owners.yml) control.
...
| id     | name | Limitation |
|--------|------|------------|
| 1.1.2  | code_tracing | If there are no merge requests in a project, this will check for issues and if there are neither a `FAIL` will be returned. If merge requests exist, and there are [related_issues](https://docs.gitlab.com/ee/api/merge_requests.html#list-issues-related-to-the-merge-request) then a `PASS` will be returned. If there are no merge requests but the project has issues then a `PASS` will be returned. |
...
| 1.4.3 | least_privilge_app_permissions | If a project has `integrations` then this check will `SKIP` to require manual verification, otherwise if none were found return a `PASS` |
