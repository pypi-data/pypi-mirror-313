...
def code_tracing(glEntity, glObject, **kwargs):
    ...
    try:
        mrs = glEntity.mergerequests.list(get_all=False)

        if len(mrs) == 0:
            # still pass if issues are present:
            ...
            # check for related issues in merge requests:
            for mr in mrs:
                if mr.related_issues:
                    return {True: 'Merge requests can be traced back to '
                            'issues in the project'}

        return {False: 'No Merge requests were found to trace back to '
                'issues'}
