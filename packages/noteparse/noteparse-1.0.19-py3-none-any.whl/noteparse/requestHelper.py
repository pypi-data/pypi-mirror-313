...
def request_get(url,headers=originHeader):
    try:
        ...
        proxies = {
            'http': f'http://{proxyConf['minute_user']}:{proxyConf['minute_pwd']}@{str(proxy_info["ip"])}:{str(proxy_info["port"])}',
            'https': f'http://{proxyConf['minute_user']}:{proxyConf['minute_pwd']}@{str(proxy_info["ip"])}:{str(proxy_info["port"])}'
        }
...
def request_post_urlencode(url,param,headers=originHeader):
    try:
        proxy_info = proxyHelper.get_singleton()
        proxies = {
            'http': f'http://{proxyConf['minute_user']}:{proxyConf['minute_pwd']}@{proxy_info["ip"]}:{str(proxy_info["port"])}',
            'https': f'http://{proxyConf['minute_user']}:{proxyConf['minute_pwd']}@{proxy_info["ip"]}:{str(proxy_info["port"])}'
        }
...
def refreshHttp():
    ...
    proxy_url = f'http://{proxyConf['minute_user']}:{proxyConf['minute_pwd']}@{proxy_info["ip"]}:{str(proxy_info["port"])}'
