...
keySet = {
    ...
    'proxyUnit' : ['采购代理机构：','代理机构：','招标代理机构：','招募代理机构：','招标机构：','招标代理名称：','招标代理机构名称：','联系人（代理机构）：','招标代理：'],
...
def getContentParse(content,noteInfo):
    noteInfo['content'] = content
    try:
        parseContentUrl = 'http://10.220.6.180:18881/content/parse'
        header = {
            'Content-Type': 'application/json'
        }
        param = noteInfo
        response = requests.post(url=parseContentUrl,headers=header,json=param)
