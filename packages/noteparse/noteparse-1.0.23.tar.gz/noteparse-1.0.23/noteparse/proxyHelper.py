...
def get_new_ip(ip_count=1):
    ...
    r = requests.get(proxyConf['minute_url'],headers=headers)
    r.close()
...
def get_new_driver():
    ...
    wireOption = {
        'proxy':{
            'http':f"http://{proxyConf['minute_user']}:{proxyConf['minute_pwd']}@{str(proxy_list[0]['ip'])}:{str(proxy_list[0]['port'])}",
            'https': f"http://{proxyConf['minute_user']}:{proxyConf['minute_pwd']}@{str(proxy_list[0]['ip'])}:{str(proxy_list[0]['port'])}",
            'ssl_verify': False
...
def get_daily_ip():
    ...
    r = requests.get(proxyConf['daily_url'],headers=headers)
    r.close()
    if r.status_code != 200:
        raise Exception('代理接口请求失败')
    if not r.json()['success']:
