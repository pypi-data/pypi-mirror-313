...
class DocumentProcessor:
    ...
    @staticmethod
    def extract_text_from_invoice_image(image: bytes) -> str:
        try:
            logger.info("Processing invoice image with AWS Textract")
            ...
            logger.error(f"Textract processing failed: {str(e)}")
            raise TextExtractionError(f"Failed to extract text from invoice image: {str(e)}")

    @staticmethod
    def format_invoice_text(data: str) -> str:
