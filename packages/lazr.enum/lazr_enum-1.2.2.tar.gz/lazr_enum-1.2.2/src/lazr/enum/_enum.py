...
try:
    ...
    removeAllProxies = lambda obj: obj  # no-op  # noqa: E731
...
class BaseItem:
    ...
    def __eq__(self, other, stacklevel=2):
        if isinstance(other, int):
            warnings.warn(
                'comparison of Item to an int: %r' % self,
            ...
            if (not getattr(self, 'enum', None)
                ...
                warnings.warn(
                    'comparison of Items which are not '
                    'enumerated types',
                    stacklevel=stacklevel
                )
                return False
...
class BaseMetaEnum(type):
    ...
    def __iter__(self):
        ...
        return map(TokenizedItem, self.items)
...
class EnumeratedType(metaclass=MetaEnum):
...
class DBEnumeratedType(metaclass=MetaDBEnum):
