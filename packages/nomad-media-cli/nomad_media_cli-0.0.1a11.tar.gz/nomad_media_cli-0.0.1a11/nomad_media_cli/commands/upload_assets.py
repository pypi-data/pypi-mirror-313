...
@click.command()
@click.option("--file", help="File to upload")
...
@click.pass_context
def upload_assets(ctx, file, id, path, recursive):
    """Upload assets"""
    ...
    try:
        ...
        if not file:
            click.echo({ "error": "Please provide a file to upload." })
            sys.exit(1)
            
        if not id and not path:
            click.echo({ "error": "Please provide a parent id or path." })
            sys.exit(1)
        ...
        if os.path.isdir(file):
            ...
            nomad_sdk.upload_asset(None, None, None, "replace", file, parent_id, None)
