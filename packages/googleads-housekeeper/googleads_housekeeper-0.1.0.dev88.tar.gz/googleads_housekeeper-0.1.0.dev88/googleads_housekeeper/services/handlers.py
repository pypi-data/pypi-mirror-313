...
def update_completed_async_preview_task(
  ...
  with uow:
    ...
    logging.warning("task_and_results before updating the db= %s", json.dumps(
      task_and_results.to_serializable_dict()))
    update_dict = {
      'status': task_and_results.status,
      'results': task_and_results.results,
    }
    logging.warning("update_dict before updating the db = "
                    "%s", json.dumps(update_dict))
