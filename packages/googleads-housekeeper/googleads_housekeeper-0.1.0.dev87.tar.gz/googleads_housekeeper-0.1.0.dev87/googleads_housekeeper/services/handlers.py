...
# pylint: disable=C0330, g-bad-import-order, I'm having two issues with local orm+sqlite implementation.
# Can I please ask you to have a quick look at the code for (`run_preview_placements` in `handlers.py`) flow?
#
# When I "add a new offline preview task" - it opens a new worker thread.
# It adds a new preview task to sqlite db (status RUNNING) and then updates it later with status = DONE.
#
#
#
# As I'm creating a new uow for this thread  (https://source.corp.google.com/h/professional-services/solutions/ads_housekeeper/+/60bc9d3b1f6e13c92a086cb9b00b4433b7359776:googleads_housekeeper/bootstrap.py;l=75)- so I'm not sure why I'm getting this:
#
# `sqlite3.ProgrammingError: SQLite objects created in a thread can only be used in that same thread. The object was created in thread id 139868617311936 and this is thread id 139868627797696.
# `
#
# Not sure why I'm getting this as well - as the flow is the same as it used to be when preview was synchronous:
#
# `sqlalchemy.exc.InvalidRequestError: Object <VideoInfo at 0x7f35df5aee50> cannot be converted to 'persistent' state, as this identity map is no longer valid.  Has the owning Session been closed?`
#
#
# Full stack trace:
# https://paste.googleplex.com/6556353880129536
#
#
#
# TIAg-multiple-import, g-importing-member
