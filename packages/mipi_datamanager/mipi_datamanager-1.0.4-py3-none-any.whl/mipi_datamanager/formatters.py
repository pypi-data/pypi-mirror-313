...
class FormatDict(dict):
    ...
    def format_incoming_df(self, df, override_formatter: dict|Self = None):  # , on=None, side_on=None, use_index=False):
        ...
        if override_formatter:
            _override = _maybe_convert_dict_to_format_dict(override_formatter)
        else:
            _override = {}
...
def _maybe_convert_dict_to_format_dict(dict_)->FormatDict:
    if isinstance(dict_, dict):
        return FormatDict(dict_)
    if isinstance(dict_, FormatDict):
        return FormatDict(dict_)
    else:
        ValueError(f"Expected type dict|FormatDict got {type(dict_)}")
