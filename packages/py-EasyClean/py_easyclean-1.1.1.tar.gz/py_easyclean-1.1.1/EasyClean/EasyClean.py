...
class EasyClean:
    ...
    def __init__(self, input_data, mode='auto', duplicates=False, missing_num=False, missing_categ=False, encode_categ=False, extract_datetime=False, outliers=False, outlier_param=1.5, logfile=True, verbose=False):  
        ...
        logger.info('EasyClean process completed in {} seconds', round(end-start, 6))

        if not verbose:
            print('EasyClean process completed in', round(end-start, 6), 'seconds')
        if logfile:
    ...
    def _initialize_logger(self, verbose, logfile):
        ...
        if logfile == True:    
            logger.add('Easyclean.log', mode='w', format='{time:DD-MM-YYYY HH:mm:ss.SS} - {level} - {message}')
        return
