...
class ResourceProviderErrorType(Enum):
    ...
    MISSING_CONTEXT_ERROR = 3
    SECRETS_DISABLED_ERROR = 4
...
class ResourceProvidersManager:
    ...
    def _get_resource(self, resource_provider_id: str, resource_provider_name) -> Any:
        ...
        if not resource:
            if resource_provider_id in self.resource_error_table:
                ...
                message = f"Unable to find Resource for Resource Provider {resource_provider_name}. Please note that newly updated Feature Services may take upto 120s to reflect changes. Using configuration generated at {self.last_refresh_time}."
    ...
    def _has_secrets_updated(
        ...
        # Secrets are updated if the secrets have been newly added or deleted or if the secrets_last_updated_time is more recent than the stored resource_provider's secret_last_updated_time
        if secrets_last_updated_time is None and stored_secrets_last_updated_time is not None:
            return True
        elif secrets_last_updated_time is not None and stored_secrets_last_updated_time is None:
            return True
        elif secrets_last_updated_time and stored_secrets_last_updated_time:
