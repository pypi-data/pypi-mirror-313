...
def basic_cleanup_dd(filepath: str, meta_selector=None) -> pl.DataFrame:
    """
    df: data+metadata data frame.
    meta_selector: metadata it determines which columns are metadata and which are data.
    """
    df = duckdb.read_parquet(filepath)
    values_df = df.select_dtypes(["float"])
    values = values_df.pl().to_numpy()  # TODO Decide between polars and python dicts
...
filepath = "/datastore/alan/gsk/profiles/CPBFFluor_20230929_090751/H00DJKJPlate1CP/A01_001.parquet"
tmp = duckdb.read_parquet(filepath)
...
select *
FROM parquet_schema('{}')
where NOT (type = 'FLOAT')
""".format(filepath)
).show()


filepath = "https://cellpainting-gallery.s3.amazonaws.com/cpg0016-jump-assembled/source_all/workspace/profiles/jump-profiling-recipe_2024_a917fa7/CRISPR/profiles_wellpos_cc_var_mad_outlier_featselect_sphering_harmony_PCA_corrected/profiles.parquet"
tmp.select_dtypes(["uint16", "varchar"])
