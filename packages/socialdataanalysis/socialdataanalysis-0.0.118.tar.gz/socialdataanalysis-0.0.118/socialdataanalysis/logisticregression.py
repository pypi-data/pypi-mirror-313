...
def analyze_glm_binomial_plots(df, dependent_var, independent_vars):
    ...
    Nota:
    O DataFrame `df` será modificado pela função com novas colunas: 'predicted_prob' e 'logit_p'.

    Parâmetros:
    - df: DataFrame com dados (assegure que não seja uma slice, de preferência crie-o sem slicing)
    ...
    # Atribuições usando .assign() para evitar SettingWithCopyWarning
    df = df.assign(predicted_prob=result.predict(sm.add_constant(df[independent_vars])))
    df = df.assign(logit_p=np.log(df['predicted_prob'] / (1 - df['predicted_prob'])))
