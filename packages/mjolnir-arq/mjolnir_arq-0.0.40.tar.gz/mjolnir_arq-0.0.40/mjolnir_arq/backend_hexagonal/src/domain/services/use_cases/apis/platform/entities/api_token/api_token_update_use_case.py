...
from src.domain.models.entities.api_token.index import ApiToken, ApiTokenUpdate
...
class ApiTokenUpdateUseCase:
    ...
    async def execute(
        ...
        result = await self.api_token_repository.update(config=config, params=params)
        if not result:
