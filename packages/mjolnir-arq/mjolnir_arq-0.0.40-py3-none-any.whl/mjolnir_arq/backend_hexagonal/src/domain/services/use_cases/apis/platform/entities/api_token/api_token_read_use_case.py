...
from src.domain.models.entities.api_token.index import ApiToken, ApiTokenRead
from src.domain.services.repositories.entities.i_api_token_repository import (
    IApiTokenRepository,
)


class ApiTokenReadUseCase:
    ...
    async def execute(
        ...
        result = await self.api_token_repository.read(config=config, params=params)
        if not result:
