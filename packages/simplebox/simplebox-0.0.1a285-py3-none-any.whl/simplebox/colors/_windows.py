...
def _color(*objects, sep=' ', end="\n", flush: bool = False, file=sys.stdout,
    ...
    attribute = cf.value if cf is not None else ColorForeground.WHITE.value | cb.value if cb is not None else ColorBackground.BLACK.value
...
def _dye(string, cf: ColorForeground = ColorForeground.WHITE,
         cb: ColorBackground = ColorBackground.BLACK, style: Style = None):
    raise NotImplementedError("Windows cmd.exe not support front dye color, but can output color front.")
