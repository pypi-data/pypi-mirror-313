...
def _007CAssayPath_007C__007C(input: Array[str]) -> str | None:
    ...
    if (len(input) == 3) if (not equals_with(_arrow3089, input, None)) else False:
        if input[0] == "assays":
...
def ARCtrl_ArcAssay__ArcAssay_ToCreateContract_6FCE9E49(this: ArcAssay, WithFolder: bool | None=None) -> Array[Contract]:
    ...
    def _arrow3093(__unit: None=None, this: Any=this, WithFolder: Any=WithFolder) -> IEnumerable_1[Contract]:
        def _arrow3091(__unit: None=None) -> IEnumerable_1[Contract]:
            ...
            def _arrow3090(p: str) -> IEnumerable_1[Contract]:
                return singleton(Contract.create_create(p, DTOType(8))) if ((p != "assays/.gitkeep") if (p != path) else False) else empty()

            return collect(_arrow3090, folder_fs.ToFilePaths(False))

        def _arrow3092(__unit: None=None) -> IEnumerable_1[Contract]:
            return singleton(c)

        return append(_arrow3091() if with_folder else empty(), delay(_arrow3092))

    return to_array(delay(_arrow3093))
