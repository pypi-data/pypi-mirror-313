...
def _arrow2377(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2378(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2379(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2377, ROCrate_decoder_1), map(_arrow2378, ROCrate_decoder_2), map(_arrow2379, ROCrate_decoder_3)]))
...
def _arrow2386(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2388(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2390(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2386, ISAJson_decoder_1), map(_arrow2388, ISAJson_decoder_2), map(_arrow2390, ISAJson_decoder_3)]))
