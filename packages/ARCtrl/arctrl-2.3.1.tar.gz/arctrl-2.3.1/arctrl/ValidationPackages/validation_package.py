...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2754(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2753(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2752(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2751(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2751))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2752))

            return append(singleton("{"), delay(_arrow2753))

        return join("\n", to_list(delay(_arrow2754)))
...
ValidationPackage_reflection = _expr2755
