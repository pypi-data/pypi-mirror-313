...
def ValidationPackagesConfig_encoder(validationpackage: ValidationPackagesConfig) -> YAMLElement:
    ...
    def _arrow2906(value: str, validationpackage: Any=validationpackage) -> YAMLElement:
        return string(value)

    def _arrow2909(validationpackage_1: ValidationPackage, validationpackage: Any=validationpackage) -> YAMLElement:
        return ValidationPackage_encoder(validationpackage_1)

    obj_seq: FSharpList[tuple[str, YAMLElement]] = choose(chooser, of_array([try_include("arc_specification", _arrow2906, validationpackage.ARCSpecification), ("validation_packages", resizearray(_arrow2909, validationpackage.ValidationPackages))]))
    def _arrow2916(__unit: None=None, validationpackage: Any=validationpackage) -> IEnumerable_1[YAMLElement]:
        def _arrow2915(match_value: tuple[str, YAMLElement]) -> IEnumerable_1[YAMLElement]:
            return singleton(YAMLElement(0, YAMLContent_create_27AED5E3(match_value[0]), match_value[1]))

        return collect(_arrow2915, obj_seq)

    return YAMLElement(3, to_list(delay(_arrow2916)))


def _arrow2929(value_2: YAMLElement) -> ValidationPackagesConfig:
    def getter(get: IGetters) -> ValidationPackagesConfig:
        def _arrow2924(__unit: None=None, get: Any=get) -> Array[ValidationPackage]:
        ...
        def _arrow2925(__unit: None=None, get: Any=get) -> str | None:
            object_arg_1: IOptionalGetter = get.Optional
            return object_arg_1.Field("arc_specification", string_1)

        return ValidationPackagesConfig(_arrow2924(), _arrow2925())

    return object(getter, value_2)


ValidationPackagesConfig_decoder: Callable[[YAMLElement], ValidationPackagesConfig] = _arrow2929
...
def ARCtrl_ValidationPackages_ValidationPackagesConfig__ValidationPackagesConfig_toYamlString_Static_71136F3F(whitespace: int | None=None) -> Callable[[ValidationPackagesConfig], str]:
    def _arrow2937(vp: ValidationPackagesConfig, whitespace: Any=whitespace) -> str:
