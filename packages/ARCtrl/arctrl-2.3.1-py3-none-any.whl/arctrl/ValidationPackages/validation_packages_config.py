...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow2784(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2783(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2781(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2780(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow2779(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow2778(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow2778))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow2779))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow2780))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow2781))

            return append(singleton("{"), delay(_arrow2783))

        return join("\n", to_list(delay(_arrow2784)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow2791(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow2790(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow2790, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow2791))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr2802
