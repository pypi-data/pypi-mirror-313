...
def _arrow1968(Item: int) -> Value:
    return Value(1, Item)


def _arrow1969(Item_1: float) -> Value:
    return Value(2, Item_1)


def _arrow1970(Item_2: OntologyAnnotation) -> Value:
    return Value(0, Item_2)


def _arrow1971(Item_3: str) -> Value:
    return Value(3, Item_3)


decoder: Decoder_1[Value] = one_of(of_array([map(_arrow1968, int_1), map(_arrow1969, float_1), map(_arrow1970, OntologyAnnotation_ISAJson_decoder), map(_arrow1971, string)]))
