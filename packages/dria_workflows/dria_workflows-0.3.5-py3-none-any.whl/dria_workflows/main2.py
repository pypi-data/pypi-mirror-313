...
from dria_workflows.workflows.interface import StackPage, Entry, MessageInput
...
def workflow(query: str) -> Workflow:
    """
    ...
    builder.search_step(
        search_query=query,
        outputs=[Write.new("search_result")],
    )
    ...
    builder.set_return_value("search_result")
    return builder.build()


wf = workflow(query="Most expensive cities in the world")


wfx = wf.model_dump_json(exclude_none=True, exclude_unset=True)

print(wfx)

validate_workflow_json(wfx)
