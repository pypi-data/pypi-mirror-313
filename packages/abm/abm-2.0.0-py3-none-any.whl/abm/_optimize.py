...
from ._helper.helper import (
    ...
    necessary_measurements_columns,
    necessary_model_columns,
    optional_dose_columns,
    optional_fitting_parameter_columns,
    prior_distribution_column,
    prior_parameter_columns,
...
def validate_and_process_measurements_table(measurements: DataFrameLike | PathLike) -> tl.DataFrame:
    ...
    validate_columns(
        measurements_table, "measurement table", necessary_columns=necessary_measurements_columns, options=options
    )
...
def validate_and_process_fitting_parameter_tables(parameters: DataFrameLike | PathLike) -> tl.DataFrame:
    ...
    additional_necessary_columns = set()
    if "prior_distribution" in parameters_table.column_names:
        additional_necessary_columns |= prior_distribution_column
        if len(set(parameters_table[:, "prior_distribution"]) - {"uniform", "loguniform"}) > 0:
            additional_necessary_columns |= prior_parameter_columns

    necessary_columns = necessary_fitting_parameter_columns | additional_necessary_columns

    validate_columns(parameters_table, "parameter table", necessary_columns=necessary_columns, options=options)
