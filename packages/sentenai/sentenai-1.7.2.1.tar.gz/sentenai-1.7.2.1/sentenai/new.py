...
class Tempest(object):
    ...
    @property
    def dbs(self):
        return [DB(db, dt64(o)) for db, o in sorted(tempest.api._get('db').json().items())]
...
class DB:
    ...
    def init(self, origin=datetime(1970,1,1)):
        if origin == None:
            r = self._api._put("db", self.name, json={'origin': None})
        else:
            r = self._api._put("db", self.name, json={'origin': iso8601(origin)})
...
class Graph:
    ...
    def tree(self, limit=None):
        ...
        if limit is not None:
            p['limit'] = limit
        return tempest.api._get('db', self.db.name, 'graph', *self._path, params=p).json()
