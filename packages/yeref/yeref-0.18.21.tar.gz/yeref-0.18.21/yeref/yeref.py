...
from yeref.l_ import l_inline_demo, l_inline_bot, l_inline_post, l_inline_media, l_inline_channel, l_inline_group, \
    ...
    l_act_wait_title, l_msg_quiz, l_msg_text, l_inline_tonest, l_inline_support, l_donate, l_post_sticker_toobig, l_web_version, l_payment_of_post, l_payment_check_token
        ...
        for state_file in state_files:
            ...
            if GROUP_VID in tmp_json:
                ...
                async with aiofiles.open(tmp_file, 'w', encoding='utf-8') as f:
                    await f.write(json.dumps(tmp_json, ensure_ascii=False))
...
async def list_star_invoice_link(bot, PROJECT_USERNAME, PROJECT_TYPE):
    try:
        ...
        r_ = await bot.create_invoice_link(
            title=f"#{PROJECT_TYPE}",
...
async def pst_gen_ent2(bot, chat_id, lc, lz, POST_TYPE, POST_MEDIA, ENT_TID, ENT_TYPE, ENT_USERNAME, ENT_TITLE, MEDIA_D,
    ...
    try:
        ...
        print(f"{POSTB_FIDNOTE}, {POST_FID}, {POSTB_FID}, {POST_FIDNOTE}")
        # result = True
    except Exception as e:
        logger.info(log_ % str(e))
        await asyncio.sleep(round(random.uniform(0, 1), 2))
    ...
    try:
        ...
        for item in data:
            ...
            try:
                ...
                till_time = (datetime.now() + timedelta(seconds=e.value + 1)).strftime(
                    "%d-%m-%Y_%H-%M")
                sql = "UPDATE SESSION SET SESSION_STATUS = ? WHERE SESSION_TID = ?"
...
async def check_session_flood(SESSION_TID, BASE_S):
    ...
    try:
        ...
        if len(t_t) == 2:
            ...
            diff = datetime.now() - datetime(year=year, month=month, day=day, hour=hour,
                                             minute=minute)
...
async def get_list_of_send_folder(CONF_P, EXTRA_D):
    ...
    for k, v in file_list_dic.items():
        try:
            ...
            if parent_folder == '' and datetime_ < datetime.strptime(name_folder, "%d-%m-%Y %H:%M").replace(tzinfo=timezone.utc):
        ...
        for item in data:
            try:
                ...
                if USER_ISPAID == 1 and USER_DTPAID and (
                        dt_ - datetime.strptime(USER_DTPAID, '%d-%m-%Y_%H-%M-%S').replace(tzinfo=timezone.utc)).days > 31:
                    await asyncio.sleep(round(random.uniform(0, 1), 2))
...
async def save_posts(chat_id, lz, username, bot, posts, is_sub, tz, ENT_TID, BASE_ENT, BASE_D, PROJECT_USERNAME, PROJECT_TYPE):
    ...
    try:
        ...
        POST_PAY = f"https://t.me/{PROJECT_USERNAME}/web?startapp={ENT_TID}_{POST_TID}_0_pay" if posts['POST_CHKBOX'][
                                                                                                     'POST_ISPAY'] and POST_TYPE in [
                                                                                                     'photo',
                                                                                                     'video'] else ''
        ...
        POST_WATER = posts['POST_WATER']
        POST_STARS = posts['POST_STARS']
        POST_ISPRIVATE = posts['POST_CHKBOX']['POST_ISPRIVATE']
        POST_PAYLINK = None
        POST_TOKEN = posts['POST_TOKEN']
        POST_TZ = tz
        POST_DT = posts['POST_DT']
        POST_TR = posts['POST_TR']

        result = {'result': True, 'bot': True, 'blog': POST_BLOG}
        if posts['POST_CHKBOX']['POST_ISPAY'] and POST_TOKEN and len(json.loads(POST_MEDIA)):
        ...
        if posts['POST_CHKBOX']['POST_ISPAY'] and POST_STARS and POST_TOKEN:
            try:
                extra_bot = Bot(token=POST_TOKEN)
                try:
...
async def post_pub(bot, lz, chat_id, ENT_TID, item, MEDIA_D, BASE_D, PROJECT_USERNAME, is_private=True, message_id=None, current=1):
    result = False
    try:
                ...
                if member_.can_post_stories and member_.can_promote_members:
                    _ = asyncio.get_event_loop().create_task(
                        story_start(bot, lz, POST_USERTID, ENT_TID, POST_TYPE, POST_TEXT, POST_MEDIA[0]['file_id'],
                                    POST_MEDIA[0]['file_name'], POST_CHKBOX))
        except Exception as e:
            logger.info(log_ % str(e))
            await asyncio.sleep(round(random.uniform(0, 1), 2))
        # endregion
        ...
        try:
            if result and result.chat.username and POST_CHKBOX['POST_ISPODCAST'] and int(chat_id) == int(
                ...
                member_ = await bot.get_chat_member(chat_id=int(ENT_TID), user_id=int(BOT_TOKEN.split(':')[0]))
                ...
                if member_.can_manage_video_chats and member_.can_promote_members:
                    ...
                    _ = asyncio.get_event_loop().create_task(
                        podcast_start(bot, POST_USERTID, lz, ENT_TID, POST_TID, POST_TYPE, POST_TEXT,
                                      POST_MEDIA[0]['file_id'], POST_MEDIA[0]['file_name'], USER_ISPAID))
        except Exception as e:
            logger.info(log_ % str(e))
            await asyncio.sleep(round(random.uniform(0, 1), 2))
        # endregion
...
async def update_media(bot, chat_id, lz, ENT_TID, POST_TOKEN, POST_MEDIA):
...
async def podcast_start(bot, chat_id, lz, ENT_TID, POST_ID, POST_TYPE, POST_TEXT, POST_FID, POST_FILENAME, USER_ISPAID):
    ...
    try:
        ...
        for _ in data[:10]:
            ...
            SESSION_TID, SESSION_STATUS = data_[0]
...
async def py_tg_calls_fun(bot, app, chat_id, lz, r, POST_ID, ENT_TID, stream, duration):
