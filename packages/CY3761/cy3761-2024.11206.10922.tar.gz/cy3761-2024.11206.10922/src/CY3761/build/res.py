...
root_05 = root_01.parent / 'template' / root_01.name.replace(root_01.suffix, '')
...
def build_base64(rows):
    ...
    for i, v in enumerate(rows):
        a_00.extend([
            a_032.join([a_035, pad_zero(i), atob(v)])
        ])
...
# 由于文件可能被过滤 (如: main.py) 现使用模板的都添加一个前缀
def build_file(sn: str, name: str, suffix: str):
    pref = 't_'
    base = name + '.' + suffix
    data = storage(root_05 / (pref + base))

    # print(root_05 / name)
    # print(data)

    ssrc = Path(switch(sn == '', sn + a_095 + base, base))
