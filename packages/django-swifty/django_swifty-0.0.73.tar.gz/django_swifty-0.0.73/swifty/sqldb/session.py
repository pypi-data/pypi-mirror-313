...
class SwiftyDBSessionMixin(SwiftyLoggerMixin):
    ...
    def bulk_update_model_data(
        ...
        update_status = self.session.bulk_update_mappings(model, data)
        if update_status:
            return self.session.query(model).filter_by(**pk_filter).all()
    ...
    def update_or_create_model_data(
        ...
        """
        bulk = isinstance(data, (list, tuple))
        extra_data_dict = extra_data_dict or {}
        pk_filter = pk_filter or {}
        try:
            if bulk:
                data: list[dict] = [
                    {**sub_data, **extra_data_dict, **pk_filter} for sub_data in data
                ]
                return self.bulk_update_model_data(
                    model=model, data=data, pk_filter=pk_filter
                )
