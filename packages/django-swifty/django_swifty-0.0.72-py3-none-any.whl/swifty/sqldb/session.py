...
class SwiftyDBSessionMixin(SwiftyLoggerMixin):
    ...
    def bulk_update_model_data(self, model, data: list[dict]):
        """_summary_
        ...
        Args:
            ...
            data (_type_): _description_
        """
        return self.session.bulk_update_mappings(model, data)
    ...
    def update_or_create_model_data(
        ...
        try:
            if bulk:
                data: list[dict] = [
                    {**sub_data, **extra_data_dict} for sub_data in data
                ]
                return self.bulk_update_model_data(model=model, data=data)
