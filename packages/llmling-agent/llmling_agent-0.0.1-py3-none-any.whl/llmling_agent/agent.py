...
from pydantic_ai.result import ResultData, RunResult, StreamedRunResult
from typing_extensions import TypeVar

from llmling_agent.agentlogger import AgentLogger, InteractionType
...
class LLMlingAgent[TResult]:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            db_url: SQLAlchemy URL for logging database
        ...
        *,
        ...
        try:
            ...
            await self._logger.log_start(
                ...
                model_name=str(model or self.pydantic_agent.model),
            )
            ...
            await self._logger.log_complete(result, duration)
    ...
    async def run_stream(
        ...
        *,
        message_history: list[Message] | None = None,
        model: models.Model | models.KnownModelName | None = None,
    ) -> StreamedRunResult[TResult, Message]:
        ...
        try:
            # Log stream start
            await self._logger.log_stream_start(
                prompt=prompt,
                messages=message_history or [],
                model_name=str(model or self.pydantic_agent.model),
            )
            ...
            async with self.pydantic_agent.run_stream(
                ...
                try:
                    async for chunk in stream:
                        await self._logger.log_stream_chunk(chunk)
                        yield chunk

                    # Log successful completion
                    duration = (datetime.utcnow() - start_time).total_seconds() * 1000
                    await self._logger.log_stream_complete(
                        final_result=stream.get(),
                        duration_ms=duration,
                        total_tokens=stream.usage.total_tokens if stream.usage else None,
                        cost=stream.cost,
                    )
                except Exception as e:
                    duration = (datetime.utcnow() - start_time).total_seconds() * 1000
                    await self._logger.log_error(
                        e,
                        InteractionType.STREAM_ERROR,
                        duration,
                    )
                    raise

        except Exception as e:
            await self._logger.log_error(e, InteractionType.STREAM_ERROR)
            raise
    ...
    def _setup_default_tools(self) -> None:
        ...
        async def load_resource(
            ...
            This tool can be used to load resources based on a URI.
            Available resources can be looked up using list_resources tool.

            Args:
                ctx: Context
                uri: Resource URI to load
            """
            return await ctx.deps.load_resource_by_uri(uri)

        @self.tool
        async def list_resources(ctx: RunContext[RuntimeConfig]) -> Sequence[Resource]:
            """List available resources."""
            return ctx.deps.get_resources()
    ...
    @property
    def last_run_messages(self) -> list[Message] | None:
    ...
    async def _validate_result(
        ...
        """Validate result with logging."""
        for validator in self._result_validators:
            result_before = result_data
            result_data = await validator.validate(
                result_data,
                deps,
                self._current_result_retry,
                tool_call,
            )
            # Log validation
            await self._logger.log_validation(
                result=result_data,
                validation_name=validator.func.__name__,
                retry_count=self._current_result_retry,
            )
        return result_data
