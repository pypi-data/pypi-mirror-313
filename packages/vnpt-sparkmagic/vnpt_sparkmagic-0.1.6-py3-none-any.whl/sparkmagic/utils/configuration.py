...
from hdijupyterutils.configuration import override_all as _override_all, get_current_default_config
...
def override_resources_of_spark_session(config, value):
    """_summary_
    Override default resources of spark session
    that user setup when starting notebook server

    Don't allow user override initial resources of spark
    """
    if config == "session_configs":
        spark_resources_conf = get_current_default_config(config_name=config, path=path)
        if spark_resources_conf:
            spark_resources_conf.update(value)
            print("=================SPARK RESOURCES==================")
            print(spark_resources_conf)
            _override(d, path, config, spark_resources_conf)
