...
def _android_path_to_os(config, path: PosixPath):
    path = path.relative_to(config.adb_push_dir)
    return fs.music_dir() / Path(path)


# def _is_known(config, path: PosixPath):
#     return cleaner.is_known(_android_path_to_os(config, path), strict_resolve=False)


def _push_allowed_files_list(config, artist: str, path: PosixPath, files: list):
    temp_file: Path = fs.adb_push_files()
    fs.touch_file(temp_file)

    with open(temp_file, "w") as file:
        for f in files:
            print(config.adb_push_dir / artist / PosixPath(f), file=file)
...
def clean(config, artist: str, file: PosixPath):
