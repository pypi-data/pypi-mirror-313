...
class OrderbookManager:
    ...
    def calculate_vwap(orders: List[Tuple], target_amount: float) -> List[Tuple]:
        ...
        total_amount = 0
        total_value = 0
        exchange = orders[0][2]  # Get exchange from first order
        ...
        for price, amount, _ in orders:
            if total_amount + amount >= target_amount:
                ...
                # Add this partial amount
                total_value += price * needed_amount

                # Create VWAP level with exact target amount
                vwap_price = total_value / target_amount
                vwap_orders.append((vwap_price, target_amount, exchange))

                # Start new chunk with remaining amount
                excess = amount - needed_amount
                if excess > 0:
                    total_value = price * excess
                    total_amount = excess
                else:
                    total_value = 0
                    total_amount = 0
            ...
            for symbol, book in exchange_obs.items():
                ...
                if symbol in self.weighting:
                    ...
                    if currency == base:
                        ...
                        base_target = target_amount / book['bids'][0][0]
    ...
    def aggregate_orderbooks(self, orderbooks: Dict) -> Dict:
        """Aggregate orderbooks while preserving VWAP structure."""
        aggregated = {}
        for symbol in self.symbols:
            ...
            for exchange, exchange_obs in orderbooks.items():
                ...
                if ob:
                    # Ensure we're adding exchange info to the orders
                    bids = [(price, amount, ex if len(order) > 2 else exchange)
                            for price, amount, *rest in ob['bids']
                            for ex in (rest if rest else [exchange])]
                    asks = [(price, amount, ex if len(order) > 2 else exchange)
                            for price, amount, *rest in ob['asks']
                            for ex in (rest if rest else [exchange])]
            ...
            if all_bids and all_asks:
                # Always sort by price, maintaining exchange information
