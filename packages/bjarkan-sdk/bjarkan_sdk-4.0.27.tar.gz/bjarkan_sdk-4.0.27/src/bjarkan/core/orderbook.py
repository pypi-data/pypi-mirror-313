"""
Manages orderbook data collection and processing from multiple exchanges.
Supports aggregation, VWAP calculations, custom weighting, and fee adjustments.
"""
...
class OrderbookManager:
    ...
    @catch_exception
    def _process_raw_orderbook(self, raw_orderbook: Dict, exchange_name: str, symbol: str, timestamp: int) -> Dict:
        ...
        processed_book = {
            'timestamp': timestamp,
            'exchange_timestamp': raw_orderbook.get('timestamp'),
            'symbol': symbol,
            'bids': bids,
            'asks': asks
        }
    ...
    def _aggregate_orderbooks(self, orderbooks: List[Dict]) -> Dict:
        ...
        for ob in orderbooks:
            if ob.get('exchange_timestamp'):
                exchange_timestamps.append(ob['exchange_timestamp'])
        ...
        aggregated_book = {
            ...
            'exchange_timestamp': min(exchange_timestamps) if exchange_timestamps else None,
    ...
    async def _get_final_orderbook(self, symbol: str) -> Dict:
        ...
        if self.config.aggregated:
            final_book = self._aggregate_orderbooks(books)
            return final_book
