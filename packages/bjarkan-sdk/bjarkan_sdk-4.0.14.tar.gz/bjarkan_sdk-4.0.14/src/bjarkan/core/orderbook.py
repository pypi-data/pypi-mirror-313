...
class OrderbookManager:
    ...
    @staticmethod
    @catch_exception
    def calculate_vwap(orders: List[Tuple], target_amount: float, is_quote_currency: bool = False) -> List[Tuple]:
        """Calculate VWAP for specific target amount chunks."""
        if not orders:
            return []
        ...
        for price, amount, _ in orders:
            if is_quote_currency:
                ...
                if total_value + order_value >= target_amount:
                    # Calculate how much of this order we need
                    needed_value = target_amount - total_value
                    needed_amount = needed_value / price

                    # Add to totals
                    total_value += needed_value
                    total_amount += needed_amount

                    # Calculate VWAP and add order
                    vwap_price = total_value / total_amount
                    vwap_orders.append((vwap_price, total_amount, exchange))

                    # Reset for next chunk
                    remaining_amount = amount - needed_amount
                    total_amount = 0
                    total_value = 0

                    # Process remaining amount if significant
                    if remaining_amount * price >= target_amount:
                        new_needed_amount = target_amount / price
                        vwap_orders.append((price, new_needed_amount, exchange))
                else:
                ...
                if total_amount + amount >= target_amount:
                    ...
                    # Process remaining amount if it's enough for another chunk
                    remaining = amount - needed_amount
                    if remaining >= target_amount:
                        vwap_orders.append((price, target_amount, exchange))
        ...
        for exchange, exchange_orderbooks in orderbooks.items():
            ...
            for symbol, book in exchange_orderbooks.items():
                ...
                if symbol in self.weighting:
                    ...
                    is_quote_currency = currency == quote

                    if currency == base:
                        # Direct VWAP calculation for base currency targets
                        vwap_orderbooks[exchange][symbol]['bids'] = self.calculate_vwap(
                            book['bids'], target_amount, False)
                        vwap_orderbooks[exchange][symbol]['asks'] = self.calculate_vwap(
                            book['asks'], target_amount, False)
                    elif is_quote_currency:
                        # VWAP calculation for quote currency targets
                        vwap_orderbooks[exchange][symbol]['bids'] = self.calculate_vwap(
                            book['bids'], target_amount, True)
                        vwap_orderbooks[exchange][symbol]['asks'] = self.calculate_vwap(
                            book['asks'], target_amount, True)
