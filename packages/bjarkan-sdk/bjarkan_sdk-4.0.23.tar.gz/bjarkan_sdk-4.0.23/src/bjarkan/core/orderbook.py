...
class OrderbookManager:
        ...
        while self.running:
            try:
                ...
                async with self.lock:
                    ...
                    # Step 4: Process based on VWAP and aggregation settings
                    if self.weighting:
                        vwap_books = self.apply_vwap(current_books)
                        if self.aggregated:
                            final_books = self.aggregate_vwap_orderbooks(vwap_books)
                            book_to_send = final_books.get(symbol)
                        else:
                            book_to_send = vwap_books.get(exchange_name, {}).get(symbol)
                    else:
                        if self.aggregated:
                            final_books = self.aggregate_orderbooks(current_books)
                            book_to_send = final_books.get(symbol)
                        else:
                            book_to_send = processed_orderbook

                    # Update aggregated books if needed
                    if self.aggregated:
                        self.aggregated_orderbooks[symbol] = book_to_send

                # Step 5: Execute callbacks with appropriate data
                if book_to_send:
    ...
    def calculate_vwap(orders: List[Tuple], target_amount: float) -> List[Tuple]:
        ...
        for price, amount, _ in orders:
            if total_amount + amount >= remaining_amount:
                partial_amount = remaining_amount - total_amount
        ...
        # Add any remaining amount as a partial VWAP
        if total_amount > 0:
            vwap_price = total_value / total_amount
            ...
            for symbol, book in exchange_orderbooks.items():
                ...
                if symbol in self.weighting:
                    ...
                    if currency == base:
                        ...
                        quote_target = target_amount / book['bids'][0][0]  # Approximate quote currency amount
    ...
    def aggregate_vwap_orderbooks(self, vwap_orderbooks: Dict[str, Dict[str, Dict]]) -> Dict[str, Dict]:
        aggregated = {}
        for exchange_obs in vwap_orderbooks.values():
            for symbol, ob in exchange_obs.items():
                if symbol not in aggregated:
                    aggregated[symbol] = {'bids': [], 'asks': []}
                aggregated[symbol]['bids'].extend(ob['bids'])
                aggregated[symbol]['asks'].extend(ob['asks'])

        for symbol, ob in aggregated.items():
            ob['timestamp'] = int(time.time() * 1000)
            ob['symbol'] = symbol
            ob['bids'] = sorted(ob['bids'], key=lambda x: x[0], reverse=True)[:self.depth]
            ob['asks'] = sorted(ob['asks'], key=lambda x: x[0])[:self.depth]
    ...
    async def get_latest_orderbooks(self) -> Dict[str, Dict]:
        async with self.lock:
            raw_orderbooks = {ex: {symbol: ob.copy() for symbol, ob in symbol_obs.items() if ob}
                              for ex, symbol_obs in self.orderbooks.items()}

            # Step 4: calculate VWAP orderbooks for each exchange individually
            # Step 5: aggregate all books
            if self.weighting:
                vwap_orderbooks = self.apply_vwap(raw_orderbooks)
                if self.aggregated:
                    final_orderbooks = self.aggregate_vwap_orderbooks(vwap_orderbooks)
                else:
                    final_orderbooks = vwap_orderbooks
            else:
                if self.aggregated:
                    final_orderbooks = self.aggregate_orderbooks(raw_orderbooks)
                else:
                    final_orderbooks = raw_orderbooks
