...
class OrderbookManager:
    ...
    def calculate_vwap(orders: List[Tuple], target_amount: float) -> List[Tuple]:
        """Calculate VWAP for exact target amount chunks."""
        if not orders:
            return []
        ...
        while remaining_orders and len(vwap_orders) < 10:  # Limit to 10 levels
            price, amount, _ = remaining_orders[0]

            if chunk_amount + amount >= target_amount:
                # Calculate how much we need from this order
                needed = target_amount - chunk_amount
                chunk_value += price * needed

                # Calculate VWAP for complete chunk
                vwap_price = chunk_value / target_amount
                vwap_orders.append((vwap_price, target_amount, exchange))

                # Set up for next chunk
                remaining = amount - needed
                if remaining > 0:
                    remaining_orders[0] = (price, remaining, exchange)
                else:
                ...
                # Add entire order to current chunk
                chunk_value += price * amount
                chunk_amount += amount
                remaining_orders.pop(0)
        ...
        for exchange, exchange_obs in orderbooks.items():
            ...
            for symbol, book in exchange_obs.items():
                ...
                if symbol in self.weighting:
                    currency, target = next(iter(self.weighting[symbol].items()))
                    base, quote = symbol.split('/')

                    if currency == base:
                        # Base currency target (e.g., 1.0 BTC or 10.0 ETH)
                        vwap_orderbooks[exchange][symbol]['bids'] = self.calculate_vwap(book['bids'], target)
                        vwap_orderbooks[exchange][symbol]['asks'] = self.calculate_vwap(book['asks'], target)
                    elif currency == quote:
                        # Quote currency target (e.g., 10000 USDT)
                        if book['bids']:
                            # For bids: convert USDT target to base currency
                            base_target = target / book['bids'][0][0]
                            vwap_orderbooks[exchange][symbol]['bids'] = self.calculate_vwap(book['bids'], base_target)
                        if book['asks']:
                            # For asks: convert USDT target to base currency
                            base_target = target / book['asks'][0][0]
                            vwap_orderbooks[exchange][symbol]['asks'] = self.calculate_vwap(book['asks'], base_target)
