    ...
    def calculate_vwap(orders: List[Tuple], target_amount: float) -> List[Tuple]:
        ...
        for price, amount, _ in orders:
            if total_amount + amount >= target_amount:
                partial_amount = target_amount - total_amount
                ...
                break  # Exit after reaching target amount
            else:
                total_value += price * amount
                total_amount += amount

        if not vwap_orders and total_amount > 0:
            # If we couldn't reach target amount, use what we have
            vwap_price = total_value / total_amount
