...
class OrderbookManager:
    ...
    def calculate_vwap(orders: List[Tuple], target_amount: float) -> List[Tuple]:
        ...
        for price, amount, _ in orders:
            # Add current order to accumulation
            current_orders.append((price, amount))
            total_amount += amount
            total_value += price * amount

            # While we have enough for one or more chunks
            while total_amount >= target_amount:
                # Calculate VWAP for this chunk
                vwap_price = total_value / total_amount
                vwap_orders.append((vwap_price, target_amount, exchange))

                # Remove the used amount
                total_amount -= target_amount
                total_value = total_amount * vwap_price  # Remaining value at VWAP price

                # If we don't have enough for another chunk, break
        ...
        for exchange, exchange_orderbooks in orderbooks.items():
            ...
            for symbol, book in exchange_orderbooks.items():
                ...
                if symbol in self.weighting:
                    ...
                    if currency == base:
                        # Direct VWAP for base currency (e.g., BTC, ETH)
                        ...
                        # VWAP for quote currency (e.g., USDT)
                        if book['bids']:
                            # Convert USDT target to base currency amount
                            avg_price = book['bids'][0][0]
                            base_target = target_amount / avg_price
