...
class OrderbookManager:
    ...
    def calculate_vwap(orders: List[Tuple], target_amount: float) -> List[Tuple]:
        """Calculate VWAP for equal-sized chunks of the target amount."""
        if not orders:
            return []
        ...
        while remaining_orders:
            ...
            for i, (price, amount, _) in enumerate(remaining_orders):
                if accumulated + amount >= target_amount:
                    # We only need a portion of this order
                    needed = target_amount - accumulated
                    vwap_value += price * needed
                    accumulated += needed
                    orders_used = i + 1
                    break
                else:
                    vwap_value += price * amount
                    accumulated += amount
                    orders_used = i + 1

            if accumulated >= target_amount:
                # Add a VWAP level with exactly the target amount
                vwap_price = vwap_value / accumulated
                result.append((vwap_price, target_amount, exchange))

                # Remove used orders and adjust the partially used last order
                last_order = remaining_orders[orders_used - 1]
                remaining_amount = last_order[1] - (accumulated - target_amount)
                remaining_orders = remaining_orders[orders_used:]
                if remaining_amount > 0:
                    remaining_orders.insert(0, (last_order[0], remaining_amount, last_order[2]))
            else:
                break
    ...
    def apply_vwap(self, orderbooks: Dict[str, Dict[str, Dict]]) -> Dict[str, Dict[str, Dict]]:
        """Apply VWAP calculation to orderbooks from each exchange."""
        ...
        for exchange, exchange_obs in orderbooks.items():
            ...
            for symbol, book in exchange_obs.items():
                ...
                if symbol in self.weighting:
                    ...
                    if currency == base:
                        # Direct VWAP calculation for base currency targets
                        ...
                        # Convert quote currency target to base currency
                        base_target_bid = target_amount / book['bids'][0][0]
                        base_target_ask = target_amount / book['asks'][0][0]
