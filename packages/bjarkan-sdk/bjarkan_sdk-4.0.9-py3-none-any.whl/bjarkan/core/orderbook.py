...
class OrderbookManager:
    ...
    @staticmethod
    @catch_exception
    def calculate_vwap(orders: List[Tuple], target_amount: float, exchange: str) -> List[Tuple]:
        vwap_orders = []
        total_amount = 0
        total_value = 0
        current_chunk = []
        ...
        for price, amount, _ in orders:
            current_chunk.append((price, amount))
            total_value += price * amount
            total_amount += amount

            if total_amount >= target_amount:
                vwap_price = total_value / total_amount
                vwap_orders.append((vwap_price, target_amount, exchange))  # Keep exchange info

                # Start new chunk with leftover
                leftover = total_amount - target_amount
                if leftover > 0:
                    last_price, _ = current_chunk[-1]
                    total_value = last_price * leftover
        ...
        for exchange, exchange_orderbooks in orderbooks.items():
            ...
            for symbol, book in exchange_orderbooks.items():
                if symbol in self.weighting:
                    ...
                    vwap_book = book.copy()
                    if currency == base:
                        vwap_book['bids'] = self.calculate_vwap(book['bids'], target_amount, exchange)
                        vwap_book['asks'] = self.calculate_vwap(book['asks'], target_amount, exchange)
                    elif currency == quote and book['bids']:
                        quote_target = target_amount / book['bids'][0][0]
                        vwap_book['bids'] = self.calculate_vwap(book['bids'], quote_target, exchange)
                        vwap_book['asks'] = self.calculate_vwap(book['asks'], quote_target, exchange)

                    vwap_orderbooks[exchange][symbol] = vwap_book
                else:
