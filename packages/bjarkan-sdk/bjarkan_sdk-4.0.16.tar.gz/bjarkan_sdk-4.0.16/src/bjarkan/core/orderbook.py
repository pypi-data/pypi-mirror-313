...
class OrderbookManager:
    ...
    def calculate_vwap(orders: List[Tuple], target_amount: float) -> List[Tuple]:
        """Calculate VWAP levels of exactly target_amount size."""
        if not orders:
            return []
        ...
        while price_level_index < len(orders) and len(vwap_orders) < 10:  # Limit to 10 VWAP levels
            price, amount, _ = orders[price_level_index]

            if accumulated_amount + amount >= target_amount:
                # We can complete a VWAP chunk
                needed = target_amount - accumulated_amount
                total_value += price * needed
                vwap_price = total_value / target_amount
        ...
        for exchange, exchange_obs in orderbooks.items():
            ...
            for symbol, book in exchange_obs.items():
                ...
                if symbol in self.weighting:
                    ...
                    if currency == base:
                        # Direct base currency target (e.g. BTC, ETH)
                        ...
                        # Quote currency target (e.g. USDT)
                        first_price = book['bids'][0][0]
                        base_target = target / first_price
                        vwap_orderbooks[exchange][symbol]['bids'] = self.calculate_vwap(book['bids'], base_target)

                        first_ask = book['asks'][0][0]
                        base_target_ask = target / first_ask
