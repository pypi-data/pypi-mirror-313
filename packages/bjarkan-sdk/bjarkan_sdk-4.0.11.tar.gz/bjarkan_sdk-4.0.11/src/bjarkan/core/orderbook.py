    ...
    def aggregate_orderbooks(self, orderbooks: Dict) -> Dict:
        ...
        for symbol in self.symbols:
            ...
            for exchange, exchange_obs in orderbooks.items():
                ...
                if ob:
                    # Process each bid/ask tuple correctly
                    bids = [(float(price), float(amount), source)
                            for price, amount, source in ob['bids']]
                    asks = [(float(price), float(amount), source)
                            for price, amount, source in ob['asks']]
