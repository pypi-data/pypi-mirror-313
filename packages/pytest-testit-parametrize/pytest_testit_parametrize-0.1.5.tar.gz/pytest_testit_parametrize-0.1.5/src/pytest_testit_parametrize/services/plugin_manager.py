...
class TmsPluginManager:
    ...
    def get_adapter_manager(cls, option=None):
        if cls.__adapter_manager is None:
            ...
            from testit_python_commons.services.adapter_manager import (
                AdapterManager,
            )
            ...
            cls.__adapter_manager = AdapterManager(
                adapter_configuration, client_configuration
            )

        return cls.__adapter_manager

    @classmethod
    def get_parameter_manager(cls, option=None):
        if cls.__parameter_manager is None:
            from testit_python_commons.client.client_configuration import (
                ClientConfiguration,
            )

            from pytest_testit_parametrize.services.parameter_manager import (
                ParameterManager,
            )
            ...
            cls.__parameter_manager = ParameterManager(client_configuration)

        return cls.__parameter_manager
