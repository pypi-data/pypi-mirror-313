    ...
    while idx < total_parts:
        ...
        while (worker_count["value"] < max_workers) and (idx < total_parts):
            for _ in range(max_retries):
                try:
                    print(f"Semaphore before acquire: {MAX_CONCURRENT_UPLOADS - upload_semaphore._value}")
                    upload_semaphore.acquire()
                    print(f"Semaphore after acquire: {MAX_CONCURRENT_UPLOADS - upload_semaphore._value}")
