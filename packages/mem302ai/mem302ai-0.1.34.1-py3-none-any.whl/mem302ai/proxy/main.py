...
from mem302ai import Memory, MemoryClient
from mem302ai.configs.prompts import MEMORY_ANSWER_PROMPT
from mem302ai.memory.telemetry import capture_client_event
...
class Completions:
    ...
    def _format_query_with_memories(self, messages, relevant_memories):
        ...
        if isinstance(self.mem0_client, mem302ai.memory.main.Memory):
            memories_text = "\n".join(memory["memory"] for memory in relevant_memories["results"])
        elif isinstance(self.mem0_client, mem302ai.client.main.MemoryClient):
