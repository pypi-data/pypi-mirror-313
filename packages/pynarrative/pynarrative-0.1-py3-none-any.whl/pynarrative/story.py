...
class Story:
    ...
    def add_context(self, text, position='left', color=None, dx=0, dy=0):
        """
    ...
    def add_source(self, text, position='bottom', vertical=False, color=None):
        """
    ...
    def create_title_layer(self, layer):
        ...
        title_chart = alt.Chart(self.chart.data).mark_text(
            ...
            y=alt.value(-50)  # Position 20 pixels from above
        )
        ...
        if layer['subtitle']:
            subtitle_chart = alt.Chart(self.chart.data).mark_text(
                ...
                x=alt.value(self.chart.width / 2),  # Centre horizontally
                y=alt.value(-20)  # Position 50 pixels from the top (below the title)
            )
