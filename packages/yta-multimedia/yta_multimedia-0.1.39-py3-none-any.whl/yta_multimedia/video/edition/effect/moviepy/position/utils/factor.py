...
def get_factor_to_fit_scene(video_position: Union[tuple, Coordinate, Position], position_in_scene: Union[tuple, Coordinate, Position], video_size: tuple, scene_size: tuple = (1920, 1080)):
    ...
    # its center to be able to calculate the factor later
    video_and_scene_size_difference = (
    ...
    # We calculate the factor we need to apply to resize the
    # video to fit the actual scene size and avoid any black
    # region
    factor = max(
        scene_size[0] / video_and_scene_size_difference[0],
        scene_size[1] / video_and_scene_size_difference[1]
    )

    return factor

def get_factor_to_fit_region(region: ImageRegion, scene_size: tuple = (1920, 1080)):
    """
    This method calculates the factor we need to apply to the
    resize method to obtain a video in which the provided 
    'region' is displayed fitting the whole scene with its
    center in the middle of it.
    """
    if not PythonValidator.is_instance(region, ImageRegion):
        raise Exception('The provided "region" is not a valid Region class instance.')
    
    if not PythonValidator.is_tuple(scene_size) or len(scene_size) != 2:
        raise Exception('The provided "scene_size" is not a valid size.')
    
    # We calculate the resize factor we need to make the
    # video fit only the region size
    factor = max(
        scene_size[0] / region.width,
        scene_size[1] / region.height
    )
