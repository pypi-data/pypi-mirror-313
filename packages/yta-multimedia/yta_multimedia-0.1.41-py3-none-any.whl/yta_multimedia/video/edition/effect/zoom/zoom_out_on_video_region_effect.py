...
class ZoomOutOnVideoRegionEffect(Effect):
    """
    Creates a zoom out effect starting from the center of
    the provided region, showing only the region, and
    ending with the whole video being shown. The zoom out
    is from the region center to the whole video center.
    """
    def apply(cls, video: Clip, region: ImageRegion, rate_func: type = RateFunction.linear):
        ...
        # whole scene size (without black regions)
        resizes = [
            TFunctionResize.resize_from_to(t, video.duration, factor_to_fit_region_size, 1, RateFunction.linear) for t in video_handler.frame_time_moments
        ]

        # Calculate new difference between the real center 
        # and the new center of the video
        video_center_difference = (
            video.w / 2 - region.center[0],
            video.h / 2 - region.center[1]
        )

        # I need to make the video_center_difference go from
        # 1 * n to 0 * n to start in the exact position but 
        # end centered in the whole video
        positions = [
    ...
    # a zoom out but using not the whole video but only the size
    # that makes, with the region still on the center of the 
    # scene, fit the maximum video size without showing black
    # borders
    def test(cls, video: Clip, region: ImageRegion, rate_func: type = RateFunction.linear):
        # TODO: What about 'rate_func' (?)
        video = VideoParser.to_moviepy(video)
