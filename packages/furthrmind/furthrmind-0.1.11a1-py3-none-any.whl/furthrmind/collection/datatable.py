...
class DataTable(BaseClass):
    ...
    def add_column(self, name: str, type: str, data: List, unit: Union[Dict, str] = None, 
        ...
        column = Column.create(name, type, data, unit, project_id=project_id)
        new_column_id_list = [c.id for c in self.columns]
        if pos:
            new_column_id_list.insert(pos, column.id)
        else:
            new_column_id_list.append(column.id)
            
        data = {"columns": new_column_id_list}
