...
from app.utils.list_utils import DEFAULT_PAGE_NUMBER, ACTIVITY_DEFAULT_PAGE_SIZE
...
class ActivityRepository(BaseRepository):
    ...
    def find_all_by_alert_id(
        ...
        *,
        ...
        cursor = self._collection.find(query).sort("created_at", pymongo.DESCENDING).skip(skip).limit(limit)

        if cursor is None:
            return []

        activities = [AlertActivity(**document) for document in cursor if document is not None]
