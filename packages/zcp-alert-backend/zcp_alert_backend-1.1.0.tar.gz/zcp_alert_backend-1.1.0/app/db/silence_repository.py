...
class SilenceRepository(BaseRepository):
    ...
    def insert(self, silence: Silence) -> str:
        ...
        return str(
            self._collection.insert_one(silence_dict).inserted_id
        )
    ...
    def __find_query(
        ...
        *, 
        ...
        query.update({"integrations": {"$in": [integration_id]}}) if integration_id else None
        ...
        if statuses is not None:
            ...
            for status in statuses:
                if status == SilenceStatus.ACTIVE:
                    or_query.append({"starts_at": {"$lte": current_time}, "ends_at": {"$gte": current_time}})
