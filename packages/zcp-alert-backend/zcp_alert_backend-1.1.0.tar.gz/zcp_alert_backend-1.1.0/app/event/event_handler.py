...
async def publish_alert_event(alerts: List[ZcpAlert]):
    ...
    for client_id, client_dict in clients.items():
        ...
        client_notification_settings: UserNotificationSettings = client_dict[SETTINGS_KEY]
        client_matched_alerts = [alert for alert in alerts if client_notification_settings.is_match(alert)]
...
async def consume_alert_event(
    ...
    if client_id not in clients:
        ...
        log.debug(f"Added new client queue for client[{client_id}]. Total clients: {len(clients)}")
    ...
    try:
        ...
        log.debug(f"Removed client queue for client[{client_id}]. Total clients: {len(clients)}")