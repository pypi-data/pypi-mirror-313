...
from app.model.integration_model import Filter, FilterMode, Integration, IntegrationStatus, IntegrationSortField
from app.schema.integration_request_model import IntegrationCreateRequest, IntegrationUpdateRequest
...
async def get_integrations(
    request: Request,
    name: Optional[str] = Query(None, max_length=100, title="channel name", description="Search query string for name. The max length is 100"),
    channel_name: Optional[str] = Query(None, max_length=100, title="channel name", description="Search query string for channel name. The max length is 100"),
    channel_type: ChannelType = Query(None, title="channel type", description="Search query string for channel type."),
    status: IntegrationStatus = Query(None, title="integration status", description="Search query string for status"),
    sort_field: IntegrationSortField = Query(IntegrationSortField.NAME, title="sort field", description="Sort field name"),
...
async def create_integration(
    ...
    return ResponseModel(data={"inserted_id": __service.create_integration(integration)})
...
async def modify_integration_status(
    ...
    status: IntegrationStatus = Path(..., title="Integration status", description="Integration status"),
    oauth_user: TokenData = Depends(get_current_user)
) -> Integration:
