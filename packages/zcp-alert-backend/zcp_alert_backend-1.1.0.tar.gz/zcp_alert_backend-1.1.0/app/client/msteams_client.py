...
from app.model.alert_model import Action, Priority, ZcpAlert, AlertStatus
...
class MSTeamsClient(HTTPBaseClient):
        ...
        if action is None:
            ...
            if alert.status == AlertStatus.OPEN:
                ...
                if alert.repeated_count > 0:
                    payload = self._convert_to_massage_for_repeated(
                        alert=alert
                    )
                else:
    ...
    def _convert_to_massage_for_user_actions(
        ...
        *, 
        ...
        subject = (
            f"{status_tilte if status_tilte else 'N/A'}"
        )        
        snoozed_info = (
        ...
        description = alert.annotations['description'] if alert.annotations['description'] else "N/A"
    ...
    def _convert_to_massage_for_new(
        self, 
        *, 
        alert: ZcpAlert
    ) -> str:
        ...
        """
        priority_for_color = alert.labels['priority'] if alert.labels.get('priority') else Priority.P5
    ...
    def _get_action_buttons(self, *, alert: ZcpAlert) -> List[Dict[str, Any]]:
        ...
        action_button_detail['targets'] = [
            {
                'os': 'default',
                'uri': f"{ALERT_VIEW_URL.format(alert_id=alert.id)}"
            }
        ]
    ...
    def _get_sections(self, *, alert: ZcpAlert) -> Dict[str, Any]:
        ...
        section['activityImage'] = "https://fonts.gstatic.com/s/e/notoemoji/15.1/1f6a8/512.png=s60"
    ...
    def _get_facts(self, *, alert: ZcpAlert) -> List[Dict[str, Any]]:
        ...
        facts.append({
            "name": "Priority",
            "value": alert.labels['priority'] if alert.labels.get('priority') else 'N/A'
        })
        facts.append({
            "name": "Severity",
            "value": alert.labels['severity'] if alert.labels.get('severity') else 'N/A'
        })
        facts.append({
            "name": "Cluster",
            "value": alert.labels['cluster'] if alert.labels.get('cluster') else 'N/A'
        })
        facts.append({
            "name": "Namespace",
            "value": alert.labels['namespace'] if alert.labels.get('namespace') else 'N/A'
        })
        facts.append({
            "name": "Occured At",
        ...
        facts.append({
            "name": "Sender",
            "value": alert.sender.value if alert.sender else 'N/A'
        })
        facts.append({
            "name": "ID",
            "value": alert.id
