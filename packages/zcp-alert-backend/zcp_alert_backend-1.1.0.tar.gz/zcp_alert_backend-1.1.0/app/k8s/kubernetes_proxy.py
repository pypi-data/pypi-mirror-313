...
class KubernetesProxy:
    def __init__(self):
        try:
            config.load_kube_config('/Users/kks/cloud/aws/clusters/dev-clusters/zcp-system-admin.conf')
    ...
    def get_pods(self, namespace:str|None):
         with self.__lock:
            if namespace is None:
                ...
                return [pod for pod in self.__pod_cache.values() if pod.metadata.namespace == namespace]
    ...
    def get_pod(self, namespace:str, pod_name:str):
        with self.__lock:
            for pod in self.__pod_cache.values():
                if pod.metadata.namespace == namespace and pod.metadata.name == pod_name:
                    return pod
            return None
    ...
    def get_pods_count(self, namespace:str|None):
        with self.__lock:
            if namespace is None:
                ...
                return len([pod for pod in self.__pod_cache.values() if pod.metadata.namespace == namespace])
