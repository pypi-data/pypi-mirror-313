...
import jwt
from jwt import ExpiredSignatureError, PyJWTError, InvalidIssuedAtError, InvalidKeyError
import requests

from app.model.auth_model import TokenData
import app.settings as settings
from app.exception.common_exception import OauthTokenValidationException, AlertError
...
# KeyCloak Endpoints
KEYCLOAK_REALM_ROOT_URL = f"{KEYCLOAK_SERVER_URL}/realms/{KEYCLOAK_REALM}/protocol/openid-connect"
...
def get_public_key():
    response = requests.get(KEYCLOAK_JWKS_ENDPOINT, verify=HTTP_CLIENT_SSL_VERIFY) # verify=False: because of the SKCC self-signed certificate
    jwks = response.json()
...
def verify_token(token: str) -> TokenData:
    try:
        ...
        raise OauthTokenValidationException(
            AlertError.INVALID_TOKEN, details=str(ese))
    except InvalidIssuedAtError as iiae:
        log.error(f"InvalidIssuedAtError: {iiae}")
        raise OauthTokenValidationException(
            AlertError.INVALID_TOKEN, details=str(iiae))
    except PyJWTError as jwte:
        log.error(f"JWTError: {jwte}")
        raise OauthTokenValidationException(
            AlertError.INVALID_TOKEN, details=str(jwte))
