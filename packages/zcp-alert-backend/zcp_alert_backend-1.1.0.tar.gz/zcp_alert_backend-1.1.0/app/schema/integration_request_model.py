...
class IntegrationCreateRequest(BaseModel):
    ...
    def assign_type_properties(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if filter_mode in [FilterMode.MATCH_ANY, FilterMode.MATCH_ALL]:
            if alert_filters is None or not isinstance(alert_filters, list) or len(alert_filters) == 0:
                raise ValueError(f"alert_filters is required more than at least 1 when filter_mode is {filter_mode}")
