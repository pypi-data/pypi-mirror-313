...
from app.model.alert_model import Action, ZcpAlert, AlertStatus
from app.model.kakaotalk_model import KakaoTalkToken, KakaoTalkFriend
...
class KakaoTalkClient:
    ...
    def _generate_payload(
        ...
        *,
        ...
        if action is None:
            if alert.status == AlertStatus.OPEN:
                if alert.repeated_count > 0:
                    status_info = f"Triggered the alert repeatedly {alert.repeated_count} times"
                else:
        ...
        snoozed_info = (
            ...
            if alert.status == AlertStatus.SNOOZED and alert.snoozed_until_at is not None
            else ""
        )
        ...
        log.debug(f"Generated KakaoTalk payload: {json.dumps(template_object, indent=2)}")
    ...
    def get_labels_text(self, zcp_alert: ZcpAlert) -> str:
        ...
        if zcp_alert.starts_at:
            labels_text += f"* Occured At: {zcp_alert.starts_at.astimezone(_DISPLAY_TIMEZONE).isoformat(timespec='milliseconds')}\n" if zcp_alert.starts_at else ""
        if zcp_alert.created_at:
            labels_text += f"* Triggered At: {zcp_alert.created_at.astimezone(_DISPLAY_TIMEZONE).isoformat(timespec='milliseconds')}\n" if zcp_alert.created_at else ""
        if zcp_alert.sender:
            labels_text += f"* Sender: {zcp_alert.sender.value}\n" if zcp_alert.sender else ""
        if zcp_alert.id:
    ...
    def kakao_refresh_token(
        ...
        *,
        ...
        response = requests.post(
            f"{KAKAO_AUTH_ENDPOINT}/oauth/token", data=data
        )
        new_token = {}
        if response.ok:
    ...
    def send_message_to_me(
        ...
        *,
        ...
        if response.ok:
            log.debug(f"Sent message successfully to me: {json.dumps(response.json(), indent=2)}")
    ...
    def send_message_to_friends(
        ...
        *,
        ...
        if response.ok:
            log.debug(f"Sent message successfully to friends: {json.dumps(response.json(), indent=2)}")
            return {"result": "success"}, None
        else:
            return {"result": "failed"}, response.text
    ...
    def send_message_to_all(
        ...
        *,
        ...
        response, message = self.send_message_to_me(alert=alert, action=action, modifier=modifier)
        if response.get("result") == "failed":
            log.warning(f"Failed to send message to me because of: {message}")
        
        # 2. send message to friends
        response, message = self.send_message_to_friends(alert=alert, action=action, modifier=modifier)
        if response.get("result") == "failed":
