...
class ChannelRepository(BaseRepository):
    ...
    def insert(self, channel: Channel) -> str:
        ...
        return str(
            self._collection.insert_one(channel_dict).inserted_id
        )
        # return str(
    ...
    def find(
        ...
        *, 
        ...
        direction = pymongo.ASCENDING if sort_direction == SortDirection.ASC else pymongo.DESCENDING
        log.debug(f"sort_field: {sort_field.value}, direction: {sort_direction} ({direction})")

        cursor = self._collection.find(query).sort(sort_field.value, direction).skip(skip).limit(limit)
