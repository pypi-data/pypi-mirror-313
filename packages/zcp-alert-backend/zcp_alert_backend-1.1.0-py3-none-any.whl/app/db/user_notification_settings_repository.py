...
class UserNotificationSettingsRepository(BaseRepository):
    ...
    def insert(self, user_notification_settings: UserNotificationSettings) -> str:
        ...
        user_notification_settings_dict = user_notification_settings.model_dump(by_alias=True, exclude=['id'])
        user_notification_settings_dict.update({"created_at": user_notification_settings.created_at})
    ...
    def find(self) -> list[UserNotificationSettings]:
        ...
        return [UserNotificationSettings(**document) for document in cursor if document is not None]
    ...
    def update(self, user_notification_settings: UserNotificationSettings) -> UserNotificationSettings:
        ...
        user_notification_settings_dict = user_notification_settings.model_dump(by_alias=True, exclude=['id', 'created_at'])
        user_notification_settings_dict.update({"updated_at": user_notification_settings.updated_at})
    ...
    def find_by_id(self, user_notification_settings_id: str) -> UserNotificationSettings:
    ...
    def find_by_username(self, username: str) -> UserNotificationSettings:
        if not username:
            raise InvalidObjectIDException("UserNotificationSettings username is required")
                
        document = self._collection.find_one({"username": username})

        if document is None:
            raise ObjectNotFoundException(f"UserNotificationSettings not found: {username}")
    ...
    def delete_by_id(self, user_notification_settings_id: str) -> bool:
        ...
        if result is None:
            raise ObjectNotFoundException(f"UserNotificationSettings not found: {user_notification_settings_id}")
