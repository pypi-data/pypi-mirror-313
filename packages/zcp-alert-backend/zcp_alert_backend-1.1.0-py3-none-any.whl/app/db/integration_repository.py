...
from app.model.integration_model import Integration, IntegrationStatus, IntegrationSortField
...
class IntegrationRepository(BaseRepository):
    ...
    def update(self, integration: Integration) -> Integration:
        ...
        integration_dict = integration.model_dump(by_alias=True, exclude=['id', 'created_at'])
    ...
    def __get_aggregate_pipeline(
        ...
        *,
        ...
        query.update({'name': {'$regex': get_escaped_regex_pattern(name), '$options': 'i'}}) if name else None
        query.update({'channel.name': {'$regex': get_escaped_regex_pattern(channel_name), '$options': 'i'}}) if channel_name else None
        ...
        if sort_field is not None and sort_direction is not None:
            direction = pymongo.DESCENDING if sort_direction == SortDirection.DESC else pymongo.ASCENDING
            pipeline.append({'$sort': {self.__get_sort_field(sort_field=sort_field): direction}})
    ...
    def update_status(
        ...
        *, 
        ...
        update = {"$set": {"status": status.value, "modifier": modifier, "updated_at": datetime.now(DEFAULT_TIME_ZONE)}}
