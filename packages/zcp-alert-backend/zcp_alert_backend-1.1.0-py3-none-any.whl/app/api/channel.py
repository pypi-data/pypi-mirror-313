...
from app.model.channel_model import Channel, ChannelType, ChannelSortField
...
from app.settings import KAKAO_AUTH_ENDPOINT, KAKAO_AUTH_REDIRECT_URI, ALERT_CONSOLE_ENDPOINT
...
async def get_channels(
    ...
    types: list[ChannelType] = Query(None, title="channel type", description="Search query string for type"),
    sort_field: ChannelSortField = Query(ChannelSortField.NAME, title="sort field", description="Sort field name"),
...
async def create_channel(
    ...
    if not channel_create_request.type_properties:
        raise AlertBackendException(
            ...
            details="type_properties should not be empty")
...
async def send_test_message(
    ...
    return ResponseModel(result=Result.SUCCESS if __service.send_test_notification(channel) else Result.FAILED)
...
async def get_kakao_friends(
    channel_id: str = Query(None, title="channel_id", description="Search query string for channel_id"),
    oauth_user: TokenData = Depends(get_current_user)
