...
from app.db.user_notification_settings_repository import UserNotificationSettingsRepository
...
class AlertService:
    ...
    def __init__(self):
        ...
        self.__integration_repository = IntegrationRepository.from_config_with_aggregation(
        ...
        self.__user_notification_settings_repository = UserNotificationSettingsRepository.from_config(
    ...
    def get_alert(self, alert_id: str) -> ZcpAlert:
        ...
        try:
            ...
            raise AlertBackendException(AlertError.ID_NOT_FOUND, 
                                        document="alert",
                                        object_id=alert_id)
        except InvalidObjectIDException as e:
            raise AlertBackendException(AlertError.INVALID_OBJECTID,
                                        details=str(e))
        ...
        *, 
        ...
        for alert in alerts:
            if alert.asis_status == AlertStatus.CLOSED:
                raise AlertBackendException(
                    AlertError.BAD_REQUEST, 
                    details="Closed alert cannot be updated")
        ...
        for alert in alerts:
            activities.append(
                AlertActivity(
                    ...
                    description=[f"Status: {alert.asis_status.value} -> {tobe_status.value}", 
        ...
        zcp_alerts = self.__alert_repository.find_by_ids([alert.alert_id for alert in alerts])
    ...
    def patch_single_alert_status(
        ...
        *, 
        ...
        log.debug(f"Snoozed until at : {snoozed_until_at}, current datetime : {datetime.now(DEFAULT_TIME_ZONE) - timedelta(minutes=1)}")
        if (
            tobe_status == AlertStatus.SNOOZED
            and snoozed_until_at is not None 
            and snoozed_until_at < (datetime.now(DEFAULT_TIME_ZONE) - timedelta(minutes=1))
        ):
    ...
    def wake_up_alerts_snoozed_time_over(self, *, modifier: str) -> int:
        ...
        if len(zcp_alerts) > 0:
            alert_action_request = [AlertActionRequest(alert_id=alert.id, asis_status=alert.status) for alert in zcp_alerts]
            self.patch_bulk_alerts_status(alerts=alert_action_request, action=Action.WAKEUP, modifier=modifier)
    ...
    def remove_alert(self, alert_id:str) -> bool:
        ...
        removed_activitiy_count = self.__activity_repository.delete_all_by_alert_id(alert_id)
    ...
    def __internal_process_step3(
        ...
        *,
        ...
        for zcp_alert, matched_integrations in alert_integration_pairs:
            if action is None and zcp_alert.status in [AlertStatus.ACKED, AlertStatus.SNOOZED]:
                log.debug(f"ZcpAlert={zcp_alert.id} is ACKED or SNOOZED status, "
                ...
                activities.append(
                    AlertActivity(
                        ...
                        created_at=datetime.now(DEFAULT_TIME_ZONE), 
                        description=[f"The notification was not sent to the related channel "
    ...
    def _send_notification_to_email(
        ...
        *,
        ...
        if response == Result.FAILED:
            log.error(f"Failed to send the notification to the email channel({email.smtp_server})"
                      f" because {message}"
    ...
    def _send_notification_to_kakaotalk(
        ...
        *,
        ...
        try:
            kakaotalk_token = self.__kakaotalk_token_repository.find_by_channel_id(channel_id)
        except ObjectNotFoundException:
        ...
        if kakaotalk_token.is_expired_access_token():
            if kakaotalk_token.is_expired_refresh_token():
                log.error(f"KakaoTalk access token and refresh token are expired : {kakaotalk_token.id}")
                activities.append(
                    AlertActivity(
                        alert_id=zcp_alert.id,
                        created_at=datetime.now(DEFAULT_TIME_ZONE),
        ...
        response, message = client.send_message_to_all(alert=zcp_alert, action=action, modifier=modifier)

        if response.get("result") == "failed" and message is not None:
            log.error(f"Failed to send the notification to the kakaotalk({kakaotalk.app_name})"
                      f" of ({integration.id}:{integration.name})"
    ...
    def _refresh_kakaotalk_token(
        ...
        *,
        ...
        log.debug(f"KakaoTalk access token is expired, so refresh the token : {kakaotalk_token.id}")
    ...
    def _send_notification_go_goolechat(
        ...
        *,
        ...
        if response is None:
            log.error(f"Failed to send the notification to the google chat({google_chat.api_url})"
    ...
    def _send_notification_to_msteams(
        ...
        *,
        ...
        if response is None:
            log.error(f"Failed to send the notification to the ms teams({msteams.api_url})"
    ...
    def _send_notification_to_webhook(
        ...
        *,
        ...
        client = WebhookClient(
            webhook.webhook_url, 
            **webhook.model_dump(exclude=['webhook_url'])
        )
        ...
        if response is None:
            log.error(f"Failed to send the notification to the webhook({webhook.webhook_url})"
                      f" of ({integration.id}:{integration.name})")
    ...
    def _send_notification_to_slack(
        ...
        *,
        ...
        if response is None:
            log.error(f"Failed to send the notification to the slack({slack.api_url})"
        ...
        for zcp_alert in zcp_alerts:
            ...
            for integration in self.__get_cached_integrations().values():
                if integration.is_all_match(alert=zcp_alert, action=action):
                    ...
                    silences = self.__silence_repository.find_all_active_by_integration_id(integration.id)
                    if silences is not None and len(silences) > 0:
                        ...
                        if channel is not None and isinstance(channel, Channel):
                            log.debug(f"ZcpAlert={zcp_alert.id} is matched with {integration.name} - {channel.name}")

                            matched_integrations.append(integration)
                        else:
                            log.debug(f"ZcpAlert={zcp_alert.id} is matched with {integration.id}:{integration.name} "
                    ...
                    log.debug(f"ZcpAlert={zcp_alert.id} is not matched with integration : "
            ...
            if len(matched_integrations) == 0:
                activities.append(
                    AlertActivity(
                        ...
                        created_at=datetime.now(DEFAULT_TIME_ZONE), 
                        user=settings.ALERT_SYSTEM_USERNAME,
                        description=["Couldn't send the notification to anywhere "
    ...
    def __internal_process_step1(self, alerts: List[Alert]) -> List[ZcpAlert]:
        ...
        zcp_alerts = []
        for alert in alerts:
            exist_zcp_alert = self.__alert_repository.find_by_fingerprint(alert.fingerprint)
            ...
            if exist_zcp_alert is None:
                ...
                new_zcp_alert.created_at = new_zcp_alert.updated_at = datetime.now(DEFAULT_TIME_ZONE)
                ...
                if updated_zcp_alert.status == AlertStatus.CLOSED:
                    updated_zcp_alert.updated_at = updated_zcp_alert.closed_at = datetime.now(DEFAULT_TIME_ZONE)
                else:
                    ...
                    if (
                        ...
                        and 
                        updated_zcp_alert.status == AlertStatus.OPEN
                    ):
                        ...
                        log.debug(f"Reset the timeline properties : {updated_zcp_alert.id} "
                ...
                activity.set_data(alert_id=exist_zcp_alert.id, 
                                  description=description)
    ...
    def get_channel(self, channel_id: str) -> Channel:
        ...
        """
        if not channel_id:
            raise AlertBackendException(AlertError.BAD_REQUEST, 
    ...
    def modify_channel(self, channel: Channel) -> Channel:
        ...
        if channel.type == ChannelType.KAKAOTALK:
            ...
            if (kakaotalk_channel.auth_code != exit_kakaotalk_channel.auth_code):
                ...
                exist_token = self.__kakaotalk_token_repository.find_by_channel_id(channel.id)
    ...
    def send_test_notification(self, channel: Channel) -> bool:
        ...
        if isinstance(channel.type_properties, KakaoTalkChannel):
            raise AlertBackendException(
                AlertError.BAD_REQUEST, 
                details="KaKaoTalk channel is not supported")
            ...
            if response == Result.FAILED:
                log.error(f"Failed to send the notification to the email channel({email_channel.smtp_server})")
                raise AlertBackendException(
            ...
            if isinstance(channel.type_properties, SlackChannel):
                ...
                response, message = client.send_test_message(
                    modifier=channel.modifier
                )
                ...
                raise AlertBackendException(
                    AlertError.BAD_REQUEST, 
                    details="Invalid channel object")
            
            if response is None:
                log.error(f"Failed to send the test notification to the channel")
    ...
    def generate_kakao_token(
        ...
        *, 
        ...
        response = requests.post(f"{settings.KAKAO_AUTH_ENDPOINT}/oauth/token", data=data)
    ...
    def get_integration(self, integration_id: str) -> Integration:
        ...
        if not integration_id:
            raise AlertBackendException(
                AlertError.BAD_REQUEST, 
                details="Integration id is required")
    ...
    def __init_cached_integrations(self) -> Dict[str, Integration]:
        """ Get all active integrations
        """
        integrations = self.__integration_repository.find_all_by_status(status=IntegrationStatus.ON)
        cache_data = {}
        for integration in integrations:
            cache_data[integration.id] = integration
            log.debug(f"Integration cached[init] : {integration.id} - {integration.name}")
    ...
    def __add_integration_to_cache(self, integration_id: str):
        ...
        log.debug(f"Integration has been added in the cache : {integration.id} - {integration.name}")
    ...
    def modify_integration(self, integration: Integration) -> Integration:
        ...
        updated_integration.channel = self.__channel_repository.find_by_id(updated_integration.channel)
    ...
    def get_integration_filters(self) -> List[Filter]:
        ...
        return [
            ...
            self._get_multi_select_type_dummy_filter(key=FilterKey.SENDER, value=[s for s in Sender]),
            self._get_multi_select_type_dummy_filter(key=FilterKey.PRIORITY, value=[p for p in Priority]),
            self._get_multi_select_type_dummy_filter(key=FilterKey.SEVERITY, value=[s for s in Severity]),
            Filter(key=FilterKey.REPEATED_COUNT, 
                   value_type=FilterValueType.NUMBER, 
                   operator=[Operator.GREATER_THAN, Operator.LESS_THAN]
                ),
        ]
    
    def _get_text_type_dummy_filter(
        self,
        *,
        key: FilterKey | None
    ) -> Filter:
    ...
    def _get_multi_text_type_dummy_filter(
        self, 
        *,
        key: FilterKey | None
    ) -> Filter:
    ...
    def create_silence(self, silence: Silence) -> str:
        ...
        if integration_ids is None or len(integration_ids) == 0:
            raise AlertBackendException(AlertError.BAD_REQUEST, 
                                        details="Integrations are required")
    ...
    def remove_silence(self, silence_id: str) -> bool:
        ...
        if not silence_id:
            raise AlertBackendException(AlertError.BAD_REQUEST, 
                                        details="Silence id is required")
    ...
    def get_user_notification_settins(self, username: str) -> UserNotificationSettings:
        ...
        try:
            user_notification_settings = self.__user_notification_settings_repository.find_by_username(username)
        except ObjectNotFoundException:
    ...
    def update_user_notification_settings(
        ...
        if user_notification_settings.id is not None:
            ...
            return self.__user_notification_settings_repository.update(user_notification_settings)
        else:
            try:
                exist_unss = (
    ...
    def delete_user_notification_settings(
        self,
        ...
        bool
        """
        return self.__user_notification_settings_repository.delete_by_id(user_notification_settings_id)