...
from app.api import (
    alert, event, channel, integration, silence, report, auth, management, healthy, basic_auth_user, user_notification_settings
)
...
app.include_router(user_notification_settings.router, tags=["notification setttings"], prefix=settings.APP_ROOT)
app.include_router(management.router, tags=["management"], prefix=settings.APP_ROOT)
app.include_router(basic_auth_user.router, tags=["basic auth user"], prefix=settings.APP_ROOT)
...
start_scheduler()
log.info(f"Scheduler started with interval {settings.WAKEUP_SCHEDULER_INTERVAL} minutes")
