...
class ZcpAlert(AlertBaseModel):
    ...
    @field_serializer('acknowledged_at', 'closed_at', 'snoozed_until_at', 'starts_at', 'ends_at')
    ...
    def from_alert(cls, *, alert: Alert):
        ...
        return cls(status=AlertStatus.OPEN if alert.status == Status.FIRING else AlertStatus.CLOSED,
                   labels=alert.labels,
    ...
    def diff(self, *, before:AlertBaseModel) -> List[str]:
        ...
        if before_dict['status'] != self.status:
            difference.append(f"status: {before_dict['status'].value} -> {self.status.value}")

        if before_dict['repeated_count'] != self.repeated_count:
            difference.append(f"repeated count: {before_dict['repeated_count']} -> {self.repeated_count}")
