...
from app.model.alert_model import AlertBaseModel, ZcpAlert, Action
...
class Filter(BaseModel):
    ...
    def is_multiple_value(self) -> bool:
        return self.value_type in [FilterValueType.MULTI_TEXT, FilterValueType.MULTI_SELECT]
    ...
    def assign_type_properties(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if value_type not in [FilterValueType.MULTI_TEXT, FilterValueType.MULTI_SELECT]:
            if isinstance(s_value, list):
                raise ValueError(f"Value should not be a single value when the value_type is {value_type}")
            ...
            if s_value is not None and not isinstance(s_value, list):
                raise ValueError(f"Value should be a list when the value_type is {value_type}")
            if isinstance(operator, Operator) and operator != Operator.CONTAINS:
                raise ValueError(f"Operator should not be a list when the value_type is {value_type}")
    ...
    def _is_match(self, zcp_alert: ZcpAlert) -> bool:
        ...
        elif self.key in [FilterKey.PROJECT, FilterKey.PRIORITY, FilterKey.SEVERITY, FilterKey.CLUSTER, FilterKey.NAMESPACE]:
...
class Integration(AlertBaseModel):
    ...
    """
    name: str = None
    channel: Union[Optional[str], Optional[Channel]] = None # Don't change it. Should be set to None by default because of the the silence list
    """ Channel ID
