...
from app.model.alert_model import AlertBaseModel, AlertStatus, Priority, ZcpAlert
...
class UserNotificationSettings(AlertBaseModel):
    ...
    def is_match(self, zcp_alert: ZcpAlert) -> bool:
        ...
        if self.clusters and zcp_alert.labels.get('cluster', '') not in self.clusters:
            return False
        if self.priorities and zcp_alert.labels.get('priority', '') not in self.priorities:
            return False
        if self.labels:
