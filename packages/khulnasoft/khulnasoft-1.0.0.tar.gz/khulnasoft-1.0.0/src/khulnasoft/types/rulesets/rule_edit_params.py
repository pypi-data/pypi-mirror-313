...
class SetCacheSettingsRuleActionParametersCacheReserve(TypedDict, total=False):
    ...
    If this is true and a request meets eligibility criteria, Khulnasoft will write
...
class SetCacheSettingsRuleActionParametersServeStale(TypedDict, total=False):
    ...
    """Defines whether Khulnasoft should serve stale content while updating.

    If true, Khulnasoft will not serve stale content while getting the latest
...
class SetCacheSettingsRuleActionParameters(TypedDict, total=False):
    ...
    Khulnasoft cache purge will not purge content cached on client browsers, so high
    ...
    """Generate Khulnasoft error pages from issues sent from the origin server.
