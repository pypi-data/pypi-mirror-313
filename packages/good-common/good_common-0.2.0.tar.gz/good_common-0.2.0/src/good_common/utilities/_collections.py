...
def object_farmhash(obj: BaseModel | typing.Any, exclude_keys: IncEx = None) -> int:
    ...
    def serialize(data: typing.Any) -> typing.Any:
        if isinstance(data, BaseModel):
            ...
            # return farmhash.hash64(orjson.dumps(sort_object_keys(data)))
            return {k: serialize(v) for k, v in sort_object_keys(data).items() if v}
        elif isinstance(data, list):
            return [serialize(item) for item in sort_if_sortable(data)]
        elif isinstance(data, tuple):
            return tuple(serialize(item) for item in data)
        elif isinstance(data, set):
            return frozenset(serialize(item) for item in sort_if_sortable(data))
