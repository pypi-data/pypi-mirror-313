...
class ProcessingSubheader(object):
    ...
    COLUMN_FORMAT_WIDTH_OFFSET_OFFSET = 0
    COLUMN_FORMAT_WIDTH_OFFSET_LENGTH = 2
    COLUMN_FORMAT_PRECISION_OFFSET_OFFSET = 2
    COLUMN_FORMAT_PRECISION_OFFSET_LENGTH = 2
    COLUMN_FORMAT_OFFSET_OFFSET = 24
    COLUMN_FORMAT_OFFSET_LENGTH = 2
    COLUMN_FORMAT_LENGTH_OFFSET = 26
    COLUMN_FORMAT_LENGTH_LENGTH = 2

    COLUMN_INFORMAT_TEXT_SUBHEADER_INDEX_OFFSET = 16
    COLUMN_INFORMAT_WIDTH_OFFSET_OFFSET = 4
    COLUMN_INFORMAT_PRECISION_OFFSET_OFFSET = 6
    COLUMN_INFORMAT_OFFSET_OFFSET = 18
    COLUMN_INFORMAT_LENGTH_OFFSET = 20
    COLUMN_INFORMAT_TEXT_SUBHEADER_INDEX_LENGTH = 2
    COLUMN_INFORMAT_WIDTH_OFFSET_LENGTH = 2
    COLUMN_INFORMAT_PRECISION_OFFSET_LENGTH = 2
    COLUMN_INFORMAT_OFFSET_LENGTH = 2
    COLUMN_INFORMAT_LENGTH_LENGTH = 2
...
class FormatAndLabelSubheader(ProcessingSubheader):
    def process_subheader(self, offset, length):
        ...
        col_format_width = (
            offset + self.COLUMN_FORMAT_WIDTH_OFFSET_OFFSET + 3 * int_len
        )
        col_format_precision = (
            offset + self.COLUMN_FORMAT_PRECISION_OFFSET_OFFSET + 3 * int_len
        )
        col_format_offset = (
        ...
        text_subheader_informat = (
                offset + self.COLUMN_INFORMAT_TEXT_SUBHEADER_INDEX_OFFSET + 3 *
                int_len
        )
        col_informat_width = (
                offset + self.COLUMN_INFORMAT_WIDTH_OFFSET_OFFSET + 3 * int_len
        )
        col_informat_precision = (
                offset + self.COLUMN_INFORMAT_PRECISION_OFFSET_OFFSET + 3 * int_len
        )
        col_informat_offset = (
                offset + self.COLUMN_INFORMAT_OFFSET_OFFSET + 3 * int_len
        )
        col_informat_len = (
                offset + self.COLUMN_INFORMAT_LENGTH_OFFSET + 3 * int_len
        )
        ...
        vals = self.parent._read_bytes({
            ...
            col_format_width: self.COLUMN_FORMAT_WIDTH_OFFSET_LENGTH,
            col_format_precision:self.COLUMN_FORMAT_PRECISION_OFFSET_LENGTH,
            ...
            text_subheader_informat:
                self.COLUMN_INFORMAT_TEXT_SUBHEADER_INDEX_LENGTH,
            col_informat_width: self.COLUMN_INFORMAT_WIDTH_OFFSET_LENGTH,
            col_informat_precision: self.COLUMN_INFORMAT_PRECISION_OFFSET_LENGTH,
            col_informat_offset: self.COLUMN_INFORMAT_OFFSET_LENGTH,
            col_informat_len: self.COLUMN_INFORMAT_LENGTH_LENGTH,
        })
        ...
        format_width = self.parent._read_val(
            'h', vals[col_format_width],
            self.COLUMN_FORMAT_WIDTH_OFFSET_LENGTH
        )
        format_precision = self.parent._read_val(
            'h', vals[col_format_precision],
            self.COLUMN_FORMAT_PRECISION_OFFSET_LENGTH
        )
        ...
        informat_idx = min(
            self.parent._read_val(
                'h', vals[text_subheader_informat],
                self.COLUMN_INFORMAT_TEXT_SUBHEADER_INDEX_LENGTH
            ),
            len(self.parent.column_names_strings) - 1
        )
        informat_width = self.parent._read_val(
            'h', vals[col_informat_width],
            self.COLUMN_INFORMAT_WIDTH_OFFSET_LENGTH
        )
        informat_precision = self.parent._read_val(
            'h', vals[col_informat_precision],
            self.COLUMN_INFORMAT_PRECISION_OFFSET_LENGTH
        )
        informat_start = self.parent._read_val(
            'h', vals[col_informat_offset],
            self.COLUMN_INFORMAT_OFFSET_LENGTH
        )
        informat_len = self.parent._read_val(
            'h', vals[col_informat_len],
            self.COLUMN_INFORMAT_LENGTH_LENGTH
        )
        ...
        informat_names = self.parent.column_names_strings[informat_idx]
        column_informat = informat_names[informat_start:informat_start + informat_len]
        current_column_number = len(self.parent.columns)
        column = Column(current_column_number, self.parent.column_names[current_column_number], column_label,
                        column_format, self.parent.column_types[current_column_number],
                        self.parent.column_data_lengths[current_column_number])
        column.format_width = format_width
        column.format_precision = format_precision

        column.informat = column_informat.decode("utf-8")
        column.informat_width = informat_width
        column.informat_precision = informat_precision
        self.parent.columns.append(column )
