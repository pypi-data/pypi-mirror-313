...
class FuncOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, function_type, *, sym_visibility=None, arg_attrs=None, res_attrs=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_660')) else
      _ods_ir.AttrBuilder.get('anonymous_660')(function_type, context=_ods_context))
...
class DotScaledOp(_ods_ir.OpView):
  ...
  def __init__(self, d, lhs, rhs, c, lhs_scale, lhs_type, rhs_type, *, rhs_scale=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    ...
    operands.append(_get_op_result_or_value(rhs))
    operands.append(_get_op_result_or_value(c))
    operands.append(_get_op_result_or_value(lhs_scale))
    if rhs_scale is not None: operands.append(_get_op_result_or_value(rhs_scale))
    _ods_context = _ods_get_default_loc_context(loc)
...
def dot_scaled(d, lhs, rhs, c, lhs_scale, lhs_type, rhs_type, *, rhs_scale=None, loc=None, ip=None) -> _ods_ir.Value:
  return DotScaledOp(d=d, lhs=lhs, rhs=rhs, c=c, lhs_scale=lhs_scale, lhs_type=lhs_type, rhs_type=rhs_type, rhs_scale=rhs_scale, loc=loc, ip=ip).result
...
def experimental_descriptor_load(result, desc_ptr, indices, *, cache=None, evict=None, loc=None, ip=None) -> _ods_ir.Value:
  return ExperimentalDescriptorLoadOp(result=result, desc_ptr=desc_ptr, indices=indices, cache=cache, evict=evict, loc=loc, ip=ip).result
...
def experimental_descriptor_store(desc_ptr, src, indices, *, loc=None, ip=None) -> _ods_ir.Operation:
  return ExperimentalDescriptorStoreOp(desc_ptr=desc_ptr, src=src, indices=indices, loc=loc, ip=ip)
...
class MakeTensorDescOp(_ods_ir.OpView):
  ...
  def __init__(self, result, base, shape, strides, tensorShape, *, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    ...
    _ods_context = _ods_get_default_loc_context(loc)
    attributes["tensorShape"] = (tensorShape if (
    isinstance(tensorShape, _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('DenseI32ArrayAttr')) else
      _ods_ir.AttrBuilder.get('DenseI32ArrayAttr')(tensorShape, context=_ods_context))
    results.append(result)
    _ods_successors = None
    super().__init__(self.build_generic(attributes=attributes, results=results, operands=operands, successors=_ods_successors, regions=regions, loc=loc, ip=ip))
  ...
  def tensorShape(self):
    return self.operation.attributes["tensorShape"]

  @tensorShape.setter
  def tensorShape(self, value):
    if value is None:
      raise ValueError("'None' not allowed as value for mandatory attributes")
    self.operation.attributes["tensorShape"] = value

  @builtins.property
  def result(self):
    return self.operation.results[0]

def make_tensor_descriptor(result, base, shape, strides, tensor_shape, *, loc=None, ip=None) -> _ods_ir.Value:
  return MakeTensorDescOp(result=result, base=base, shape=shape, strides=strides, tensorShape=tensor_shape, loc=loc, ip=ip).result
