    ...
    async def wrapper(*args, **kwargs):
        ...
        for i in range(1, 4):
            ...
            if add_url:
                ...
                # Check if new_param matches any existing path parameter
                for param_name, enum_values in path_params.items():
                    if new_param in enum_values:
                        # Проверяем, если параметр уже добавлен
                        if f'/{param_name}' not in endpoint:
                            endpoint += f'/{{{param_name}}}'  # Добавляем параметр к endpoint
                        matched = True
                        break

                # Если new_param не соответствует существующим параметрам
                if not matched:
                    if len(path_params) == 1:
                        # Если только один параметр пути, добавляем его
                        for param_name in path_params.keys():
                            if f'/{param_name}' not in endpoint:  # Проверяем, есть ли он уже
                                endpoint += f'/{{{param_name}}}'
                    else:
                        for param_name in path_params.keys():
                            if param_name not in endpoint:
                                endpoint += f'/{{{param_name}}}'  # Добавляем не совпадающие параметры
                    break
