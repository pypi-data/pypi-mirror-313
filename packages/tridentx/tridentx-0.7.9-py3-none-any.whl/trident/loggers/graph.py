...
def depth_first_search(layer, visitor, depth=0):
    '''
    ...
    Args:
        layer (:class:`~trident.backend.pytorch_backend.Layer` or :class:`~trident.backend.tensorflow_backend.Layer`): the root module to start the journey from
    ...
    stack = [(layer.get_root(), depth)]  # node
...
def plot(layer, filename=None):
    ...
    Args:
        layer (graph node): the node to start the journey from
    ...
    def shape_desc(node):
        ...
        static_shape = str(node.output_shape)
        return '"#dyn: %i\nstatic: %s"' % (0, static_shape)
