...
from sisc.alignment.XmlAligner import XmlAligner
from sisc.eval import Evaluation
from sisc.fingerprint.TextFingerprinter import TextFingerprinter
from sisc.fingerprint.XmlFingerprinter import XmlFingerprinter
...
def __run_eval(eval_type: str, file_1_path: str, file_2_path: str, include_tags: List[str], exclude_tags: List[str]):
    ...
    if isfile(file_1_path) and isfile(file_2_path):
        ...
        # print(f'Average: {average_levenshtein}')
...
def __align_file(input_path: str, fingerprint_path: str, annotation_path: str, output_path: str, key_to_update,
    ...
    if fingerprint_ext == '.txt':
        ...
        input_content = ElementTree.parse(annotation_path)
        aligner = XmlAligner()
    ...
    if not al1:
        logging.warning(f'Could not align {filename}')
        return
...
def __fingerprint_xml(text_file_path: str, output_path: str, fingerprint_type: str, symbol: str, move_notes: bool,
                      add_quotation_marks: bool, distance: int, context_size: int, tag: str, keep_text: bool):
    filename = splitext(basename(text_file_path))[0]
    element_tree = ElementTree.parse(text_file_path)

    if fingerprint_type == 'uniform':
        obfuscater = UniformObfuscater(symbol=symbol, distance=distance)
    ...
    fingerprinter = XmlFingerprinter(obfuscater, move_notes=move_notes, add_quotation_marks=add_quotation_marks,
                                     keep_tag=tag)
...
def __fingerprint_txt(text_file_path: str, output_path: str, fingerprint_type: str, symbol:str, distance: int) -> None:
    filename = splitext(basename(text_file_path))[0]
    ...
    if fingerprint_type == 'uniform':
        ...
        logging.fatal(f'Fingerprint type {fingerprint_type} is not supported.')
        return
    else:
        logging.fatal(f'Unknown fingerprint type: {fingerprint_type}')
        return
...
def main():
    ...
    parse_align.add_argument('--annotation-type', choices=['txt', 'json', 'xml'], dest='annotation_type',
    ...
    parse_eval.add_argument('type', nargs=1, choices=['txt', 'json', 'xml'], metavar='type',
                                default='xml', help='TBD')
    parse_eval.add_argument('input_file_paths', nargs=2, metavar='input_file_paths')
    parse_eval.add_argument('-i', '--include-tags', nargs='+', type=str, dest='include_tags', help='TBD',
                            required=False)
    parse_eval.add_argument('-e', '--exclude-tags', nargs='+', type=str, dest='exclude_tags', help='TBD',
                            required=False)
    ...
    if args.top_subparser == 'fingerprint':
        if args.fingerprint_subparser == 'uniform' or args.fingerprint_subparser == 'context':
            ...
            if isfile(input_path):
                ...
                if input_file_ext == '.txt':
                    __fingerprint_txt(input_path, output_path, fingerprint_type, symbol, distance)
                elif input_file_ext == '.xml':
                    __fingerprint_xml(input_path, output_path, fingerprint_type, symbol, move_notes,
                                      add_quotation_marks, distance, context_size, tag, keep_text)
                else:
                ...
                for file_or_folder in listdir(input_path):
                    ...
                    if isfile(full_path):
                        ...
                        if input_file_ext == '.txt' and file_type == 'txt':
                            __fingerprint_txt(full_path, output_path, fingerprint_type, symbol, distance)
        ...
        if isfile(content_path):
            ...
            for file_or_folder in listdir(content_path):
                ...
                if isfile(full_path):
                    ...
                    fingerprint_file_path = None
                    if fingerprint_path:
                        fingerprint_file_path = join(fingerprint_path, f'{filename}_fingerprint.txt')

                    if annotation_type == 'xml':
                        annotation_file_path = join(annotation_path, f'{filename}_fingerprint.xml')
