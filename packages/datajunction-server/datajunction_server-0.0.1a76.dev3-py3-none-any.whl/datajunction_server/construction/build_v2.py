    ...
    for parent_node, children in common_parents.items():  # type: ignore
        ...
        final_query = ast.Query(
            ...
            select=ast.Select(
                ...
                from_=ast.From(relations=[ast.Relation(primary=ast.Table(ast.Name(amenable_name(parent_node.name))))]),
        ...
        for metric in children:
            for measure in m2m[metric.name][0]:
                ...
                temp_select = cached_parse(f'SELECT {measure.aggregation}({measure.expression}) AS {measure.name}').select
                for col in temp_select.find_all(ast.Column):
                    if col.alias_or_name.name in parent_ast.select.column_mapping:
                        col.add_type(parent_ast.select.column_mapping.get(col.alias_or_name.name).type)
...
class QueryBuilder:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    async def build(self) -> ast.Query:
        ...
        if self.physical_table and not self._filters and not self.dimensions and not self._measures:
