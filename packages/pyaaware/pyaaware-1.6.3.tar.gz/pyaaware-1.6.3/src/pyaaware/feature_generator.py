...
class FeatureGenerator:
    def __init__(self, feature_mode: str, truth_parameters: dict[str, int | None] | None = None) -> None:
        ...
        self._total_truth_parameters = sum([x for x in self._truth_parameters.values() if x is not None])
    ...
    def execute_all(
        self, xf: np.ndarray, truth_in: dict[str, Any] | None = None
        ...
        if truth_in is not None:
            ...
            for name, value in self._truth_parameters.items():
                if value is not None:
                    if truth_in[name].ndim != 2:
                        raise ValueError(f"truth_in, '{name}', must be an 2-dimensional array")
                    if truth_in[name].shape[0] != input_frames:
                        raise ValueError(f"truth_in, '{name}', must have {input_frames} frames")
                    if truth_in[name].shape[1] != value:
                        raise ValueError(f"truth_in, '{name}', must have {value} parameters")

            flat_truth = np.concatenate(
                tuple(truth_in[name] for name, value in self._truth_parameters.items() if value is not None), axis=1
            )
        else:
            flat_truth = None
        ...
        if truth_in is not None:
            ...
            for name, value in self._truth_parameters.items():
                if value is not None:
                    truth_out[name] = truth[:, :, start_index : start_index + value]
                    start_index += value
                else:
                    truth_out[name] = truth_in[name]
