...
Given parameters $\theta = (\theta_1, \theta_2, \ldots, \theta_k)$,
we want to sample from the joint posterior $p(\theta \mid y)$. Gibbs sampling iterates
...
TensorFlow Probability (TFP) offers powerful abstractions for probabilistic modeling and inference.
...
Below is a refactored CausalImpact code example with docstrings suitable for mkdocs,
