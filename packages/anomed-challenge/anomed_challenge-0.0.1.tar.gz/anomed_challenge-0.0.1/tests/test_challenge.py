...
from anomed_challenge import challenge
from anomed_challenge.challenge import NumpyDataset
...
def example_dataset() -> NumpyDataset:
    return challenge.InMemoryNumpyArrays(np.arange(10), np.arange(10))
...
def test_NpzFromDisk(empty_ndarray, ten_elem_ndarray):
    with tempfile.NamedTemporaryFile(mode="w+b") as tmp_file:
        ...
        dataset = challenge.NpzFromDisk(tmp_file.name)
...
def test_InMemoryNumpyArrays(empty_ndarray, ten_elem_ndarray):
    dataset = challenge.InMemoryNumpyArrays(empty_ndarray, empty_ndarray)
    assert _compare_dataset_components(empty_ndarray, empty_ndarray, dataset)
    dataset = challenge.InMemoryNumpyArrays(ten_elem_ndarray, ten_elem_ndarray)
    assert _compare_dataset_components(ten_elem_ndarray, ten_elem_ndarray, dataset)
...
def _join_datasets(
    left: NumpyDataset, right: NumpyDataset
) -> challenge.InMemoryNumpyArrays:
    (X_left, y_left) = left.get()
    (X_right, y_right) = right.get()
    return challenge.InMemoryNumpyArrays(
...
def _is_subset(subset: np.ndarray, superset: np.ndarray) -> bool:
    return all([e in subset for e in subset])
