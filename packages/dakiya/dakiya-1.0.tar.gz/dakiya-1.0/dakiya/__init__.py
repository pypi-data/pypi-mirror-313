...
class Transmitter:
    ...
    def send_x(self, what, template, *k, **kw):
        ...
        if "attachments" in serialized_kw:
            all_attachments = serialized_kw["attachments"]
            ...
            serialized_kw["attachments"] = encoded_attachements

        res = None

        # Now, serialize kw before sending
        payload = json.dumps(serialized_kw, default=serialize)
        if os.getenv("MODE", "") == "DEVELOPMENT":
