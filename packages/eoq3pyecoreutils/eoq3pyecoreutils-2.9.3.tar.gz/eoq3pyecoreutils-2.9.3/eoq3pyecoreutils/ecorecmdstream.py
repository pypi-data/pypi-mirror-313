...
class EObjectInStream(CmdStream):
            ...
            if(EClass == type(c)):
                ...
                for f in c.eStructuralFeatures:
                    ...
                    if(EAttribute == type(f)):
                        ...
                        try:
                            ...
                            self.config.self.config.logger.Warn('Skipped: %s.%s because type %s is not supported or disabled'%(c.name,f.name,eType))
                            continue
                ...
                for f in c.eStructuralFeatures:
                    ...
                    if((EReference == type(f)) and not f.containment):
                        ...
                        if(EObject.eClass==eType): #eClass seems necessary, since EObject is a build in class
                            ...
                            self.config.self.config.logger.Warn('Skipped: %s.%s because type is not set.'%(c.name,f.name))
                            continue
                        else:
                            self.config.self.config.logger.Warn('Skipped: %s.%s because type %s is not supported.'%(c.name,f.name,eType))
                            continue
                        dstMul = f.upperBound
                    ...
                    if((EReference == type(f)) and f.containment):
                        ...
                        if(EObject.eClass==eType):
                            ...
                            try: 
                                ...
                                self.config.self.config.logger.Warn('Skipped %s.%s because type cannot be resolved. Are subpackages disabled?'%(c.name,f.name))
                                continue
                        elif(None==eType):
                            self.config.self.config.logger.Warn('Skipped %s.%s because type is not set.'%(c.name,f.name))
                            continue
                ...
                for s in c.eSuperTypes:
                    ...
                    try:
                        ...
                        self.config.self.config.logger.Warn('Skipped %s -[inherits]-> %s because %s can not be resolved. Are subpackages disabled?'%(c.name,s.name,s.name))
                        continue
    ...
    def LoadUserModelEObjectReferencesOnly(self, eObj:EObject)->None:
        ...
        try:
            ...
            self.config.self.config.logger.Warn("Skipped: Unknown object %s."%(str(eObj)))   
            return
                ...
                if(f.many):
                    ...
                    for v in eValue:
                        ...
                        try:
                            ...
                            self.config.self.config.logger.Warn("Skipped: Found unresolvable proxy in <%s>.%s:%d."%(eClass.name,f.name,i))   
                        i+=1 
                    ...
                    try:
                        ...
                        self.config.self.config.logger.Warn("Skipped: Found unresolvable proxy in <%s>.%s."%(eClass.name,f.name))   
        ...
        for f in eClass.eAllStructuralFeatures():
            ...
            if EAttribute == type(f):
                if(IsEPrimitiveType(eType) or isinstance(eType,EEnum)):
                    ...
                    self.config.self.config.logger.Warn('Skipped %s.%s because type %s is not supported.'%(eObj,f.name,eType.name))
            elif(EReference == type(f) and f.containment):
                eValue = eObj.eGet(f)
                if(f.many):
                    for v in eValue:
    ...
    def __OnProxyResolveFailed(self, p:EProxy, e:Exception):
        self.config.self.config.logger.Warn("Unresolvable proxy found and removed: %s"%(e))
