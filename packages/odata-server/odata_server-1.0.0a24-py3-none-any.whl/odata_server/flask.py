...
def get(mongo: pymongo.database.Database, RootEntitySet, subject, id_value, prefers, session=None):
    ...
    if prefix == "":
        ...
        results = tuple(mongo_collection.aggregate(pipeline, session=session, maxTimeMs=settings.MONGO_SEARCH_MAX_TIME_MS))
        if len(results) == 0:
            abort(404)
        data = results[0]
