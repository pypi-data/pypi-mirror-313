...
def test_pull_condition_as_capsules():
    ...
    # If there has been some GC, retry the test and abort this run
    while True:
        pull_df = spy.pull(push_df.iloc[0], start='2019-01-01T00:00:00.000Z', end='2019-06-01T00:00:00.000Z',
                           session=session)
        if (pull_df.spy.status.df.iloc[0]['Data Processed'].
                request_timings.garbage_collection_duration.total_seconds() == 0):
            break
        else:
            print(
                "CRAB-42830: GC was detected, so this test will be retried. There is no maximum number of retries. If "
                "you see this, please comment on CRAB-42830.")
