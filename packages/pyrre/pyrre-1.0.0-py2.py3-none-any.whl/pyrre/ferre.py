...
def fit(slist,inter=3,algor=1,init=1,indini=None,nruns=1,
    ...
    cont : int, optional
       Continuum parameter:
         ...
         2   segmented normalization  (ncont segments)
...
def cfit(slist,vrel,cont=1,ncont=0,loggrelation=False,grid='jwstgiant5b.dat',
    ...
    for i in range(nspectra):
        ...
        try:
            ...
            if verbose:
                ...
                printvals = []
                for k in range(fr.nlabels):
                    printvals += [fullpars[k],fullperror[k]]
                fmt = 'Best parameters: '+', '.join(fr.nlabels*['{:f}+/-{:.3g}'])
                print(fmt.format(*printvals))
                print('Chisq: ',chisq)
                
            # Construct the output dictionary
            out1 = {'index':i,'vrel':vrel1,'snr':spec.snr,'pars':fullpars,'perror':fullperror,
                    'wave':pspec['wave'],'flux':pspec['flux'],'err':pspec['err'],
                    'model':bestmodel,'chisq':chisq,'loggrelation':loggrelation,'success':True}
            success = True
        except:
            traceback.print_exc()
            success = False
    ...
    # Create output table
    dt = [('index',int),('vrel',float),('snr',float),('pars',float,fr.nlabels),
          ('perror',float,fr.nlabels),('chisq',float),('success',bool)]
