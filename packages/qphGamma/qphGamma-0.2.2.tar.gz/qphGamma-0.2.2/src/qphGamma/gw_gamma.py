...
def get_vasp_dos(Te, N, E, dos_file, core_E):
    # gets tet. method DOS
    ...
    if dos_file != None:
        ...
        print("\n\nNo VASP DOS .h5 file included in GWAvgGamma!!\n\n")
        return [0] * len(E)


# get FG dos and then interpolates for data matching
# The chemical potential should always be 0 eV for the scattering rates that are going to be used with this dos
def get_fg_dos(Te, N, V, E, core_E):
    check_core_E_contribution(Te, core_E)
...
def get_avg_gamma(E, gamma, occ, dos):
    return integrate.simpson(gamma * occ * dos, E)
...
class GWGamma:
    ...
    def __init__(self, file, core_E, dos_file=None) -> None:
        ...
        self.E_mu = E_mu
        self.occ = occ
        self.qp_gamma = gamma * (1 - occ)
        self.qh_gamma = gamma * occ
        self.total_gamma = gamma
        self.vasp_dos = get_vasp_dos(params.Te, params.N, E_mu, dos_file, core_E)
        self.fg_dos = get_fg_dos(params.Te, params.N, params.volume, E_mu, core_E)
        self.avg_vasp_gamma = get_avg_gamma(E_mu, gamma * (1 - occ), occ, self.vasp_dos)
        self.qh_avg_vasp_gamma = get_avg_gamma(
            E_mu, gamma * occ, (1 - occ), self.vasp_dos
        )
        self.avg_fg_gamma = get_avg_gamma(E_mu, gamma * (1 - occ), occ, self.fg_dos)
        self.qh_avg_fg_gamma = get_avg_gamma(E_mu, gamma * occ, (1 - occ), self.fg_dos)
