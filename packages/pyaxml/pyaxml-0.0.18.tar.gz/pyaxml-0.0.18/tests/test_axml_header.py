...
import ctypes
from pyaxml.header import TypedValue, AXMLHeader, AXML_HEADER_SIZE, AXMLHeaderXML, AXML_RES_TABLE_HEADER_SIZE, AXMLHeaderRESTABLE
...
class TestTypedValue(unittest.TestCase):
    def test_complex_to_float(self):
        ...
        expected_float= 6144
        self.assertAlmostEqual(TypedValue.complex_to_float(complex_value), expected_float)
    ...
    def test_coerce_to_string(self):
        ...
        self.assertEqual(TypedValue.coerce_to_string(TypedValue.TYPE_REFERENCE, 0x1234), "@1234")
        self.assertEqual(TypedValue.coerce_to_string(TypedValue.TYPE_ATTRIBUTE, 0x5678), "?5678")
        self.assertEqual(TypedValue.coerce_to_string(TypedValue.TYPE_INT_HEX, 0xABCDEF), "0xabcdef")
        self.assertEqual(TypedValue.coerce_to_string(TypedValue.TYPE_INT_BOOLEAN, 1), "true")
        self.assertEqual(TypedValue.coerce_to_string(TypedValue.TYPE_INT_BOOLEAN, 0), "false")
...
class TestAXMLHeaderRESTABLE(unittest.TestCase):
    def test_pack(self):
        ...
        # Verify the packed data
        expected_data = pack("<HHL", 0x0002, AXML_RES_TABLE_HEADER_SIZE, 64) + pack("<L", 5)
        self.assertEqual(packed_data, expected_data)

    def test_from_axml(self):
        # Create a packed buffer
        packed_data = pack("<HHL", 0x0002, AXML_RES_TABLE_HEADER_SIZE, 64) + pack("<L", 5) + b"extra_data"
        # Parse the buffer
        ...
        self.assertEqual(header_restable.proto.hnd.header_size, AXML_RES_TABLE_HEADER_SIZE)
