    ...
    async def on_event(e):
        if e.sessionID in conn.sessions:
            ...
            if e.eventTarget == "page" and e.eventName == "close":
                logger.info(f"Session marked for closure: {e.sessionID}")
                # Guarda los valores actuales de eventTarget y eventName
                original_event_target = e.eventTarget
                original_event_name = e.eventName

                # Espera 60 segundos antes de continuar
                await asyncio.sleep(60)

                # Verifica si la sesión aún existe
                if e.sessionID in conn.sessions:
                    # Revalida los valores de eventTarget y eventName
                    current_session = conn.sessions[e.sessionID]
                    current_event_target = e.eventTarget 
                    current_event_name = e.eventName 

                    if current_event_target == original_event_target and current_event_name == original_event_name:
                        logger.info(f"Session closed after delay: {e.sessionID}")
                        conn.sessions.pop(e.sessionID, None)
                        current_session._close()
                        del current_session
                    else:
                        logger.info(f"Session state changed, closure canceled: {e.sessionID}")
                else:
                    logger.info(f"Session already removed: {e.sessionID}")
