...
def read_ffs_file(libfile, read='active', returnObj='G'):
    ...
    if Nfiles < 1 or Nfiles <= filenr:
        return None
    file = im.ffsList[filenr]
    if returnObj=='file':
        return file
    
    # get active analysis
    Nanal = file.nrAnalyses
    if readlist:
        analnr = read[2]
    else:
        analnr = file.activeAnalysis
    if Nanal < 1 or Nanal <= analnr:
        return None
    analysis = file.analysisList[analnr]
    ...
    # get active fit
    Nfits = analysis.nrFits
    if readlist:
        fitnr = read[3]
    else:
        fitnr = analysis.activeFit
...
def read_g_from_ffs(libfile, read='active'):
    ...
    Gsingle = analysis.getCorr(elements[0])
    N = len(Gsingle)
    ...
    for i in range(Ncurves):
        Gsingle = analysis.getCorr(elements[i])
        tau = analysis.getCorr()[:,0]
        ...
        if len(analysis.fits) > 0 and analysis.activeFit is not None:
            ...
            fitrange = fits.fitAllCurves[0].fitrange
            start = fitrange[0]
            stop = fitrange[1]
            ...
            fitres = fits.fitAllCurves[i].fitresult
