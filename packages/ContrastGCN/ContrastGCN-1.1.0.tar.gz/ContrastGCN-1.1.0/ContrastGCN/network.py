...
def cl_train(X_train, 
    ...
    for epoch in tqdm(range(num_epochs), desc=f"Phase 1: contrastive learning"):
        model.train()
        running_loss = 0.0
...
def predict(train_adata, 
    ...
    for epoch in tqdm(range(num_epochs), desc=f"Phase 2: GCN"):
        gcn.train()
        optimizer.zero_grad()
    ...
    annotations_df = pd.DataFrame(annotations, columns=['Annotations of ContrastGCN'])
