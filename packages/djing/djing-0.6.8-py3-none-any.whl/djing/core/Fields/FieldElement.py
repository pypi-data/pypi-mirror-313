...
class FieldElement(Element):
    ...
    def hide_from_index(self, callback=True) -> Self:
        if callable(callback):
            self._show_on_index = lambda *args, **kwargs: not callback(*args, **kwargs)
        else:
    ...
    def hide_from_detail(self, callback=True) -> Self:
        if callable(callback):
            self._show_on_detail = lambda *args, **kwargs: not callback(*args, **kwargs)
        else:
    ...
    def hide_when_creating(self, callback=True) -> Self:
        if callable(callback):
            self._show_on_creation = lambda *args, **kwargs: not callback(
                *args, **kwargs
            )
        else:
    ...
    def hide_when_updating(self, callback=True) -> Self:
        if callable(callback):
            self._show_on_update = lambda *args, **kwargs: not callback(*args, **kwargs)
        else:
