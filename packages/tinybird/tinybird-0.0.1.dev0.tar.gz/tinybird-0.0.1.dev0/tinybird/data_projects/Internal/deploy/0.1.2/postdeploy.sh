...
sleep 30
tb --semver ${VERSION} datasource copy pipe_stats_rt --sql "select * from live.pipe_stats_rt where start_datetime >= now() - interval 7 day and start_datetime <= '$BACKFILL_VALUE'" --wait
