...
namespace BT{
	...
	tuple < std::vector<D>, int > 
		Simplex(OP f,										// target function
		std::vector<D> init,								// initial guess of the parameters
		D tol = 1e-9,										// termination criteria 1
		D ftol = 1e-9,										// termination criteria 2 (suggested by QJ and David)
		std::vector<std::vector<D> > x = std::vector<std::vector<D> >(),
		//x: The Simplex
		int iterations = 1E5){    //iteration step number

		//*** This is the previous termination criteria
		// D tol = 1E8*std::numeric_limits<D>::epsilon();
		// std::numeric_limits<D>::epsilon() is the difference between 1 and the smallest value greater than 1
		// for double objects is : 2.22045e-016

		// One setting
		// D tol = 1E6*std::numeric_limits<D>::epsilon(),		// termination criteria 1
		// D ftol = 1e-8,		// termination criteria 2 (suggested by QJ and David)
		...
		double f_val_old = -1e10;
		double f_val_new = 1e10;
		...
		for (cnt = 0; cnt<iterations; ++cnt)
		{
			...
			// The average of objective function values is saved and then applied to judge convergence
			f_val_old = f_val_new;
			f_val_new = 0.0;
			for (int i = 0; i<N + 1; ++i)
			{
				f_val_new = f_val_new + vf[i];
			}
			f_val_new = f_val_new / double(N);
			...
			D diff1 = 0.0;        //calculate the difference of the simplex centers
			//see if the difference is less than the termination criteria
			for (int i = 0; i<N; ++i)
				diff1 += fabs(xcentroid_old[i] - xcentroid_new[i]);

			//termination condition 2
			D diff2 = 0.0;		//calculate the difference of the objective values at the simplex centers
			//see if the difference is less than the termination criteria
			diff2 = fabs(f_val_new - f_val_old);
			
			//termination?
			if (((diff1 / N) < tol) || (diff2 < ftol))
			//if ((diff1 / N) < tol)
			//if (diff2 < ftol)
			{
		...
		if (cnt == iterations){//max number of iteration achieves before tol is satisfied
			std::cout << cnt << ": Reached maximum number of allowed iterations. The result may not be optimal" << std::endl;
		}

		return make_tuple(x[x1], cnt);
	}
