...
for i in [1000000, 2000000, 4000000, 8000000, 16000000, 32000000, 64000000]:
    ...
    for j in range(3):
        start = time.time()
        results, centers, rotation_vectors = egocentrically_align_pose(data=data_arr, anchor_1_idx=2, anchor_2_idx=3, anchor_location=np.array([250, 250]).astype(np.int32), direction=90)
