...
def set_dataset_metadata_temporal_coverage_end_date(temporal_coverage_end_date: str | datetime | None, dataset_id: str = None, dataset_uid: str = None, publish: bool = True, should_override_remote_value: bool = True, tzinfo: ZoneInfo =ZoneInfo("Europe/Zurich")) -> None:
    """
    Set the temporal coverage end date for a dataset. Either `dataset_id` or `dataset_uid` can be used to specify the
    dataset.
    ...
    if temporal_coverage_end_date is not None:
        zurich_time = temporal_coverage_end_date.replace(tzinfo=tzinfo)
        end_time_utc_obj = zurich_time.astimezone(ZoneInfo("UTC"))
        end_time_utc = end_time_utc_obj.strftime("%Y-%m-%dT%H:%M:%SZ")
    else:
        end_time_utc = None # Not strictly necessary, but we do this to satisfy the interpreter

    if temporal_coverage_end_date is None:
        updated_json['dcat'].pop('temporal_coverage_end', None)
    else:
        if 'dcat' not in updated_json:
            updated_json['dcat'] = {}

        if 'temporal_coverage_end' not in updated_json['dcat']:
            updated_json['dcat']['temporal_coverage_end'] = {}


        updated_json['dcat']['temporal_coverage_end']['override_remote_value'] = should_override_remote_value
        remote_value = updated_json['dcat']['temporal_coverage_end'].get('remote_value', None)
        updated_json['dcat']['temporal_coverage_end']['remote_value'] = remote_value
        updated_json['dcat']['temporal_coverage_end']['value'] = end_time_utc
