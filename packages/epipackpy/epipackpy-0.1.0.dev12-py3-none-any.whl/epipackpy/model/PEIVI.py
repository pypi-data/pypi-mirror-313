...
class model_dataset2(Dataset):
    def __init__(self, counts, peak_embedding, batch_id, log_trans=True):
        ...
        counts_norm = np.log1p(counts/size_factor)
        if log_trans:
...
class PEIVI(nn.Module):
    ...
    def inference(self, m_promoter, batch_id = None, clamp_promoter = 0.01, eval=False, print_stat = False):
        ...
        mu_p, logvar_p = self.Encoder(m_promoter, batch_id)
    ...
    def loss(self, 
        ...
        #2. promoter rec loss
        if rec_type == 'MSE':
            rec_rate = rec['rec_promoter']*size_factor
    ...
    def train_model(self, 
        ...
        for epoch in loop:
            for id, x in enumerate(self.train_loader):
                ...
                #pass encoder
                dict_inf = self.inference(m_promoter = x['counts_norm'].to(self.device_use), 
