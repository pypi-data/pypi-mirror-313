...
class GetSmDevicesResult:
    ...
    def system_types(self) -> Optional[Sequence[str]]:
        """
    ...
    @property
    @pulumi.getter
    def uuids(self) -> Optional[Sequence[str]]:
        """
    ...
    def wifi_macs(self) -> Optional[Sequence[str]]:
        """
...
def get_sm_devices(ending_before: Optional[str] = None,
    ...
    :param Sequence[str] fields: fields query parameter. Additional fields that will be displayed for each device.
           The default fields are: id, name, tags, ssid, wifiMac, osName, systemModel, uuid, and serialNumber. The additional fields are: ip,
           systemType, availableDeviceCapacity, kioskAppName, biosVersion, lastConnected, missingAppsCount, userSuppliedAddress, location, lastUser,
           ownerEmail, ownerUsername, osBuild, publicIp, phoneNumber, diskInfoJson, deviceCapacity, isManaged, hadMdm, isSupervised, meid, imei, iccid,
           simCarrierNetwork, cellularDataUsed, isHotspotEnabled, createdAt, batteryEstCharge, quarantined, avName, avRunning, asName, fwName,
           isRooted, loginRequired, screenLockEnabled, screenLockDelay, autoLoginDisabled, autoTags, hasMdm, hasDesktopAgent, diskEncryptionEnabled,
           hardwareEncryptionCaps, passCodeLock, usesHardwareKeystore, androidSecurityPatchVersion, cellular, and url.
    :param Sequence[str] ids: ids query parameter. Filter devices by id(s).
    ...
    :param Sequence[str] scopes: scope query parameter. Specify a scope (one of all, none, withAny, withAll, withoutAny, or withoutAll) and a set of tags.
    :param Sequence[str] serials: serials query parameter. Filter devices by serial(s).
    :param str starting_after: startingAfter query parameter. A token used by the server to indicate the start of the page. Often this is a timestamp or an ID but it is not limited to those. This parameter should not be defined by client applications. The link for the first, last, prev, or next page in the HTTP Link header should define it.
    :param Sequence[str] system_types: systemTypes query parameter. Filter devices by system type(s).
