...
class ActionManager:
    ...
    def Connect(self, domain:Domain, sessionId:str):
        ...
        if(metamodelExists):
            self.config.logger.Info('Found actions meta-model.')
        else:
            self.config.logger.Info('Uploading actions meta-model..')
            self.actionsM2Model = self.domain.Do(ACTIONS_MODEL_CMD,self.sessionId,asDict=True)['o0']
            self.config.logger.Info('ok')
        ...
        try:
            ...
            self.config.logger.Info('Reconnected to actions container %s'%(self.actionsContainer))
            ...
            self.config.logger.Info('Created new actions container %s'%(self.actionsContainer))
