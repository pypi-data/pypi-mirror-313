...
class ActionHandler():
    ...
    def __ConnectToDomain(self):
        ...
        try:
            ...
            self.config.logger.Info('Reconnected to actions handler %s'%(self.actionHandler))
            ...
            self.config.logger.Info('Created new action handler %s'%(self.actionHandler))
    ...
    def __HandleNewJob(self,jobObj:Obj)->None:
        ...
        if(actionObj in self._actionRegistry and 'READY' == status and 'RUN' == controlFlag):
            ...
            self.config.logger.Info("%s called"%(action.name))
            ...
            if('READY' != res[0].GetVal()):
                ...
                self.config.logger.Info("%s is already processed. Skipping request."%(action.name))
            else:
    ...
    def __ExecJob(self,job:Job)->None:
        self.config.logger.Info("Executing job %s."%(job.jobObj))
    ...
    def __AbortJob(self,job:Job)->None:
        self.config.logger.Info("Aborting job %s."%(job.jobObj))
        job.Abort()
        if(job.HasStarted()):
        ...
        for pythonfile in pythonfiles:
            ...
            try: #prevent server crash by broken action scripts
                ...
                if(actionfunctionname not in actionmodule.__dict__):
                    self.config.logger.Warn("Skipped no-action %s in %s."%(actionname,pythonfile))
                    continue
                self.config.logger.Info("Found external action %s in file %s"%(actionname,pythonfile))
                ...
                if actionTags:
                    self.config.logger.Info("Action has tags: %s"%(actionTags))
                actionCategory = ""
                ...
                self.config.logger.Error("Error loading external action %s from %s: %s"%(actionname,pythonfile,str(e)))
    ...
    def __ParseActionArguments(self,actionName,actionFunction):
        ...
        if(nArguments==0):
            self.config.logger.Warn("Action %s has no argument. Actions must have at least one argument of type Domain."%(actionName))
        else:
            ...
            if(argumentName not in actionArgumentTypeInfos):
                self.config.logger.Warn("Argument 1 of action %s is not annotated assuming Domain as type."%(actionName))
            else:
                argumentAnnotation = actionArgumentTypeInfos[argumentName]
                if("Domain"!=argumentAnnotation):
                    self.config.logger.Warn("Argument 1 of action %s has type %s but expected is %s. This will probably not work."%(actionName,argumentAnnotation,"Domain"))
            #add all args after the first 
            for i in range(1,nArguments):
                argumentName = actionArguments[i]
                argument = None
                if(argumentName not in actionArgumentTypeInfos):
                    self.config.logger.Warn("Argument %s of action %s has no annotation. Assuming * as type"%(argumentName,actionName))
    ...
    def __ParseActionResults(self,actionName,actionFunction):
        ...
        if(argumentName not in actionArgumentTypeInfos):
            self.config.logger.Warn("Action %s has no return annotation. Assuming no return value"%(actionName))
        else:
            argumentAnnotation = actionArgumentTypeInfos[argumentName]
    ...
    def __ParseArgumentAnnotation(self,argumentAnnotation):
        ...
        if(multiplicityStart > 0):
            ...
            if(multiplicityEnd>0 and multiplicityEnd>multiplicityStart):
                ...
                self.config.logger.Warn("Malformed multiplicity in argument annotation %s"%(argumentAnnotation))
        ...
        if(choiceStart > 0):
            ...
            if(choiceEnd>0 and choiceEnd>choiceStart+1): #skip { and }
                ...
                self.config.logger.Warn("Malformed choice in argument annotation %s"%(argumentAnnotation))
        #default
        if(defaultStart > 0):
            ...
            if(defaultEnd>0 and defaultEnd>defaultStart+1):
                ...
                self.config.logger.Warn("Malformed default in argument annotation %s"%(argumentAnnotation))
        ...
        if(descriptionStart > 0):
            ...
            if(descriptionEnd>0 and descriptionEnd>descriptionStart+1): #skip :
                ...
                self.config.logger.Warn("Malformed description in argument annotation %s"%(argumentAnnotation))
    ...
    def __UpdateActionRegister(self,actions):
        ...
        for a in actions:
            ...
            if(a.name in actionsInDomainLut):
                ...
                if(self.actionHandler == existingActionRecord[1]): #this is my action, update it
                    ...
                    self.config.logger.Info('Updating action %s.'%(a.name))
                else: #this is not my action, skip it.
                    self.config.logger.Warn('Action %s is already registered and belongs to another action handler (%s). Skipping.'%(a.name,existingActionRecord[1]))
            ...
            #actionRef = self.domain.Do(Get(Try(Cls(ACTION_CLASS_ID).Sel( Pth('name').Equ(a.name).And( Pth('provider').Eqa(self.name) ) ).Idx(0) ,None)))
                ...
                self.config.logger.Info('New action %s.'%(a.name))
        ...
        for k,v in actionsInDomainLut.items():
            if(not v[3]):
                ...
                self.config.logger.Info('Removing action %s.'%(k))     
    ...
    def __JobMonitorThread(self, job:Job):
        self.config.logger.Info("Job %s started."%(job.jobObj))
            ...
            while(proc.is_alive()): #wait until the thread has finished
                ...
                if(hasTimeout):
                    ...
                    if(not wasAborted and pTime > job.timeout): 
                        ...
                        self.config.logger.Info('Job %s: Timeout of %f s reached. Aborting.'%(job.jobObj, job.timeout))
