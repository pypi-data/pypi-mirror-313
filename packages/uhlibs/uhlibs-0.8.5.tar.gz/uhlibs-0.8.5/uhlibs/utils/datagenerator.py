# python implementation of Docker's container name generator:
# https://github.com/kennethd-uh/moby/blob/master/pkg/namesgenerator/names-generator.go
import random

MIDDLE_NAME_CHANCE = 0.25
ADDRESS_2_CHANCE = 0.25
...
def generate_name():
    """returns 3-tuple of strings (first, middle, last)

    there is a 3/4 chance middle will be empty string"""
    first = random.choice(adjectives)
    if random.random() < MIDDLE_NAME_CHANCE:
        middle = random.choice(adjectives)
    else:
        middle = ''
...
def generate_address():
    """returns 2-tuple of strings (address_1, address_2)

    there is a 3/4 chance address_2 will be empty string"""
    bldgno = random.randint(1, 10000)
    street = random.choice(surnames)
    street_type = random.choice(['St.', 'Ave.', 'Blvd.', 'Pl.'])
    address1 = f'{bldgno} {street.capitalize()} {street_type}'
    address2 = ''
    if random.random() < ADDRESS_2_CHANCE:
        address2 = ' '.join([
            random.choice(['Apt.', '#', 'Ste.', 'Fl.']),
