...
from ascend.sdk.definitions import (Credential, Connection, Dataflow, ReadConnector, Transform, WriteConnector, ComponentGroup, ComponentUuidToIdMap, DataFeed,
                                    DataFeedConnector, DataShare, DataShareConnector, DataService)
...
def dataflow_from_proto(client: Client, data_service_id: str, proto: api_pb2.Dataflow) -> 'Dataflow':
  ...
  data_shares = [DataShare.from_proto(c, data_service_id, role_to_ds_map, uuid_map) for c in component_protos if c.type == "data_share"]
  ...
  for c in component_protos:
    ...
    elif c.type == "data_share_connector":
      share = client.get_data_share_for_data_share_connector(data_service_id, proto.id, c.id).data
      payload = client.get_data_share_connector(data_service_id, proto.id, c.id).data
      data_share_connectors.append(DataShareConnector.from_proto(payload, share))
    elif c.type == "group":
