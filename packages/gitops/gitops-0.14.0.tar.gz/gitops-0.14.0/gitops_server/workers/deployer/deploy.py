...
class Deployer:
    ...
    async def deploy(self):
        ...
        self.post_ts = await self.post_deploy_summary()
    ...
    async def post_deploy_summary(self):
        ...
        if not self.post_ts:
            await slack.post(message=message)
        else:
            await slack.update(ts=self.ts, message=message)
