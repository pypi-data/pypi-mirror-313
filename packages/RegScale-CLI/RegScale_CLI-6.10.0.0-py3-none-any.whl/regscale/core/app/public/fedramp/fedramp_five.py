...
def _update_existing_control(
    ...
    update_existing_control(
        ...
        new_statement if new_statement else control_dict.get("statement"),
        control_dict.get("responsibility"),
        isinstance(primary_role, dict) and primary_role.get("id") if primary_role else None,
        parent_id,
    )
    if params := control_dict.get("parameters"):
        handle_params(
...
def get_or_create_system_role(role: str, ssp_id: int) -> Dict:
    ...
    try:
        ...
        existing_sys_roles = SystemRole.get_all_by_parent(
            parent_id=ssp_id, parent_module=SecurityPlan.get_module_slug()
        )
        existing_role = next(
            (role for role in existing_sys_roles if role["roleName"].lower().strip() == role_name.lower().strip()),
            None,
        )
        if existing_role:
