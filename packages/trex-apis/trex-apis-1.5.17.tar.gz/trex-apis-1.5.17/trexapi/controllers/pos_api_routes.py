...
from trexapi.utils.api_helpers import create_api_message
...
def getPOSAccountSetting(activation_code):
    if is_not_empty(activation_code):
        ...
        if pos_setting:
            ...
            return create_rest_message('Invalid activation code', status_code=StatusCode.BAD_REQUEST, error_code=API_ERR_CODE_INVALID_ACTIVATION_CODE)
    else:
        logger.info('activation_code is empty');
        return create_rest_message('Activation code is required', status_code=StatusCode.BAD_REQUEST)
...
def check_catalogue_status():
    ...
    if is_not_empty(activation_code):
        ...
        if pos_setting is None:
            return create_rest_message('Invalid activation code', status_code=StatusCode.BAD_REQUEST, err_code=API_ERR_CODE_INVALID_ACTIVATION_CODE)
        else:
