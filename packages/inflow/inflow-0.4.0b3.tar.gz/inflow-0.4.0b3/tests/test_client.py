...
@pytest.fixture
def client():
    return Client("https://user:pass@localhost:8086", db="testdb", timeout=1)
...
class TestInit:
    def test_connection(self):
        ...
        client = Client("http://user:pass@localhost:8086", db="databasename")
...
class TestWrite:
    ...
    def test_retention_policy_on_write_call(self, post):
        client = Client("https://user:pass@localhost:8086", db="testdb", timeout=1)
        client.write(
            "temperature",
            value=21.3,
    ...
    def test_escaped_tags(self, client, post):
        client.write(
            "temperature", tags={"a,=b ": "a,=b "}, value=21.3, timestamp=1476107241
        )
    ...
    def test_escaped_string_values(self, client, post):
        client.write(
            "temperature",
            value='well, a pretty nice "temperature"',
            timestamp=1476107241,
        )
    ...
    def test_measurement_instance(self, client, post):
        client.write(Measurement("temperature", value=21.3, timestamp=1476107241))
    ...
    def test_measurement_no_timestamp(self):
        measurement = Measurement("temperature", value=21.3)
    ...
    def test_measurement_list(self, client, post):
        client.write(
            [
                Measurement("temperature", value=32.1, timestamp=1476107241),
                Measurement("temperature", value=21.9, timestamp=1476107319),
            ]
        )
    ...
    def test_exception(self, client, status_code, exception, post, post_mock_response):
...
class TestQuery:
    ...
    @pytest.mark.parametrize(
        "query", ['SELECT * FROM "temperatures"', 'SHOW FIELD KEYS from "temperatures"']
    )
    ...
    def test_exception(self, client, status_code, exception, get, get_mock_response):
