...
impl MyMove {
    ...
    pub fn new(uci:&str, my_chess:&MyChess) -> MyMove {
    	let chess_move = UciMove::from_ascii(uci.as_bytes()).expect("REASON").to_move(&my_chess.chess).expect("REASON");
        MyMove{
...
impl MyMove {
    ...
    pub fn new_rust(move_:Move) -> MyMove {
        ...
        MyMove {
            chess_move: move_.clone(),
            uci:move_.to_uci(CastlingMode::Standard).to_string()
        }
    }
...
impl MyChess {
    ...
    fn play_and_return_o(&mut self, my_move_: &MyMove) -> PyResult<(u64,u64,u64,u64,u64,u64,u64,u64,u64,u8,i32,u64)> {
        self.chess.play_unchecked(&my_move_.chess_move);
...
cas,
 ...
 turn, ep_square ,promoted      )
   ...
   // fn legal_moves_dic(&mut self) -> PyResult<HashMap<String,MyMove,>> {
   ///     let move_list :MoveList = self.chess.legal_moves();
   //     let mut legal_moves_my = HashMap::new();
    //    for a_move in move_list {
      //      legal_moves_my.push((MyMove::new_rust(a_move.clone()),a_move.clone().to_uci(CastlingMode::Standard).to_string() ));
       //     }
 //       Ok(legal_moves_my)
  //  }
    ...
    fn legal_moves(&mut self) -> PyResult<Vec<MyMove>> {
        ...
        for a_move in move_list {
            legal_moves_my.push(MyMove::new_rust(a_move.clone()));
            }
        Ok(legal_moves_my)
    }
