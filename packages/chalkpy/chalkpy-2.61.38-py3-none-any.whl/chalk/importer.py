    ...
    if isinstance(underscore, UnderscoreFunction):
        for arg in underscore._chalk__args:
            ...
            if isinstance(expr, _NamespaceExpr):
                raise _UnderscoreValidationError(
                    f"The underscore expression '{underscore!r}' is invalid because the input '{arg!r}' is a feature namespace '{expr.namespace}' which cannot be used as a scalar value"
                )
