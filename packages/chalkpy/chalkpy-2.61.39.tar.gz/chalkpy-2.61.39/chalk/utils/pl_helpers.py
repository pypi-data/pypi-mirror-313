...
def pl_json_decode(series: pl.Series):
    if is_new_polars:
        return series.map_elements(json_loads)  # pyright: ignore -- polars backcompat
    return series.apply(json_loads)
...
def _json_encode_inner(expr: pl.Expr, dtype: pl.PolarsDataType) -> pl.Expr:
    ...
    if isinstance(dtype, type):
        ...
        if issubclass(
            dtype,
            (pl.Float32, pl.Float64, pl.Int16, pl.Int32, pl.Int64, pl.Int8, pl.UInt16, pl.UInt32, pl.UInt64, pl.UInt8),
        ):
            # Ints can be cast directly
            return expr.cast(pl.Utf8, strict=True)
        if issubclass(dtype, pl.Utf8):
