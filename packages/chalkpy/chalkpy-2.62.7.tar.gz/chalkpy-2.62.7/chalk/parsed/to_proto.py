...
class _GetStaticOperatorError(Exception):
    ...
    def __init__(self, resolver_fqn: str, message: str, underlying_exception: Exception | None):
...
class ToProtoConverter:
    ...
    def convert_online_or_offline_resolver(cls, r: Union[OnlineResolver, OfflineResolver]) -> pb.Resolver:
        ...
        if r.static:
            if len(r.inputs) > 0 or not (
                len(r.output.features) == 1
                and isinstance(r.output.features[0], type)
                and issubclass(r.output.features[0], DataFrame)
            ):
            ...
            try:
                static_operator = r.fn()
            except Exception as e:
                raise _GetStaticOperatorError(
                    resolver_fqn=r.fqn, message="Resolver failed with an exception", underlying_exception=e
                )
            else:
