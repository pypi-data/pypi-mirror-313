    ...
    def convert_online_or_offline_resolver(cls, r: Union[OnlineResolver, OfflineResolver]) -> pb.Resolver:
        ...
        if r.static:
            if len(r.inputs) > 0 or not (
                ...
                raise ValueError("Static resolver must take no arguments and have exactly one DataFrame output")
            static_operator = r.fn()
            if not isinstance(static_operator, StaticOperator):
                raise ValueError("Static resolver must return a StaticOperator")
