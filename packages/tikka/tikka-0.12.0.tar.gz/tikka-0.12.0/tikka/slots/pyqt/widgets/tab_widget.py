...
class TabWidget(QTabWidget):
    ...
    def __prepare_menu(self, p):
        ...
        if tab_idx != -1:
            ...
            close_tab_action = QAction("Close Tab")
            close_tab_action.triggered.connect(self.close_tab)

            close_all_tab_action = QAction("Close All Tabs")
            close_all_tab_action.triggered.connect(self.close_all_tab)

            close_other_tab_action = QAction("Close Other Tabs")
            close_other_tab_action.triggered.connect(self.close_other_tab)

            close_tab_to_the_left_action = QAction("Close Tabs to the Left")
            close_tab_to_the_left_action.triggered.connect(self.close_tab_to_left)

            close_tab_to_the_right_action = QAction("Close Tabs to the Right")
            close_tab_to_the_right_action.triggered.connect(self.close_tab_to_right)

            reopen_closed_tab_action = QAction("Reopen Closed Tab")
