...
from dbt.adapters.gaussdbdws.connections import GaussDBDWSConnectionManager
from dbt.adapters.gaussdbdws.relation import GaussDBDWSRelation


GET_RELATIONS_MACRO_NAME = "gaussdbdws__get_relations"


@dataclass
class GaussDBDWSIndexConfig(dbtClassMixin):
    columns: List[str]
    unique: bool = False
    ...
    @classmethod
    def parse(cls, raw_index) -> Optional["GaussDBDWSIndexConfig"]:
...
@dataclass
class GaussDBDWSConfig(AdapterConfig):
    unlogged: Optional[bool] = None
    indexes: Optional[List[GaussDBDWSIndexConfig]] = None


class GaussDBDWSAdapter(SQLAdapter):
    Relation = GaussDBDWSRelation
    ConnectionManager = GaussDBDWSConnectionManager
    Column = GaussDBDWSColumn

    AdapterSpecificConfigs = GaussDBDWSConfig
    ...
    @available
    def parse_index(self, raw_index: Any) -> Optional[GaussDBDWSIndexConfig]:
        return GaussDBDWSIndexConfig.parse(raw_index)
