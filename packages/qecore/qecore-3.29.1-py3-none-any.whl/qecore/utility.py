...
class RunAndLog:
    ...
    def run_and_log(self):
        ...
        try:
            ...
            ).strip("\n") # Lets improve logging by stripping the last line break.
            ...
            LOGGING.debug(
                " ".join((
                    f"Command run: output='{self._output}',",
            ...
            self._output = error.output.strip("\n")
            self._return_code = error.returncode
            self._error = error
            LOGGING.debug(
                " ".join((
                    f"CalledProcessError caught: output='{self._output}',",
                    f"return_code='{self._return_code}',",
                    f"error='{self._error}'"
                ))
            )
            ...
            LOGGING.debug(
                " ".join((
                    f"TimeoutExpired caught: output='{self._output}',",
                    f"return_code='{self._return_code}',",
                    f"error='{self._error}'"
                ))
            )
            ...
            LOGGING.debug(
                " ".join((
                    f"Unexpected Exception caught: output='{self._output}',",
                    f"return_code='{self._return_code}',",
                    f"error='{self._error}'"
                ))
            )
    ...
    def output(self):
        ...
        LOGGING.debug(f"Output value: '{self._output}'")
