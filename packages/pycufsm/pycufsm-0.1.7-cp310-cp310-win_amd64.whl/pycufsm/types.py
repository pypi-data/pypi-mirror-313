...
Cufsm_MAT_File = TypedDict(
    "Cufsm_MAT_File",
    {
        ...
        "springs": list,
        "curve": list,
        "GBTcon": Any,  # this is some special structure with string dictionaries but a dtype attribute
        "shapes": list,
        "clas": str,
    },
)
...
New_Spring = TypedDict(
    "New_Spring",
    {
        ...
        "k_type": Literal["foundation", "total", "node_pair"],  # "foundation"|"total"|"node_pair" - stiffness type,
...
Yield_Force = TypedDict(
    "Yield_Force",
    {"force": ForceTypes, "direction": Directions, "f_y": float, "restrain": bool, "offset": Optional[ArrayLike]},
)
