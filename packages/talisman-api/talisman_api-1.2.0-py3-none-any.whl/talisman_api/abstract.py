...
class AbstractTalismanAPI(metaclass=ABCMeta):
    ...
    @classmethod
    async def create(cls, client: CompositeTalismanAPIClient) -> Self:
        ...
        for api_schema in cls._required_apis():
            ...
            async with api_client:
                if not api_client.validate(document):
                    raise ValueError(f'{resource_name} (package {package}) do not match runtime server {api_schema} API')
