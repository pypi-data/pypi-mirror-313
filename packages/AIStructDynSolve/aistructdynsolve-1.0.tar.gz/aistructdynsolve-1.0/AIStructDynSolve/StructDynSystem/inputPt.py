...
class inputPt:
    def __init__(self, StructDynSystemParm, duration_time, pt_time, pt_value):
        ...
        # 返回所有最大值中的最大值
        max_value = max([np.abs(Pt).max() if isinstance(Pt, np.ndarray) else Pt.abs().max() for Pt in self.pt_value])
        self.dimensionsless_Pt = max_value / StructDynSystemParm.get_dimensionsless_input() /StructDynSystemParm.get_dimensionsless_unit()
