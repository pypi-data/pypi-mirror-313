...
class LossEQ:
    ...
    def lossObserve(self):
        ...
        if self.observe_data is not None:
            ...
            observelist = self.observe_data[2]/self.dimensionsless_Pt  # 位移或加速度观察值列表
            ...
            if self.observe_data[0] == "Disp":
                ...
                for dof, disp_observe in zip(observelist_DOF, observelist):
                    ...
                    disp_observe_tensor = torch.tensor(disp_observe, device=DEVICE)
                    # 计算损失
                ...
                for dof, acc_observe in zip(observelist_DOF, observelist):
                    ...
                    acc_observe_tensor = torch.tensor(acc_observe, device=DEVICE)
                    # 计算损失
