...
impl<'a> StridedIndex<'a> {
    pub(crate) fn new(dims: &'a [usize], stride: &'a [usize], start_offset: usize) -> Self {
        ...
        StridedIndex { next_storage_index, multi_index: vec![0; dims.len()], dims, stride }
    }
...
impl Iterator for StridedIndex<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        for ((multi_i, max_i), stride_i) in
            self.multi_index.iter_mut().zip(self.dims.iter()).zip(self.stride.iter()).rev()
        {
