...
def courses(id=0):
	# s_course uitbreiden:
	# veld extra is list met dicts van summatieve dingen
	syslo = Sysls()
	lijsten = syslo.get_lijsten_nicename()
	all_courses = syslo.get_sysl('s_course')
	allitems = list()
	for k, v in all_courses.items():
		if not 'extra' in v:
			v['extra'] = list()
		elif not isinstance(v['extra'], list):
				v['extra'] = list()
		allitems.append(JINJAstuff(v, syslo.get_model()))
