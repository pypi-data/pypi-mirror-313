...
class Mainroad:
	...
	def test_settings(cls) -> bool:
		...
		if not 'onedrive' in thing:
			cls.loglog(f"test_settings_thing 2: no onedrive in {thing}")
			return False
	...
	def get_onedrive_path(cls, firstrun: bool = False) -> str|None:
		...
		if settings is None:
			if not firstrun:
				sys.exit('ERROR: No Settings found [1].')
			return None
		try:
			return settings['onedrive']
		except:
			# geen onedrive in settings
			sys.exit('ERROR: No OneDrive found [2].')
