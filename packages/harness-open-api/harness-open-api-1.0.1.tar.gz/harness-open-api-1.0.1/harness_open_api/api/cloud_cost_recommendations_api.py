...
class CloudCostRecommendationsApi(object):
    ...
    def recommendation_resource_type_stats(self, body, account_identifier, **kwargs):  # noqa: E501
        """Return Recommendations statistics Grouped on Resource Type  # noqa: E501

        Returns the Cloud Cost Recommendations statistics for the specified filters Grouped on Resource Type.  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.recommendation_resource_type_stats(body, account_identifier, async_req=True)
        >>> result = thread.get()

        :param async_req bool
        :param CCMRecommendationFilterProperties body: CCM Recommendations filter body. (required)
        :param str account_identifier: Account Identifier for the Entity. (required)
        :return: ResponseDTOListRecommendationResourceTypeStats
                 If the method is called asynchronously,
                 returns the request thread.
        """
        kwargs['_return_http_data_only'] = True
        if kwargs.get('async_req'):
            return self.recommendation_resource_type_stats_with_http_info(body, account_identifier, **kwargs)  # noqa: E501
        else:
            (data) = self.recommendation_resource_type_stats_with_http_info(body, account_identifier, **kwargs)  # noqa: E501
            return data

    def recommendation_resource_type_stats_with_http_info(self, body, account_identifier, **kwargs):  # noqa: E501
        """Return Recommendations statistics Grouped on Resource Type  # noqa: E501

        Returns the Cloud Cost Recommendations statistics for the specified filters Grouped on Resource Type.  # noqa: E501
        This method makes a synchronous HTTP request by default. To make an
        asynchronous HTTP request, please pass async_req=True
        >>> thread = api.recommendation_resource_type_stats_with_http_info(body, account_identifier, async_req=True)
        >>> result = thread.get()
        ...
        for key, val in six.iteritems(params['kwargs']):
            if key not in all_params:
                raise TypeError(
                    "Got an unexpected keyword argument '%s'"
                    " to method recommendation_resource_type_stats" % key
                )
            params[key] = val
        del params['kwargs']
        # verify the required parameter 'body' is set
        if ('body' not in params or
                params['body'] is None):
            raise ValueError("Missing the required parameter `body` when calling `recommendation_resource_type_stats`")  # noqa: E501
        # verify the required parameter 'account_identifier' is set
        if ('account_identifier' not in params or
                params['account_identifier'] is None):
            raise ValueError("Missing the required parameter `account_identifier` when calling `recommendation_resource_type_stats`")  # noqa: E501
        ...
        return self.api_client.call_api(
            '/ccm/api/recommendation/overview/resource-type/stats', 'POST',
            path_params,
            query_params,
            header_params,
            body=body_params,
            post_params=form_params,
            files=local_var_files,
            response_type='ResponseDTOListRecommendationResourceTypeStats',  # noqa: E501
            auth_settings=auth_settings,
            async_req=params.get('async_req'),
            _return_http_data_only=params.get('_return_http_data_only'),
            _preload_content=params.get('_preload_content', True),
