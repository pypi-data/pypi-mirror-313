...
from cavd.modules.struc_analysis import write_poscar
...
class Node:
    ...
    def as_dict(self):
        d = {"label": self.label, "conn_num": self.conn_num}
        return d
...
class MigrationPaths(object):
    ...
    def setNetwork(self):
        ...
        for key, value in self.interstices.items():
            ...
            graph.add_node(
                key, label=value["label"], cart_coord=carts, frac_coord=fracs
            )
    ...
    def comDis(self, p1, p2):
        dis = self.struc.get_mic_dis(p1, p2)
        return dis
    ...
    def setKeyInterstices(self):
        ...
        mobiles = [
            ...
            if migrant in sites[idx].atom_symbols[0]
        ]
        ...
        for mobileSite in mobiles:
            ...
            for inter in iterList:
                ...
                dis = self.comDis(mobileSite[1].coord, iterSite)
                if dis < minDis:
                    minDis = dis
                    minId = iterId
            tmpDict = {}
            tmpDict["interId"] = minId
            tmpDict["mobileLabel"] = mobileSite[1].label[
                : mobileSite[1].label.rfind("_")
            ]
    ...
    def getPaths(self, suorceInter, sinkInter, n=2, cutoff=5.0):
        ...
        if pathLenPairs:
            ...
            if n > 0 and n <= len(pathLenPairs):
                ...
                raise Exception(
                    "Given n is larger than the total number of paths between source and sink!!!"
                )
    ...
    def setAllPaths(self, n=0):
        ...
        for source in self.keyInterstices:
            for sink in self.keyInterstices:
                if source["interId"] < sink["interId"]:
                    originPaths.extend(
                        self.getPaths(source["interId"], sink["interId"], n)
                    )
        ...
        for path in originPaths:
            ...
            for idx in range(len(path)):
                interId = path[idx]
                itLabelsOfPath.append(self.network.nodes[interId]["label"])
                itCartOfPath.append(self.network.nodes[interId]["cart_coord"])
                itFracOfPath.append(self.network.nodes[interId]["frac_coord"])
                if idx == 0:
                    itPDVofPath.append([0, 0, 0])
                else:
                    if path[idx - 1] < path[idx]:
                        nodePDV = np.array(itPDVofPath[idx - 1]) + np.array(
                            self.network[path[idx - 1]][path[idx]]["ascPDV"]
                        )
                        itPDVofPath.append(nodePDV.tolist())
                    else:
                        nodePDV = np.array(itPDVofPath[idx - 1]) + np.array(
                            self.network[path[idx - 1]][path[idx]]["desPDV"]
                        )
                        itPDVofPath.append(nodePDV.tolist())
            ...
            if itLabelsOfPath in symUniqPaths:
                ...
                tmpDict = {}
                tmpDict["type"] = (
                    self.keyInterMobileDict[itLabelsOfPath[0]],
                    self.keyInterMobileDict[itLabelsOfPath[-1]],
                )
                tmpDict["ids"] = path
    ...
    def outPathToPOSCAR(self, fname):
        ...
        for i in range(len(self.allPaths)):
            ...
            for j in range(len(pathCoords)):
                ...
                if j == 0:
                    imageCoords = stru.sites[startIdx].coord
                elif j == len(pathCoords) - 1:
                    imageCoords = stru.sites[endIdx].coord
                else:
                    imageCoords = pathCoords[j]
                site.coord = imageCoords
                site.x = imageCoords[0]
                site.y = imageCoords[1]
                site.z = imageCoords[2]
                site.label = imageProps["_atom_site_label"]
                self.struc.sites.insert(startIdx, site)
                stru.sites.pop(startIdx)
                stru.sites.insert(startIdx, site)
                stru.sites.pop(endIdx)
                stru.write_to_poscar(filename=imageDir + "/POSCAR")
                # write_poscar(filename=imageDir + "/POSCAR", stru=stru)
    ...
    for key, value in voidnet.items():
        ...
        for conn in void["conn"]:
            ...
            edge = Edge(
                void["label"],
                to_void["label"],
                round(conn["bot_radius"], preci),
                round(conn["conn_legth"], preci),
            ).as_dict()
