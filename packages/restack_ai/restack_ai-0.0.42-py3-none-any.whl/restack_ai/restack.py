...
from temporalio.client import Client, Schedule, ScheduleActionStartWorkflow, ScheduleSpec, ScheduleCalendarSpec, ScheduleIntervalSpec, ScheduleRange
from temporalio.common import SearchAttributePair, SearchAttributeKey,TypedSearchAttributes
...
class Restack:
    ...
    def get_connection_options(self) -> Dict[str, Any]:
        target_host = self.options.address if self.options and self.options.address is not None else 'localhost:7233'
        engine_id = self.options.engine_id if self.options and self.options.engine_id is not None else 'local'
        options = {
    ...
    async def create_client(self, connection_options: Optional[CloudConnectionOptions] = None) -> Client:
        ...
        namespace = self.options.temporal_namespace if self.options and self.options.temporal_namespace is not None else 'default'
        api_key = self.options.api_key if self.options and self.options.api_key is not None else None
        tls = True if self.options and self.options.api_key is not None else False
        metadata = connect_options['metadata']
        data_converter = self.options.data_converter if self.options and self.options.data_converter is not None else DataConverter.default
        try:
    ...
    async def create_service(self, workflows: Optional[List[Any]] = None, functions: Optional[List[Any]] = None, task_queue: Optional[str] = None,options: Optional[ServiceOptions] = None) -> Worker:
        try:
            ...
            if e.__cause__ and "http.client.incompleteread.__mro_entries__".lower() in str(e.__cause__).lower():
                log_with_context("ERROR", "Failed to start service: Functions in workflow steps need to be imported with import_functions(). See docs at https://docs.restack.io/libraries/python/workflows")
            else:
        ...
        if self.client:
            try:
                ...
                search_attribute_pairs = [
                    SearchAttributePair(key=SearchAttributeKey.for_text('engineId'), value=engine_id)
                ]
                search_attributes = TypedSearchAttributes(search_attributes=search_attribute_pairs)
