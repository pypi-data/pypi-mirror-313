...
class Workflow:
    ...
    async def step(self, activity, *args, task_queue: Optional[str] = 'restack', schedule_to_close_timeout: Optional[timedelta] = timedelta(minutes=2), retry_policy: Optional[RetryPolicy] = None, **kwargs):
        input_arg = kwargs.pop('input', None)
        if input_arg is not None:
            args = (*args, input_arg)
        return await temporal_workflow.execute_activity(activity, *args, task_queue=task_queue, schedule_to_close_timeout=schedule_to_close_timeout, retry_policy=retry_policy, **kwargs)
