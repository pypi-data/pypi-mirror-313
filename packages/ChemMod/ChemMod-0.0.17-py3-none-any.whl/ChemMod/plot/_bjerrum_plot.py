...
def bjerrum_plot(pKa=[],optimalpH=None,pHvalues=[],Ka=[],acidname=['','','','','','','','']):
  ...
  for i in range(len(pKa)):
    ...
    for j in range(len(pHvalues)):
      plt.plot(pHvalues[j],(1 / ( 1 + 10**(  pHvalues[j] - (pKa[i])  ) )),label=f'pH = {pHvalues[j]}:  ({acidname[i]}) = {round((1 / ( 1 + 10**(  pHvalues[j] - (pKa[i] ) ) ))*100,3)} %',marker='o',color=viridis(j,1))
      n_labels+=1
      if n_labels % 15 == 0:
        legend_ncols+=1
  ...
  plt.axvline(x = optimalpH, color = theme_list_for_formatting_of_plots[4], label = f'Bufferarea for pH = {optimalpH}',dash_capstyle='projecting', lw=2.5, dashes=(4,5),  )
  plt.axvspan(optimalpH+1, optimalpH-1, facecolor=theme_list_for_formatting_of_plots[5], alpha=0.2 , hatch='+', ec=theme_list_for_formatting_of_plots[5])
  plt.legend(loc='center left',bbox_to_anchor=(1, 0,legend_ncols*(0.5)+0.1,1.1), bbox_transform=None,mode="expand", borderpad=1, fontsize=9, title='Reactions',title_fontsize=12,facecolor=theme_list_for_formatting_of_plots[8],edgecolor=theme_list_for_formatting_of_plots[5], shadow=True, fancybox=True,labelcolor=theme_list_for_formatting_of_plots[6], framealpha=1,ncols=legend_ncols,columnspacing=1000)
  ...
  # [0 border_color, 1 border_width, 2 text_color, 3 data_color, 4 marker_color, 5 markeredge_color, 6 label_color, 7 outside_face_color, 8 indside_face_color, 9 grid_color]