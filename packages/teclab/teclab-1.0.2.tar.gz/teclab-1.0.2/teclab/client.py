...
class Client:
    def __init__(self,address,middlewares=None,SECRET=None):
        # 参数
        self.adapter=adapter.manager.AdapterManager([adapter.parse_address("client",address)])
        self.SECRET=SECRET
        # 构建连接
    ...
    def request(self,function,params,auto_retry=True,confirm_timeout=3,result_timeout=30):
        if not self.running: raise Exception("Client not started.")
        id=str(uuid.uuid4())
        # 发送请求
