...
class Course(JsonModel, extra=Extra.allow):
    course: str = Field(index=True)

    
# Questions as from the Digi dataset
class Question(JsonModel, use_enum_values=True):
    question_number: str  = Field(index=True)
    question: str
    correct_answer: str
...
class StatementOfWork(JsonModel):
   ...
   course: str = Field(index=True) # actually this is the course
   level: str
