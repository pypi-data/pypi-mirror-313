...
def add_session_to_sow(course: str, level: str, session: Dict):
   try:
      ...
      if sow_list:
         ...
         logging.error(f"Cannot find SOW for course {course} level {level}")
   except Exception as e:
      logging.error(f"Failed to create SOW: {str(e)}")
      raise e
...
def read_question_by_number(course: str, question_number:str):
   """
   
   """
   try:  
      q = Question.find(Question.course==course, Question.question_number==question_number).first()
      return q
   except Exception as e:
     logging.error(f"Question {question_number} for course {course} not found.")
...
def create_question(course: str, question:str, question_number: str,
   ...
   try:
      ...
      if (q:= read_question_by_number(course, question_number)):
         logging.error(f"Failed to create question: question {question_number} already exists in course {course}")
         raise HTTPException(404, detail=f"Question {question_number} already exists in course {course}")
...
def update_question(question_pk:str, **kwargs):
   """
   
   """
   try:
      q = Question.find(Question.pk==question_pk).first()
      if kwargs: 
...
def create_course(course_name: str):
   ...
   """
   try:
      course = read_course_by_name(course_name)
      if course:
         logging.error(f"A course with name {course_name} already exists.")
         raise HTTPException(404, detail="A course with name {course_name} already exists.")
      c = Course(course=course_name)
      c.save()
      return c
   except Exception as e:
      logging.error(f"Failed to create Course {course_name}: {str(e)}")
      raise e
...
def read_course_by_name(course_name: str):
   """
   Get Courses by course name
   """
   try:
      course = Course.find(Course.course==course_name).first()
      return course
   except Exception as e:
