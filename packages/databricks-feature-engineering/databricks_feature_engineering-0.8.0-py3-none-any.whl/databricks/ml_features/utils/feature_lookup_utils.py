...
def _spark_asof_join_features_native(
    ...
    # Filter out rows where features timestamp key is not the max timestamp key
    window_spec = (
        Window.partitionBy([labels_df_keys_only[key] for key in primary_keys])
        .orderBy(labels_df_keys_only[timestamp_key])
        .rangeBetween(Window.unboundedPreceding, 0)
    )
    labels_df_keys_with_features_keys = labels_df_keys_with_features_keys.withColumn(
        "__max_ts",
        F.max(labels_df_keys_with_features_keys["__features_tk"]).over(window_spec),
    )
    labels_df_keys_with_features_keys = labels_df_keys_with_features_keys.filter(
        (F.col("__features_tk") == F.col("__max_ts"))
    )
    labels_df_keys_with_features_keys = labels_df_keys_with_features_keys.select(
        labels_df_keys_only["*"], F.col("__max_ts")
    ).distinct()
