...
def extract_scripts(content):
    ...
    script_data = [script.string for script in scripts if script.string]
    return script_data
...
# Handle the output based on requested format
def handle_output(content, status_code, script_data=None):
    if status_code != 200:
    ...
    try:
        ...
        response = {
            ...
            'xml': xml_content(content) if content else "Not valid XML",
            'js': script_data if script_data else "Not valid JS",
            'statuscode': status_code,
            'sourcecode': f"{status_code}\n{content}",
        }
