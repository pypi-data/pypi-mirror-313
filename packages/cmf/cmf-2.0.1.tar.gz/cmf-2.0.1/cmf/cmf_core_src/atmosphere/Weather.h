	...
	namespace atmosphere{
		...
		/**  @brief Calculates the global radiation in MJ/(m2 day) from the sun position and the sunshine fraction
			...
			The following formula is used:
			 - \f$ \phi\f$ Latitude in \f$rad\f$
			 - \f$ \delta = 0.409 \sin\left(\frac{2\pi}{365}DOY - 1.39\right)\f$ Declination, DOY is day of year
			 - \f$ \omega_s = \arccos(-\tan\phi\tan\delta) \f$ Sunset angle
			 - \f$ G_{sc} = 0.0802 \frac{MJ}{m^2min} \f$ Solar constant
			 - \f$ d_r = 1+0.033 \cos\left(\frac{2\pi}{365}DOY\right) \f$ Inverse relative distance Earth-Sun
			 - \f$ b = \frac{2\pi(DOY-81)}{364}\f$
			 - \f$ S_c = 0.1645\sin(2b)-0.1255\cos(b)-0.025\sin(b) \f$  Seasonal correction for solar time
			 - \f$ \omega = \frac {\pi} {12}	\left(t_h+\frac{(\mbox{geogr. Longitude})^\circ}{15^\circ}-\mbox{Timezone}+S_c-12\right) \f$ solar time in \f$rad\f$
			 - If daily: \f$ R_a = \frac{24\ 60}{\pi}G_{sc}\ d_r \left(\omega_s \sin\phi \sin\delta + \cos\phi \cos\delta \sin\omega_s\right) \f$
		...
		/**  @brief Calculates the global radiation in MJ/(m2 day) from the sun position and the sunshine fraction
			...
			@param Ra extra terrestrial radiation
			...
			The following formula is used:
			 - \f$R_a(t, \phi, \lambda)\f$  Extraterrestrial radiation (@ref cmf::atmosphere::extraterrestrial_radiation) \f$\frac{MJ}{m^2 day}\f$
			 - \f$\frac n N\f$ Fractional sunshine duration
			 - \f$R_s = \left(0.25+\left(0.5+2\ 10^{-5}z\right)\frac{n}{N}\right)R_a \f$ Global radiation in \f$\frac{MJ}{m^2 day}\f$
			 - \f$z\f$: Height a.s.l. in m
			 **/
		...
		public:
			double
				...
				Ra,			   ///< Extra terrestrial radiation \f$R_s \left[\frac{MJ}{m^2 day}\right]\f$
			...
			**/
			...
			/// @param Ra       actual extraterrestrial shortwave global radiation in MJ/(m2 day)
			...
			Weather(double T=15.0,double Tmax=17.0,double Tmin=13.0, 
					...
					double sunshine=0.5,double  Rs=15, double Ra=30, double daylength=12);
