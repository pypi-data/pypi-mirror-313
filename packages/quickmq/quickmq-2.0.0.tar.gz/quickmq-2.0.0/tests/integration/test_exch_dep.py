...
Make sure AmqpExchange still connects, but also raises a deprecation warning.
"""
...
import amqp
import pytest
from ssec_amqp.amqp import AmqpExchange
...
def test_connection(parallel_containers: ParallelTopologyFactory):
    """Make sure the AmqpExchange connects and disconnects."""
    topo = parallel_containers(1)
    params = topo.connection_params[0]
    with pytest.deprecated_call():
        ex = AmqpExchange(**params)
    ex.connect()
    assert ex.connected
...
def test_connect_auth_error(parallel_containers: ParallelTopologyFactory):
    ...
    with pytest.deprecated_call():
        exch = AmqpExchange(host=params["host"], port=params["port"], user=unauth_user, password=unauth_pass)
...
def test_produce(parallel_containers: ParallelTopologyFactory):
    ...
    with pytest.deprecated_call():
        ex = AmqpExchange(**params, exchange="")
    ex.connect()
...
def test_channel_auto_reopen(parallel_containers: ParallelTopologyFactory):
    """Make sure that AmqpExchange automatically re-opens the AMQP channel
