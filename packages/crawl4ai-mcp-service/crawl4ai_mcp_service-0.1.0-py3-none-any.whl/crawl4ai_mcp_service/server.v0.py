        ...
        try:
            async with AsyncWebCrawler(
                ...
                if hasattr(result, "markdown_v2") and result.markdown_v2:
                    ...
                    logger.debug(f"Content preview: {preview.replace("\n", " - ")}")
                    logger.info(
                ...
                if result.success:
                    if not hasattr(result, "markdown_v2") or not result.markdown_v2:
                        ...
                        status, content, error_msg =  False, "", "No markdown content generated"

                    if not hasattr(result.markdown_v2, "raw_markdown"):
                        logger.error("No raw_markdown attribute in markdown_v2")
                        status, content, error_msg =  False, "", "No raw markdown content available"

                    content = result.markdown_v2.raw_markdown
                    if not content:
                        logger.error("Empty markdown content")
                        status, content, error_msg =  False, "", "Empty content extracted"

                    logger.info(
                        f"Successfully extracted {len(content)} characters of content"
                    )
                    status, content, error_msg =  True, content, ""
                else:
                    status, content, error_msg =  False, "", result.error_message
            ...
            status, content, error_msg = False, "", error_msg

        finally:
            
            logger.debug(f"Fetch result - Success: {status}, Error: {error_msg}")
            return status, content, error_msg
