...
def rhealpix2geojson(rhealpix_code):
    ...
    planar_cell_width = round(rdggs.cell_width(resolution, plane=True),2) # If plane = False, then return None, because the ellipsoidal cells don't have constant width.
    geodesic_cell_area = round(rdggs.cell_area(resolution, plane=False),2)
    ...
    center_lon = round(sum(longitudes) / len(longitudes),6)
    center_lat = round(sum(latitudes) / len(latitudes),6)
