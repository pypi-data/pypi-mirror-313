...
def eaggrisea4t2geojson(eaggr_isea4t):
    ...
    eaggr_cell_shape = DggsShape(DggsCell(eaggr_isea4t), DggsShapeLocation.ONE_FACE)._shape
...
def eaggrisea4t2geojson_cli():
    ...
    args = parser.parse_args()
    geojson_data = json.dumps(eaggrisea4t2geojson(args.eaggr))
    print(geojson_data)

def eaggrisea3hgeojson(eaggr_isea3h):
    eaggr_dggs = Eaggr(Model.ISEA3H)
    eaggr_cell_shape = DggsShape(DggsCell(eaggr_isea3h), DggsShapeLocation.ONE_FACE)._shape
    cell_to_shp = eaggr_dggs.convert_dggs_cell_outline_to_shape_string(eaggr_cell_shape,ShapeStringFormat.WKT)
