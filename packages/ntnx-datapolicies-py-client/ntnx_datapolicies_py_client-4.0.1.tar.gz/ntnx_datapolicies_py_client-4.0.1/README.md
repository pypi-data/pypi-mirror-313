# Python Client For Nutanix Data Policies APIs

The Python client for Nutanix Data Policies APIs is designed for Python client application developers offering them simple and flexible access to APIs that manage Policies for Disaster Recovery and Storage.
## Features
...
```sh
<my-env>/bin/pip install ntnx-datapolicies-py-client
```
...
```sh
<your-env>\Scripts\pip.exe install ntnx-datapolicies-py-client
```
...
## Configuration
The python client for Nutanix Data Policies APIs can be configured with the following parameters
...
```python
# Initialize the API
protection_policies_api_instance = ProtectionPoliciesApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# Get a protection policy
try:
    api_response = protection_policies_api_instance.get_protection_policy_by_id(extId)
...
try:
    ...
    api_response = protection_policies_api_instance.update_protection_policy_by_id(body, extId, if_match=etag_value) # Use the extracted etag value
except ApiException as e:
```
...
try:
    api_response = protection_policies_api_instance.list_protection_policies(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=datapolicies&version=v4.0&language=python). This documentation is auto-generated, and the location may change.
