...
class ModelManager:
    ...
    #     """
    #     Generate a response using the specified API or custom model.
    #     :param prompt: Text prompt for the LLM.
    #     :param api: API to use ('openai', 'ollama', 'groq', or 'custom').
    #     :param model_name: Specific model to use for the API.
    #     :return: Generated response or error message.
    #     """
    #     api = api or self.api
    #     if api == 'custom' and self.custom_model:
    #         return self._generate_with_custom_model(prompt)
    #     # Use other APIs for OpenAI, Ollama, or Groq
    #     return super().generate_response(prompt, api=api, model_name=model_name)
