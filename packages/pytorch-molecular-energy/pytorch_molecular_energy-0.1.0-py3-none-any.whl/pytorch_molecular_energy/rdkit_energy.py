...
class RDKitEnergy(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Passed molecules all must have explicit Hydrogens because the force field
        # requires explicit Hydrogens to generate sensible results.
        if AddHs(mol).GetNumAtoms() != mol.GetNumAtoms():
            raise ValueError("Explicit Hydrogens are required.")
        ...
        if n_threads is None:
            ...
            n_threads = multiprocessing.cpu_count() if n_threads is 0 else n_threads
