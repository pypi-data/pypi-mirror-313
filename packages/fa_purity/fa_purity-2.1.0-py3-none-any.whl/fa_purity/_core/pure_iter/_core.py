...
class PureIter(Generic[_T]):
    ...
    def generate(
        ...
        return PureIter(
            _Private(),
            self._new_iter.map(
                lambda i: iter_factory.gen_next(function, init_state, i)
            ),
        )
    ...
    def reduce(self, function: Callable[[_R, _T], _R], init: _R) -> _R:
        """
        Apply the supplied function to the iterable such that:
