...
def test_find_first() -> None:
    ...
    assert (
        Unsafe.compute(items.find_first(lambda n: n > 9999)) == Maybe.empty()
    )
...
def test_bind() -> None:
    items = StreamFactory.from_commands(
        PureIterFactory.from_range(range(5)).map(
            lambda i: Cmd.wrap_impure(lambda: i)
        )
    )
    items2: Callable[
        [int], Stream[int]
    ] = lambda n: StreamFactory.from_commands(
        PureIterFactory.from_list((n, n)).map(
            lambda i: Cmd.wrap_impure(lambda: i)
        )
    )
