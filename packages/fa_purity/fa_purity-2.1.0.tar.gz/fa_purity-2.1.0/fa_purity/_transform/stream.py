...
class StreamTransform:
    ...
    def squash(stm: Stream[Cmd[_T]]) -> Stream[_T]:
        return Unsafe.stream_from_cmd(
            Unsafe.stream_to_iter(stm).bind(iter_factory.squash)
        )
    ...
    def consume(stm: Stream[Cmd[None]]) -> Cmd[None]:
        return Cmd.wrap_impure(
            lambda: iter_factory.deque(
                iter(
                    Unsafe.compute(a)
                    for a in Unsafe.compute(Unsafe.stream_to_iter(stm))
                )
            )
        )
    ...
    def filter_opt(stm: Stream[Optional[_T]]) -> Stream[_T]:
        return Unsafe.stream_from_cmd(
            Unsafe.stream_to_iter(stm).map(iter_factory.filter_none)
        )
    ...
    def until_none(stm: Stream[Optional[_T]]) -> Stream[_T]:
        return Unsafe.stream_from_cmd(
            Unsafe.stream_to_iter(stm).map(iter_factory.until_none)
        )
