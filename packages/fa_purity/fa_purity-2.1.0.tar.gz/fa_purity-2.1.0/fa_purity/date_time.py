...
def new_datetime(raw: RawDatetime) -> ResultE[datetime]:
    ...
    return _handle_overflow(lambda: _handle_value_error(_build)).bind(
        lambda r: r
    )
        ...
        return (
            ...
            .alt(
                lambda _: LibraryBug.new(
                    ValueError(
                        "`DatetimeTZ` plus some delta result is missing a timezone"
                    )
                )
            )
            .to_union()
...
class DatetimeUTC:
    ...
    def __add__(self, delta: timedelta) -> DatetimeUTC:
        return (
            self.assert_utc(self.date_time + delta)
            .alt(LibraryBug.new)
            .to_union()
        )
...
class DatetimeFactory:
    ...
    def to_tz(
        ...
        return (
            DatetimeTZ.assert_tz(item.astimezone(time_zone))
            .alt(LibraryBug.new)
            .to_union()
        )
    ...
    def date_now() -> Cmd[DatetimeUTC]:
        ...
        return Cmd.wrap_impure(_now).map(
            lambda d: DatetimeUTC.assert_utc(d).or_else_call(
                lambda: LibraryBug.new(ValueError(d))
            )
        )
