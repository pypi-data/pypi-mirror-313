...
def _transform_json(
    ...
    return ResultTransform.all_ok(key_values).map(
        lambda x: FrozenDict(dict(x))
    )
...
class Unfolder:
    ...
    def get(cls, item: JsonValue, key: str) -> ResultE[JsonValue]:
        ...
        return (
            cls.to_json(item)
            .alt(cast_exception)
            .bind(lambda d: ResultTransform.try_get(d, key))
        )
...
class JsonUnfolder:
    ...
    def optional(
        ...
        return (
            (
                Maybe.from_result(
                    ResultTransform.try_get(item, key).alt(lambda _: None)
                )
