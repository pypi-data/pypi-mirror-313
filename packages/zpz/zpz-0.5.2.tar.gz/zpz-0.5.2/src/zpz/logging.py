...
def health_check(handler_name: str):
    for h in rootlogger.handlers:
        try:
            name = h._name_
        except AttributeError:
            # The handler is not added by our system. Do not allow that,
            # for otherwise there could be tricky issues.
            if str(h.__module__) == "_pytest.logging":
                # encountered in tests
                name = str(type(h))
            else:
                raise RuntimeError(
                    "logging is already configured; please remove it and try again"
                )
        if name == handler_name:
            raise RuntimeError(f"handler '{name}' is already being used")


def use_console_handler(**kwargs):
    health_check("console")
