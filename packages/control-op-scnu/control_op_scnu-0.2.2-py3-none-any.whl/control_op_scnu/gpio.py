...
if 'win' in system_platform and not os.environ.get('REMOTE_MODE'):
    raise BaseException("请在树莓派/香橙派上使用本库")
else:
    try:
        from control_op.lcd import LCD_2inch4  # 屏幕的包
    except:
        raise BaseException('树莓派/香橙派没有打开SPI接口！')
    import serial
    import Adafruit_DHT
...
class Io:
    ...
    def set_in_out(self, in_or_out: str):
        ...
        """
        if in_or_out == 'IN':
            GPIO.pinMode(self.gpio_num, GPIO.INPUT)
        elif in_or_out == 'OUT':
            GPIO.pinMode(self.gpio_num, GPIO.OUTPUT)
    ...
    def set_io_out(self, dian_ping: str):
        ...
        """
        if dian_ping == 'HIGH':
            GPIO.digitalWrite(self.gpio_num, GPIO.HIGH)
        elif dian_ping == 'LOW':
            GPIO.digitalWrite(self.gpio_num, GPIO.LOW)
    ...
    def get_io_in(self):
        ...
        """
        # 获取GPIO口的输入电平
        if GPIO.digitalRead(self.gpio_num) == 0:
            # 低电平,返回false
            time.sleep(0.01)
            if GPIO.digitalRead(self.gpio_num) == 0:
                # 防抖设计
...
class IoToPwm:
    ...
    def __init__(self, io_num: int, freq: int = 50, duty: int = 50):
        ...
        GPIO.pinMode(self.__pwm_io, GPIO.OUTPUT)
        self.freq = freq
        self.duty = duty
    ...
    def start(self):
        ...
        :return:None
        """
        GPIO.softPwmCreate(self.__pwm_io, 50, 100)
    ...
    def set_duty(self, pwm_duty: int):
        ...
        """
        self.duty = pwm_duty
        GPIO.softPwmWrite(self.__pwm_io, self.duty)
    ...
    def end(self):
        ...
        :return: None
        """
        GPIO.softPwmStop(self.__pwm_io)
...
class CSB:
    ...
    def __init__(self, uw_num: int):
        ...
        self.dis = 0
        GPIO.pinMode(self.__trig_p, GPIO.OUTPUT)
        GPIO.pinMode(self.__echo_p, GPIO.INPUT)
    ...
    def __sent_t_pulse(self):
        ...
        """
        # 发送超声波
        GPIO.digitalWrite(self.__trig_p, 0)
        time.sleep(0.0002)
        GPIO.digitalWrite(self.__trig_p, 1)
        time.sleep(0.0001)
        GPIO.digitalWrite(self.__trig_p, 0)
    ...
    def __wait_for_e(self, value: bool, timeout: int):
        ...
        """
        count = timeout
        while GPIO.digitalRead(self.__echo_p) != value and count > 0:
            count = count - 1
...
class Servo:
    ...
    def __init__(self, pin: int):
        ...
        GPIO.pinMode(self.servo_io, GPIO.INPUT)# 清空PWM口
