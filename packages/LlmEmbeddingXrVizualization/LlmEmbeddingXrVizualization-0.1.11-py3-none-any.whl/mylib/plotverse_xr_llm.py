...
def main(
    ...
    **kwargs,
    ...
    reduced_embeddings.iloc[:, 0] = (
        5
        * (reduced_embeddings.iloc[:, 0] - min(reduced_embeddings.iloc[:, 0]))
        / (max(reduced_embeddings.iloc[:, 0]) - min(reduced_embeddings.iloc[:, 0]))
    )
    reduced_embeddings.iloc[:, 1] = 5 * (
        reduced_embeddings.iloc[:, 1] - min(reduced_embeddings.iloc[:, 1])
    )
    reduced_embeddings.iloc[:, 2] = (
        5
        * (reduced_embeddings.iloc[:, 2] - min(reduced_embeddings.iloc[:, 2]))
        / (max(reduced_embeddings.iloc[:, 2]) - min(reduced_embeddings.iloc[:, 2]))
    )
