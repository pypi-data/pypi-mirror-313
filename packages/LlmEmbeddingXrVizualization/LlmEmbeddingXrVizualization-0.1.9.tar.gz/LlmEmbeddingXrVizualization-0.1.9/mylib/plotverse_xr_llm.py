...
def main(
    ...
    **kwargs,
    ...
    reduced_embeddings[:, 0] = (
        5
        * (reduced_embeddings[:, 0] - min(reduced_embeddings[:, 0]))
        / (max(reduced_embeddings[:, 0]) - min(reduced_embeddings[:, 0]))
    )
    reduced_embeddings[:, 1] = (
        5
        * (reduced_embeddings[:, 1] - min(reduced_embeddings[:, 1]))
        / (max(reduced_embeddings[:, 1]) - min(reduced_embeddings[:, 1]))
    )
    reduced_embeddings[:, 2] = (
        5
        * (reduced_embeddings[:, 2] - min(reduced_embeddings[:, 2]))
        / (max(reduced_embeddings[:, 2]) - min(reduced_embeddings[:, 2]))
    )
