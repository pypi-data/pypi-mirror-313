...
class _Query(_OperationContainer):
    ...
    def query(
        ...
        num_threads = 1
        if args:
            num_threads = int(args.get('query_concurrency', '1'))
    ...
    def query_to_flight_stream(
        ...
        num_endpoints = 1
        if args:
            num_endpoints = int(args.get('num_endpoints', '1'))
        ...
        readers = []
        for e in endpoints:
            standalone_client: flight.FlightClient = flight.FlightClient(
                f'{flight_protocol}://{flight_endpoint}'
            )
            reader = standalone_client.do_get(
                e.ticket,
