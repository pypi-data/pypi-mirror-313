# File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.

from __future__ import annotations

import os
from typing import Any, cast

import pytest

from tests.utils import assert_matches_type
from senso_developers import SensoDevelopers, AsyncSensoDevelopers
from senso_developers.types.orgs import Document, DocumentListResponse
from senso_developers.types.shared import Message

base_url = os.environ.get("TEST_API_BASE_URL", "http://127.0.0.1:4010")


class TestDocuments:
    parametrize = pytest.mark.parametrize("client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    def test_method_create(self, client: SensoDevelopers) -> None:
        document = client.orgs.documents.create(
            org_id="org_id",
            content="content",
            title="title",
        )
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    def test_raw_response_create(self, client: SensoDevelopers) -> None:
        response = client.orgs.documents.with_raw_response.create(
            org_id="org_id",
            content="content",
            title="title",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        document = response.parse()
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    def test_streaming_response_create(self, client: SensoDevelopers) -> None:
        with client.orgs.documents.with_streaming_response.create(
            org_id="org_id",
            content="content",
            title="title",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            document = response.parse()
            assert_matches_type(Document, document, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_create(self, client: SensoDevelopers) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_id` but received ''"):
            client.orgs.documents.with_raw_response.create(
                org_id="",
                content="content",
                title="title",
            )

    @parametrize
    def test_method_retrieve(self, client: SensoDevelopers) -> None:
        document = client.orgs.documents.retrieve(
            document_id="document_id",
            org_id="org_id",
        )
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    def test_raw_response_retrieve(self, client: SensoDevelopers) -> None:
        response = client.orgs.documents.with_raw_response.retrieve(
            document_id="document_id",
            org_id="org_id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        document = response.parse()
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    def test_streaming_response_retrieve(self, client: SensoDevelopers) -> None:
        with client.orgs.documents.with_streaming_response.retrieve(
            document_id="document_id",
            org_id="org_id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            document = response.parse()
            assert_matches_type(Document, document, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_retrieve(self, client: SensoDevelopers) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_id` but received ''"):
            client.orgs.documents.with_raw_response.retrieve(
                document_id="document_id",
                org_id="",
            )

        with pytest.raises(ValueError, match=r"Expected a non-empty value for `document_id` but received ''"):
            client.orgs.documents.with_raw_response.retrieve(
                document_id="",
                org_id="org_id",
            )

    @parametrize
    def test_method_update(self, client: SensoDevelopers) -> None:
        document = client.orgs.documents.update(
            document_id="document_id",
            org_id="org_id",
        )
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    def test_method_update_with_all_params(self, client: SensoDevelopers) -> None:
        document = client.orgs.documents.update(
            document_id="document_id",
            org_id="org_id",
            content="content",
            title="title",
        )
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    def test_raw_response_update(self, client: SensoDevelopers) -> None:
        response = client.orgs.documents.with_raw_response.update(
            document_id="document_id",
            org_id="org_id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        document = response.parse()
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    def test_streaming_response_update(self, client: SensoDevelopers) -> None:
        with client.orgs.documents.with_streaming_response.update(
            document_id="document_id",
            org_id="org_id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            document = response.parse()
            assert_matches_type(Document, document, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_update(self, client: SensoDevelopers) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_id` but received ''"):
            client.orgs.documents.with_raw_response.update(
                document_id="document_id",
                org_id="",
            )

        with pytest.raises(ValueError, match=r"Expected a non-empty value for `document_id` but received ''"):
            client.orgs.documents.with_raw_response.update(
                document_id="",
                org_id="org_id",
            )

    @parametrize
    def test_method_list(self, client: SensoDevelopers) -> None:
        document = client.orgs.documents.list(
            "org_id",
        )
        assert_matches_type(DocumentListResponse, document, path=["response"])

    @parametrize
    def test_raw_response_list(self, client: SensoDevelopers) -> None:
        response = client.orgs.documents.with_raw_response.list(
            "org_id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        document = response.parse()
        assert_matches_type(DocumentListResponse, document, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: SensoDevelopers) -> None:
        with client.orgs.documents.with_streaming_response.list(
            "org_id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            document = response.parse()
            assert_matches_type(DocumentListResponse, document, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_list(self, client: SensoDevelopers) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_id` but received ''"):
            client.orgs.documents.with_raw_response.list(
                "",
            )

    @parametrize
    def test_method_delete(self, client: SensoDevelopers) -> None:
        document = client.orgs.documents.delete(
            document_id="document_id",
            org_id="org_id",
        )
        assert_matches_type(Message, document, path=["response"])

    @parametrize
    def test_raw_response_delete(self, client: SensoDevelopers) -> None:
        response = client.orgs.documents.with_raw_response.delete(
            document_id="document_id",
            org_id="org_id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        document = response.parse()
        assert_matches_type(Message, document, path=["response"])

    @parametrize
    def test_streaming_response_delete(self, client: SensoDevelopers) -> None:
        with client.orgs.documents.with_streaming_response.delete(
            document_id="document_id",
            org_id="org_id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            document = response.parse()
            assert_matches_type(Message, document, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    def test_path_params_delete(self, client: SensoDevelopers) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_id` but received ''"):
            client.orgs.documents.with_raw_response.delete(
                document_id="document_id",
                org_id="",
            )

        with pytest.raises(ValueError, match=r"Expected a non-empty value for `document_id` but received ''"):
            client.orgs.documents.with_raw_response.delete(
                document_id="",
                org_id="org_id",
            )


class TestAsyncDocuments:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_create(self, async_client: AsyncSensoDevelopers) -> None:
        document = await async_client.orgs.documents.create(
            org_id="org_id",
            content="content",
            title="title",
        )
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    async def test_raw_response_create(self, async_client: AsyncSensoDevelopers) -> None:
        response = await async_client.orgs.documents.with_raw_response.create(
            org_id="org_id",
            content="content",
            title="title",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        document = await response.parse()
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    async def test_streaming_response_create(self, async_client: AsyncSensoDevelopers) -> None:
        async with async_client.orgs.documents.with_streaming_response.create(
            org_id="org_id",
            content="content",
            title="title",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            document = await response.parse()
            assert_matches_type(Document, document, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_create(self, async_client: AsyncSensoDevelopers) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_id` but received ''"):
            await async_client.orgs.documents.with_raw_response.create(
                org_id="",
                content="content",
                title="title",
            )

    @parametrize
    async def test_method_retrieve(self, async_client: AsyncSensoDevelopers) -> None:
        document = await async_client.orgs.documents.retrieve(
            document_id="document_id",
            org_id="org_id",
        )
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    async def test_raw_response_retrieve(self, async_client: AsyncSensoDevelopers) -> None:
        response = await async_client.orgs.documents.with_raw_response.retrieve(
            document_id="document_id",
            org_id="org_id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        document = await response.parse()
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    async def test_streaming_response_retrieve(self, async_client: AsyncSensoDevelopers) -> None:
        async with async_client.orgs.documents.with_streaming_response.retrieve(
            document_id="document_id",
            org_id="org_id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            document = await response.parse()
            assert_matches_type(Document, document, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_retrieve(self, async_client: AsyncSensoDevelopers) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_id` but received ''"):
            await async_client.orgs.documents.with_raw_response.retrieve(
                document_id="document_id",
                org_id="",
            )

        with pytest.raises(ValueError, match=r"Expected a non-empty value for `document_id` but received ''"):
            await async_client.orgs.documents.with_raw_response.retrieve(
                document_id="",
                org_id="org_id",
            )

    @parametrize
    async def test_method_update(self, async_client: AsyncSensoDevelopers) -> None:
        document = await async_client.orgs.documents.update(
            document_id="document_id",
            org_id="org_id",
        )
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    async def test_method_update_with_all_params(self, async_client: AsyncSensoDevelopers) -> None:
        document = await async_client.orgs.documents.update(
            document_id="document_id",
            org_id="org_id",
            content="content",
            title="title",
        )
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    async def test_raw_response_update(self, async_client: AsyncSensoDevelopers) -> None:
        response = await async_client.orgs.documents.with_raw_response.update(
            document_id="document_id",
            org_id="org_id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        document = await response.parse()
        assert_matches_type(Document, document, path=["response"])

    @parametrize
    async def test_streaming_response_update(self, async_client: AsyncSensoDevelopers) -> None:
        async with async_client.orgs.documents.with_streaming_response.update(
            document_id="document_id",
            org_id="org_id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            document = await response.parse()
            assert_matches_type(Document, document, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_update(self, async_client: AsyncSensoDevelopers) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_id` but received ''"):
            await async_client.orgs.documents.with_raw_response.update(
                document_id="document_id",
                org_id="",
            )

        with pytest.raises(ValueError, match=r"Expected a non-empty value for `document_id` but received ''"):
            await async_client.orgs.documents.with_raw_response.update(
                document_id="",
                org_id="org_id",
            )

    @parametrize
    async def test_method_list(self, async_client: AsyncSensoDevelopers) -> None:
        document = await async_client.orgs.documents.list(
            "org_id",
        )
        assert_matches_type(DocumentListResponse, document, path=["response"])

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncSensoDevelopers) -> None:
        response = await async_client.orgs.documents.with_raw_response.list(
            "org_id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        document = await response.parse()
        assert_matches_type(DocumentListResponse, document, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncSensoDevelopers) -> None:
        async with async_client.orgs.documents.with_streaming_response.list(
            "org_id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            document = await response.parse()
            assert_matches_type(DocumentListResponse, document, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_list(self, async_client: AsyncSensoDevelopers) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_id` but received ''"):
            await async_client.orgs.documents.with_raw_response.list(
                "",
            )

    @parametrize
    async def test_method_delete(self, async_client: AsyncSensoDevelopers) -> None:
        document = await async_client.orgs.documents.delete(
            document_id="document_id",
            org_id="org_id",
        )
        assert_matches_type(Message, document, path=["response"])

    @parametrize
    async def test_raw_response_delete(self, async_client: AsyncSensoDevelopers) -> None:
        response = await async_client.orgs.documents.with_raw_response.delete(
            document_id="document_id",
            org_id="org_id",
        )

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        document = await response.parse()
        assert_matches_type(Message, document, path=["response"])

    @parametrize
    async def test_streaming_response_delete(self, async_client: AsyncSensoDevelopers) -> None:
        async with async_client.orgs.documents.with_streaming_response.delete(
            document_id="document_id",
            org_id="org_id",
        ) as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"

            document = await response.parse()
            assert_matches_type(Message, document, path=["response"])

        assert cast(Any, response.is_closed) is True

    @parametrize
    async def test_path_params_delete(self, async_client: AsyncSensoDevelopers) -> None:
        with pytest.raises(ValueError, match=r"Expected a non-empty value for `org_id` but received ''"):
            await async_client.orgs.documents.with_raw_response.delete(
                document_id="document_id",
                org_id="",
            )

        with pytest.raises(ValueError, match=r"Expected a non-empty value for `document_id` but received ''"):
            await async_client.orgs.documents.with_raw_response.delete(
                document_id="",
                org_id="org_id",
            )
