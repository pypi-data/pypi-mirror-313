...
def run_api_client_mode():
    ...
    def getApiOutput(command: str):
        if hasattr(config, "use_local_api_endpoint") and config.use_local_api_endpoint:
            endpoint = f"http://{NetworkUtil.get_ip()}:{config.httpServerPort}/plain"
        else:
            endpoint = config.web_api_endpoint
