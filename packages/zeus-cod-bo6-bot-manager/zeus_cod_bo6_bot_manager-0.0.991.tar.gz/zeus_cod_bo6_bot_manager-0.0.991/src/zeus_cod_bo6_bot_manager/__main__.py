...
def set_frontend_port(port):
    """Set the frontend port globally."""
...
def clean_ansi_codes(line):
    ...
    ansi_escape = re.compile(r'(?:\x1B[@-_]|[\x80-\x9F]|\x1B\[.*?[A-Za-z])')
...
def read_output(pipe, log_prefix, port_callback=None):
    ...
    for line in iter(pipe.readline, ''):
        ...
        print(f"{log_prefix}: {cleaned_line}")

        # Detect the port dynamically without regex
        if port_callback and "http://localhost" in cleaned_line:
            try:
                # Extract the port from the line
                start_index = cleaned_line.index("http://localhost:") + len("http://localhost:")
                end_index = cleaned_line.index("/", start_index)
                port = int(cleaned_line[start_index:end_index])
                print(f"Detected server port: {port}")
                port_callback(port)
            except (ValueError, IndexError) as e:
                print(f"Error parsing port from line: {cleaned_line} - {e}")
    pipe.close()
