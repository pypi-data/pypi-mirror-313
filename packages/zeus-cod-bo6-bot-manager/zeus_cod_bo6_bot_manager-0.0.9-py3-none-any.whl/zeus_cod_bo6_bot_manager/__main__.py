...
def start_frontend():
    ...
    if not os.path.exists(os.path.join(FRONTEND_DIR, "node_modules")):
        ...
        try:
            subprocess.run(["npm", "install"], cwd=FRONTEND_DIR, check=True, env=get_updated_env())
        except FileNotFoundError:
            print("Error: npm not found. Please ensure Node.js and npm are correctly installed.")
            sys.exit(1)
        except subprocess.CalledProcessError as e:
    ...
    subprocess.run(["npm", "run", "build"], cwd=FRONTEND_DIR, check=True, env=get_updated_env())

    print("Starting Vue.js frontend preview server...")
    vue_process = subprocess.Popen(
        ["npm", "run", "serve"], cwd=FRONTEND_DIR, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=get_updated_env()
    )
