...
#---------------------------------------------------------
    ...
    def _verify_ue_initialization(self) -> bool:
        """Verify UE initialization by checking docker logs for successful TUN interface creation"""
        try:
            ...
            def check_logs(timeout=20):
                ...
                while (time.time() - start_time) < timeout:
                    ...
                    if match:
                        # Extract interface details from the message
                        interface_match = re.search(r"TUN interface\[(\w+),", line)
                        if interface_match:
                            interface = interface_match.group(1)
                            successful_interfaces.add(interface)
                            logger.info(f"Detected successful creation of {interface}")
                            
                            if len(successful_interfaces) >= num_ues:
                                logger.info(f"All {num_ues} UE interfaces created successfully")
                                process.terminate()
                                return True
                                
                process.terminate()
                logger.warning(f"Timeout reached. Only {len(successful_interfaces)} interfaces created out of {num_ues}")
                return False

            # Check for successful interface creation
            if check_logs():
                # Verify interfaces physically exist
                result = subprocess.run(
