...
#---------------------------------------------------------
    ...
    def _monitor_background_processes(self, timeout: int = 60):
        ...
        while (time.time() - start_time) < timeout:
            try:
                # Check UE API by trying to get UE IP from first UE port
                ue_ready = False
                try:
                    response = requests.get(f"{self.ue_base_url}:8080/get_ue_ip", timeout=1)
                    ue_ready = response.status_code == 200
                    if ue_ready:
                        logger.info("UE API endpoints are ready")
                except requests.RequestException:
                    pass
                
                # Check UPF API by trying receive endpoint
                upf_ready = False
                try:
                    response = requests.get(f"{self.upf_base_url}/receive", timeout=1)
                    # 204 means no data but API is working
                ...
                self._background_process_status.update({
                    'ue_api_ready': ue_ready,
                    'upf_api_ready': upf_ready,
                    'error_message': None
                })
                
                if ue_ready and upf_ready:
                    self._run_background_nodes_complete = True
                    logger.info("All APIs are running and accessible")
                    return
