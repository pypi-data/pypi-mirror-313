    ...
    def _monitor_background_processes(self, timeout: int = 60):
        ...
        while (time.time() - start_time) < timeout:
            try:
                ...
                ue_ready = False
                try:
                    response = requests.get(f"{self.ue_base_url}/status", timeout=1)
                ...
                upf_ready = False
                try:
                    response = requests.get(f"{self.upf_base_url}/status", timeout=1)
                    upf_ready = response.status_code == 200
                except requests.RequestException:
                    pass
        ...
        # If we get here, we've timed out
        self._background_process_status['error_message'] = "Timeout waiting for services to start"
        logger.error(self._background_process_status['error_message'])
