...
class AtomValueFact(Identifiable, AbstractValueFact[AtomValueType, AbstractValue], PrunableMixin):
    ...
    def _collect_not_empty_links(self, links: Iterable) -> Iterator[Hashable]:
        from tdm.datamodel.facts import PropertyFact

        for p in links:
            if isinstance(p, PropertyFact):
                yield p.source  # TODO: return prop fact instead of cpt fact? or maybe (prop_type, cpt_fact) pair
    ...
    def choose_one(
        ...
        if not self.value:  # equal empty values => check links
            ...
            links = set(self._collect_not_empty_links(map(restore_element, related(self.id))))
            other_links = set(self._collect_not_empty_links(map(restore_element, related(other.id))))
            if not (links <= other_links or other_links <= links):
                return None
