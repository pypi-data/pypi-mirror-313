...
class StochasticOptimizer:
    ...
    def initialize(self) -> None:
        ...
        print("Optimization decision variables:")
        print(self.decision_variables)
        ...
        if self.using_constraints:
            print("Adding constraints to the optimization:")
            for raw_constraint in self._constraints:
                ...
                print(
                    f"{raw_constraint['id']}: "
                    f"{raw_constraint['ConstrainedKpi']} {raw_constraint['ConstraintType']} {raw_constraint['ConstraintValue']} "
        ...
        print("Setting up uncertainty analyzer to be used in optimization.")
        ...
        print("Setting up optimization Task.")
        def task(
    ...
    def run(self) -> tuple:
        ...
        with Timer("[Run stochastic optimization]") as t:
            ...
            t.display_message("Running optimization. This may take a while...")
            opt.run()
            t.display_message(f"Optimization finished after {opt.number_of_evaluations} Task evaluations. Retrieving results...")
    ...
    def _obtain_kpi_results(self, optimal_variables: dict[str, int|float]) -> pandas.DataFrame:
        ...
        return pandas.DataFrame(results, index=[0], columns=(
            'Objective',
            'KPI',
            'SimulationRun',
            'SubDataset',
...
def _compute_objective(
    ...
    if KPI != "IndividualOnTimeAndInLateFillRateServiceLevel":
        ...
        df_service = results["StocksAtEndOfSimulation"][
            results["StocksAtEndOfSimulation"]["Indicator"] == "OnTimeAndInLateFillRateServiceLevel"
        ]
...
def _retrieve_constrained_kpi_values(ua_results: dict, constraints: list) -> dict:
    """
