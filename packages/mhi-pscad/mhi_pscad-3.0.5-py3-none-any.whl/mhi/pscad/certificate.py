...
class Certificate:
    ...
    def find_cert(certs, xml):
        ...
        if lic.tag != 'License':
            lic = lic.find('.//License')
            if lic is None:
                raise ValueError("Invalid Certificate XML")
    ...
    def __str__(self):
        return (f"Certificate[{self.name()} ({self.account()}), "
                f"{self.available()}/{self.total()}]")
...
class Feature:
    ...
    def cost(self):
        ...
        if self._owned > 0:
            fixed, variable = Feature.COSTS.get(self._name,
                                                Feature.DEFAULT_COST)
            cost = fixed
            if self._value is not None:
                cost += variable * self._value
        else:
            cost = 0
