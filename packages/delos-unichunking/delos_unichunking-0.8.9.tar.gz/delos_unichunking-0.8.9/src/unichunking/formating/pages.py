...
async def compute_pages(
    ...
    with pymupdf.Document(path.with_suffix(".pdf")) as doc:
        ...
        logger.debug(f"Pdf.page_count ({doc.page_count}) does not match num_pages ({num_pages}).")
        logger.debug(f"Computing page numbers with minimal intersection length {min_intersection_length}...")
