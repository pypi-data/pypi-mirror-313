...
if "actionId" not in st.session_state:
    st.session_state.actionId = 0
...
def show_df():
    action = gd_datatable(
        ...
        width=800,
        actionId=st.session_state.actionId,
        key="datatable",
    )
    ...
    if action:
        ...
        if action["actionId"] > st.session_state.actionId:
            st.session_state.df = apply_action(action, st.session_state.df)
            st.session_state.actionId = action["actionId"]
