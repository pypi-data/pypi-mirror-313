...
def get_data_from_oracle(table_map, mess, table_scn, col_desc, table_metadata=None, page=None, page_rn=None):
    ...
    def ready():
        try:
            ...
            return src_db, dst_db, once_num, rs, if_exists_clob, if_exists_blob, merge_col, cols_for_copy
        except Exception as _:
    ...
    try:
        tp = ThreadPool(1)
        tid = tp.run(ready)
        src_db, dst_db, once_num, rs, if_exists_clob, if_exists_blob, merge_col, cols_for_copy = tp.get_result(tid, timeout=60)
    except Exception as _:
...
def get_data_from_sql(table_map): 
    ...
    try:
        ...
        while True:
            ...
            if len(rows) == 0:
                break
            tp.run(save_to_pg, (dst_db, table_map, rows, cols, mess, select_time, '', if_exists_blob))
            i = i + 1
        tp.wait()
    except Exception as _:
