...
def bag_create(bag, elements, multiples, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(BagCreateOp(bag=bag, elements=elements, multiples=multiples, loc=loc, ip=ip))
...
def bag_difference(original, diff, *, inf=None, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(BagDifferenceOp(original=original, diff=diff, inf=inf, loc=loc, ip=ip))
...
def bag_select_random(bag, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(BagSelectRandomOp(bag=bag, loc=loc, ip=ip))
...
def invoke_sequence(sequence, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(InvokeSequenceOp(sequence=sequence, loc=loc, ip=ip))
...
def sequence_closure(sequence, args, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(SequenceClosureOp(sequence=sequence, args=args, loc=loc, ip=ip))
...
def sequence(sym_name, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(SequenceOp(sym_name=sym_name, loc=loc, ip=ip))
...
def set_create(set, elements, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(SetCreateOp(set=set, elements=elements, loc=loc, ip=ip))
...
def set_difference(original, diff, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(SetDifferenceOp(original=original, diff=diff, loc=loc, ip=ip))
...
def set_select_random(set, *, loc=None, ip=None) -> _ods_ir.Value:
  return _get_op_result_or_op_results(SetSelectRandomOp(set=set, loc=loc, ip=ip))
...
class TargetOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, target, *, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_390')) else
      _ods_ir.AttrBuilder.get('anonymous_390')(target, context=_ods_context))
    _ods_successors = None
    super().__init__(self.build_generic(attributes=attributes, results=results, operands=operands, successors=_ods_successors, regions=regions, loc=loc, ip=ip))
...
def target(sym_name, target, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(TargetOp(sym_name=sym_name, target=target, loc=loc, ip=ip))
...
def test(sym_name, target, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(TestOp(sym_name=sym_name, target=target, loc=loc, ip=ip))
