...
app.add_typer(sub_app, name="subscription", help="Manage subscriptions.")
...
def _subscriptions_url(obj: Common) -> str:
    return "{}/registry/subscription".format(obj.hub_api_url.rstrip("/"))


def _subscription_url(obj: Common, identifier: str) -> str:
    return "{}/registry/subscription/{}".format(obj.hub_api_url.rstrip("/"), identifier)
...
def list_subscriptions(
    ctx: typer.Context,
    core_identifier: typing.Optional[str] = typer.Option(
        None,
        "--core-id",
        "-cid",
        help="Core identifier",
    ),
    status: typing.Optional[SubscriptionStatus] = typer.Option(None),
    account: typing.Optional[str] = ACCOUNT_OPT,
    _fields: util.Fields = None,
    _output: util.Output = constant.Output.json.value,
    _verbose: util.Verbosity = 0,
...
def update_subscription(
    ...
    util.put_and_process(
        ctx,
        constant.REGISTRY,
        _subscription_url(ctx.obj, identifier),
        account=account,
        json={
            "status": status.value,
            "reason": reason,
