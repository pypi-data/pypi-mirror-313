...
class ZedPromptMixin:
    ...
    @property
    def arguments(self) -> Sequence[ExtendedPromptArgument]:
        ...
        return [
            ExtendedPromptArgument(
                ...
                description=(
                    ...
                    f"(Original args: {', '.join(a.name for a in self.original_prompt.arguments)})"
                ),
                required=True,
            )
        ]
...
def decode_zed_args(arguments: dict[str, Any] | None) -> dict[str, Any]:
    ...
    first_arg_name = next(
        (arg.name for arg in arguments if arg.required), "input"
    )
