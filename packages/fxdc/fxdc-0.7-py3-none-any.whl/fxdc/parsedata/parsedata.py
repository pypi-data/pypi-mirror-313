    ...
    def parse(self):
        ...
        while self.current_token.type != TT_EOF:
            ...
            if self.current_token.type != TT_IDENTIFIER:
                raise InvalidData(f"Expected identifier, got {self.current_token} at line {self.current_token.line}")
            ...
            if self.current_token.type == TT_DEVIDER:
                ...
                if self.current_token.type != TT_KEYWORD:
                    raise InvalidData(f"Expected keyword class, got {self.current_token} at line {self.current_token.line}\nMake sure you have imported the class in the config file")
                type_ = self.current_token.value
                self.advance()
            self.get_indent_count()
            if self.current_token.type not in  (TT_EQUAL, TT_COLON):
                raise InvalidData(f"Expected equal sign/colon, got {self.current_token} at line {self.current_token.line}")
            
            if self.current_token.type == TT_COLON:
                self.advance()
                self.get_indent_count()
                if self.current_token.type != TT_NEWLINE:
                    raise InvalidData(f"Expected new line, got {self.current_token} at line {self.current_token.line}")
                self.advance()
                indentcount = self.get_indent_count()
                debug(indentcount)
                if indentcount == 0:
                    raise InvalidData(f"Expected indented block, got {self.current_token} at line {self.current_token.line}")
                ...
                if self.current_token.type not in (TT_STRING, TT_NUMBER, TT_FLOAT):
                    raise InvalidData(f"Expected value, got {self.current_token} at line {self.current_token.line}")
            
                value = self.current_token.value
                if type_:
                    if type_ == 'str':
                        if not self.current_token.type == TT_STRING:
                            raise InvalidData(f"Expected string, got {self.current_token.type} at line {self.current_token.line}")
                        value = str(value)
                    elif type_ == 'int':
                        if not self.current_token.type == TT_NUMBER:
                            raise InvalidData(f"Expected number, got {self.current_token.type} at line {self.current_token.line}")
                        try:
                            value = int(value)
                        except ValueError:
                        ...
                        if not self.current_token.type == TT_FLOAT:
                            raise InvalidData(f"Expected float, got {self.current_token.type} at line {self.current_token.line}")
                        try:
                            value = float(value)
                        except ValueError:
